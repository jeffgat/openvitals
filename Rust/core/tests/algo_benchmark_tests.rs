...
fn algo_benchmark_reports_runtime_coverage_and_label_error() {
    ...
    let status = Command::new(env!("CARGO_BIN_EXE_open-vitals-algo-benchmark"))
    ...
    assert_eq!(report["schema"], "open_vitals.algo-benchmark-report.v1");
...
fn algo_benchmark_reference_comparison_supports_sleep_v1_input() {
    ...
    assert_eq!(report["data_coverage"]["input_ids_count"], 1);
    assert_eq!(
        report["data_coverage"]["open_vitals_output_data_coverage_fraction"],
        0.92
    );
    assert!(
  ...
  "input_ids": ["synthetic.bad.hrv"]
    ...
    assert!(
        report["next_actions"]
            ...
            .unwrap()
            .iter()
            .any(|action| action["reason"] == "open_vitals_algorithm_error")
    );
}
