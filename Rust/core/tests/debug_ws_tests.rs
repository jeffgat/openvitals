    ...
    append_debug_event(
        ...
        &DebugEventInput {
            ...
            data: json!({
                "export_job_id": "export-1",
                "row_count": 3,
                "bundle_path": "export.openvitalsbundle"
            }),
        },
    )
    .unwrap();
...
fn debug_ws_server_streams_persisted_events_over_loopback_websocket() {
    ...
    let url = format!("ws://127.0.0.1:{port}/open-vitals-debug/stream?token=test-token");
    ...
    assert_eq!(first_event["schema"], "open_vitals.debug.event.v1");
...
fn debug_ws_server_reports_empty_stream_as_validation_gap() {
    ...
    let listener = bind_debug_ws_listener(&options).unwrap();
    let port = listener.local_addr().unwrap().port();
    let url = format!("ws://127.0.0.1:{port}/open-vitals-debug/stream?token=empty-token");
...
fn debug_ws_server_rejects_missing_or_wrong_token_at_handshake() {
    ...
    let listener = bind_debug_ws_listener(&options).unwrap();
    let port = listener.local_addr().unwrap().port();
    let bad_url = format!("ws://127.0.0.1:{port}/open-vitals-debug/stream?token=wrong-token");
    let server_options = options.clone();
    let server = thread::spawn(move || serve_debug_ws_listener_once(listener, server_options));
    ...
    assert!(report.next_actions.iter().any(|action| {
        action.scope == "debug_ws_handshake"
            ...
            && action
                .action
                .contains("path /open-vitals-debug/stream and the current per-session token")
    }));
}
