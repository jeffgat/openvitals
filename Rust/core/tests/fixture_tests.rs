...
use open_vitals_core::activity_candidates::{
...
use open_vitals_core::historical_sync::{
...
use open_vitals_core::openwhoop_reference::{
...
fn indexes_synthetic_fixture_with_required_metadata_and_checksum() {
    ...
    let fixture = index
        .fixtures
        .iter()
        .find(|fixture| fixture.id == "synthetic.open_vitals.v5.get_hello_frame")
        .unwrap();
    assert_eq!(fixture.id, "synthetic.open_vitals.v5.get_hello_frame");
...
fn indexes_gen5_historical_sync_command_validation_fixture_with_sequence_and_row_metadata() {
    ...
    let fixture = index
        ...
        .iter()
        .find(|fixture| {
            fixture.id == "synthetic.open_vitals.v5.historical_sync_gen5_dry_run_command_validation"
        })
        .unwrap();
...
fn indexes_packet_derived_activity_candidate_fixture_with_provenance_and_reasons() {
    ...
    assert!(
        hr_only
            ...
            == Some("synthetic.open_vitals.v5.historical_k18_packet")
    );
    ...
    assert!(
        motion_only
            ...
            .and_then(|value| value.get("fixture_id"))
            .and_then(serde_json::Value::as_str)
            == Some("synthetic.open_vitals.v5.k10_motion_summary_short")
    );
...
fn indexes_openwhoop_gen5_history_plan_fixture_with_command_planning() {
    ...
    assert_eq!(
        fixture.schema,
        open_vitals_core::historical_sync::HISTORICAL_SYNC_DRY_RUN_SCHEMA
    );
    assert_eq!(fixture.kind, "synthetic");
    assert!(fixture.source.contains(OPENWHOOP_REFERENCE_COMMIT));
    assert!(fixture.notes.contains(OPENWHOOP_REFERENCE_LICENSE_CAVEAT));
...
fn parser_runner_validates_indexed_frame_fixture_expectations() {
    ...
    let historical = report
        .fixtures
        .iter()
        .find(|fixture| fixture.id == "synthetic.open_vitals.v5.historical_k18_packet")
        .unwrap();
    assert_eq!(
        historical
            .parsed
    ...
    for (fixture_id, expected_summary_kind) in [
        (
            "synthetic.open_vitals.v5.r17_optical_summary",
            "r17_optical_or_labrador_filtered",
        ),
        (
            "synthetic.open_vitals.v5.k10_motion_summary_short",
            "raw_motion_k10",
        ),
        (
            "synthetic.open_vitals.v5.k21_motion_summary_short",
            "raw_motion_k21",
        ),
...
fn parser_runner_reports_next_action_for_missing_expected_fields() {
    let tempdir = tempfile::tempdir().unwrap();
    let valid_frame =
        fs::read_to_string("fixtures/synthetic/open_vitals_v5_get_hello_frame.hex").unwrap();
...
fn planned_activity_session_subset(
    session: &open_vitals_core::health_sync::PlannedActivityHealthWrite,
) -> serde_json::Value {
    json!({
        ...
        "open_vitals_marker": &session.open_vitals_marker,
    })
}
...
fn historical_sync_step(
    report: &open_vitals_core::historical_sync::HistoricalSyncDryRunReport,
