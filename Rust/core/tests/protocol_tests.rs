...
#[test]
fn parses_hand_derived_open_vitals_v5_get_hello_frame() {
...
fn payload_crc_mismatch_preserves_parseable_header_with_warning() {
    ...
    let parsed = parse_frame(DeviceType::OpenVitals, &frame).unwrap();
...
fn malformed_length_fails_safely() {
    ...
    let error = parse_frame(DeviceType::OpenVitals, &frame).unwrap_err();
...
fn normal_history_zero_hr_marker_is_not_treated_as_hr_present() {
    ...
    payload.resize(18, 0);
    payload[17] = 0;
    let parsed = parse_frame(DeviceType::OpenVitals, &build_v5_payload_frame(&payload)).unwrap();
