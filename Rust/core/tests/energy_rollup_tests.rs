use open_vitals_core::{
    energy_rollup::{
        EnergyDailyRollupOptions, OPENVITALS_ENERGY_UNAVAILABLE_STATUS_V0_ID,
        OPENVITALS_ENERGY_UNAVAILABLE_STATUS_V0_VERSION,
        rollup_energy_unavailable_daily_status_for_store,
    },
    store::{DailyActivityMetricInput, OpenVitalsStore},
};
...
fn energy_unavailable_status_skips_calories_when_available_metric_exists() {
    ...
    store
        .upsert_daily_activity_metric(DailyActivityMetricInput {
            ...
            provenance_json: r#"{"algorithm":"open_vitals.energy.local_estimate.v0","source_kind":"local_estimate"}"#,
        })
        .unwrap();
