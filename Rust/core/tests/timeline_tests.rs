...
fn timeline_normalizes_command_event_history_and_batch_rows() {
    ...
    let command = rows
        .iter()
        .find(|row| row.evidence_id == "synthetic.open_vitals.v5.get_hello_frame")
        .unwrap();
    ...
    let event = rows
        .iter()
        .find(|row| row.evidence_id == "synthetic.open_vitals.v5.temperature_event")
        .unwrap();
    ...
    let historical = rows
        .iter()
        .find(|row| row.evidence_id == "synthetic.open_vitals.v5.historical_k18_packet")
        .unwrap();
