...
fn bridge_runs_historical_sync_dry_run() {
    let response = request(serde_json::json!({
        ...
        "args": {
            "schema": "open_vitals.historical-sync-dry-run.v1",
            "generation": "gen5",
...
fn bridge_validates_historical_sync_physical_evidence() {
    let response = request(serde_json::json!({
        ...
        "args": {
            "schema": "open_vitals.historical-sync-physical-validation.v1",
            "generation": "gen5",
    ...
    assert_eq!(
        result["generated_by"],
        "open-vitals-historical-sync-physical-validator"
    );
    assert_eq!(result["pass"], true);
...
fn bridge_returns_historical_sync_physical_evidence_template() {
    ...
    assert_eq!(
        result["schema"],
        "open_vitals.historical-sync-physical-evidence-template.v1"
    );
    assert_eq!(result["generation"], "gen5");
...
fn bridge_exposes_algorithm_registry_and_score_methods() {
    ...
    assert!(
        definitions
            ...
            .unwrap()
            .iter()
            .any(|definition| definition["algorithm_id"] == "open_vitals.recovery.v0")
    );
    assert!(
        definitions
            .as_array()
            .unwrap()
            .iter()
            .any(|definition| definition["algorithm_id"] == "open_vitals.sleep.v1")
    );
    ...
    let sleep = request(serde_json::json!({
        ...
        "method": "metrics.open_vitals_sleep_v0",
        "args": {
    ...
    let sleep_v1 = request(serde_json::json!({
        ...
        "method": "metrics.open_vitals_sleep_v1",
        "args": {
            "start_time": "2026-05-27T22:30:00Z",
            "end_time": "2026-05-28T06:30:00Z",
            "sleep_duration_minutes": 420.0,
    ...
    assert_eq!(sleep_v1_result["algorithm_id"], "open_vitals.sleep.v1");
    ...
    let external_sleep_comparison = request(serde_json::json!({
        ...
        "args": {
            ...
            "reference_report": {
                "schema": "open_vitals.reference-algo-report.v1",
                "family": "sleep",
...
fn bridge_runs_perf_budget_for_debug_surface() {
    ...
    assert!(response.ok, "{:?}", response.error);
    let result = response.result.unwrap();
    assert_eq!(result["schema"], "open_vitals.perf-budget-report.v1");
    assert_eq!(result["generated_by"], "open-vitals-perf-budget");
...
fn bridge_runs_property_suite_for_debug_surface() {
    ...
    assert!(response.ok, "{:?}", response.error);
    let result = response.result.unwrap();
    assert_eq!(result["schema"], "open_vitals.property-test-report.v1");
    assert_eq!(result["generated_by"], "open-vitals-property-test-suite");
...
fn bridge_persists_algorithm_preferences_for_settings_algorithms() {
    ...
    let set_debug = request(serde_json::json!({
        ...
        "args": {
            ...
            "metric_family": "sleep",
            "algorithm_id": "open_vitals.sleep.v0",
            "version": "0.1.0"
        }
    }));
...
fn bridge_applies_stored_calibration_run_to_local_score() {
    ...
    let response = request(serde_json::json!({
        ...
        "args": {
            "database_path": db_path,
            "metric_family": "recovery",
            "algorithm_id": "open_vitals.recovery.v0",
    ...
    assert_eq!(result["output_kind"], "open_vitals_calibrated_local_score");
...
fn bridge_imports_and_lists_user_owned_calibration_labels() {
    ...
    assert_eq!(
        ...
        "open_vitals.calibration-label-import-report.v1"
    );
...
fn bridge_evaluates_sleep_v1_stored_labels_with_date_holdout_and_sessions() {
    ...
    assert_eq!(result["algorithm_id"], "open_vitals.sleep.v1");
...
fn bridge_imports_app_captured_frame_batch_into_sqlite_and_timeline() {
    ...
    let response = request(serde_json::json!({
        ...
        "args": {
            "database_path": db_path,
            "parser_version": "open-vitals-core/bridge-test",
            "frames": [
                {
...
fn bridge_builds_capture_observability_timeline_from_packets_and_debug_events() {
    ...
    let import = request(serde_json::json!({
        ...
        "args": {
            "database_path": db_path,
            "parser_version": "open-vitals-core/bridge-observability-test",
            "frames": [
                {
    ...
    let debug_started = request(serde_json::json!({
        ...
        "args": {
            ...
            "bridge": {
                "url": "ws://127.0.0.1:49152/open-vitals-debug/stream?token=test",
                "bind_host": "127.0.0.1",
                "token_required": true,
                "token_present": true,
...
fn bridge_records_capture_session_lifecycle_for_live_owned_capture() {
    ...
    assert_eq!(list_result["schema"], "open_vitals.capture-session-list.v1");
...
fn bridge_manages_local_activity_sessions_metrics_and_intervals() {
    ...
    assert_eq!(
        ...
        "open_vitals.activity-correction-plans.v1"
    );
    ...
    assert_eq!(
        ...
        "open_vitals.activity-correction-result.v1"
    );
    ...
    assert_eq!(
        ...
        "open_vitals.activity-metric-result.v1"
    );
    ...
    assert_eq!(
        ...
        "open_vitals.activity-interval-result.v1"
    );
    ...
    assert_eq!(
        ...
        "open_vitals.activity-interval-list.v1"
    );
    ...
    assert_eq!(
        ...
        "open_vitals.activity-session-delete-result.v1"
    );
...
fn bridge_reports_capture_arrival_plan_for_device_day_readiness() {
    ...
    assert_eq!(
        ...
        "open_vitals.local-health-validation-manifest-review.v1"
    );
    assert_eq!(
...
fn bridge_runs_step_counter_daily_rollup_from_persisted_device_samples() {
    ...
    assert_eq!(
        ...
        "open_vitals.hourly-activity-metric-list.v1"
    );
...
fn bridge_validates_hrv_against_whoop_label_without_promoting_metric() {
    ...
    assert_eq!(report["rr_interval_count"], 4);
    assert_eq!(report["trusted_rr_interval_count"], 4);
    assert_eq!(report["algorithm_id"], OPENVITALS_HRV_V0_ID);
    assert_eq!(report["algorithm_version"], OPENVITALS_HRV_V0_VERSION);
    assert_eq!(
        report["promotion_status"],
    ...
    assert_eq!(
        ...
        "open_vitals.hrv-feature-report.v1"
    );
...

    {
        ...
        store
            .insert_daily_recovery_metric(DailyRecoveryMetricInput {
                ...
                provenance_json: r#"{"owner":"user","bridge_test":true,"algorithm":"open_vitals.hrv.device_sensor.v0"}"#,
            })
            .unwrap();
    }
...
fn bridge_rolls_up_local_energy_into_daily_activity_metric() {
    ...
    assert_eq!(
        metric_provenance_json["algorithm"],
        OPENVITALS_ENERGY_LOCAL_ESTIMATE_V0_ID
    );
    assert_eq!(
        metric_provenance_json["algorithm_version"],
        OPENVITALS_ENERGY_LOCAL_ESTIMATE_V0_VERSION
    );
...

    {
        ...
        store
            .upsert_daily_activity_metric(DailyActivityMetricInput {
                ...
                provenance_json: r#"{"source_kind":"device_counter","algorithm":"open_vitals.steps.device_counter.v0"}"#,
            })
            .unwrap();
        store
            .upsert_daily_activity_metric(DailyActivityMetricInput {
                ...
                provenance_json: r#"{"source_kind":"local_estimate","algorithm":"open_vitals.steps.raw_motion_estimate.v0"}"#,
            })
            .unwrap();
    }
...
fn bridge_builds_local_recovery_score_from_feature_reports_and_provided_vitals() {
    ...
    let response = request(serde_json::json!({
        ...
        "args": {
            ...
            "provided_vitals_provenance_json": "{\"source_kind\":\"device_sensor\",\"decoder\":\"open_vitals_packet_decoder\",\"packet_family\":\"vital_event\",\"source\":\"bridge_test\"}",
            "persist_algorithm_run": true,
    ...
    let export_dir = tempdir.path().join("recovery-export.openvitalsbundle");
    let export = request(serde_json::json!({
        "schema": "open_vitals.bridge.request.v1",
...
fn bridge_persists_command_validation_and_returns_direct_send_gates() {
    ...
    assert_eq!(emulator_report["schema"], "open_vitals.command-evidence.v1");
    ...
    assert_eq!(
        ...
        "open_vitals.command-local-frame-match-report.v1"
    );
    ...
    assert_eq!(
        ...
        "open_vitals.command-direct-send-preflight.v1"
    );
...
fn bridge_records_debug_session_command_events_for_debug_tab_stream() {
    ...
    let started = request(serde_json::json!({
        ...
        "args": {
            ...
            "started_at_unix_ms": 1779840000000u64,
            "bridge": {
                "url": "ws://127.0.0.1:49152/open-vitals-debug/stream?token=session-token",
                "bind_host": "127.0.0.1",
                "token_required": true,
                "token_present": true,
                "remote_bind_enabled": false,
    ...
    let command_started = request(serde_json::json!({
        ...
        "args": {
            ...
            "command": {
                "schema": "open_vitals.debug.command.v1",
...
fn bridge_persists_battery_status_history_debug_events() {
    let tempdir = tempfile::tempdir().unwrap();
    let database_path = tempdir.path().join("open_vitals.sqlite");
...
fn bridge_exports_raw_timeframe_for_debug_export_flow() {
    ...
    let db = tempdir.path().join("open_vitals.sqlite");
    seed_fixture_database(&db);
    let export_dir = tempdir.path().join("debug-export.openvitalsbundle");
    let zip_path = tempdir.path().join("debug-export.openvitalsbundle.zip");
    ...
    let response = request(serde_json::json!({
        ...
        "args": {
            ...
            "start": "2026-05-01T00:00:00Z",
            "end": "2026-05-28T00:00:00Z",
            "app_version": "open-vitals-app/bridge-test",
            "core_version": "open-vitals-core/bridge-test",
            "include_sqlite": true
        }
    }));
...
fn bridge_privacy_lint_serializes_next_actions_for_leaky_artifact() {
    ...
    fs::write(
        &path,
        "ws://127.0.0.1/open-vitals-debug/stream?token=secret\n\
    ...
    assert!(response.ok, "{:?}", response.error);
    let result = response.result.unwrap();
    assert_eq!(result["schema"], "open_vitals.privacy-lint-report.v1");
    assert_eq!(result["pass"], false);
    assert_eq!(result["input_valid"], true);
...
fn bridge_capture_sanitize_redacts_owned_capture_before_privacy_lint() {
    ...
    assert!(response.ok, "{:?}", response.error);
    let result = response.result.unwrap();
    assert_eq!(result["schema"], "open_vitals.capture-sanitize-report.v1");
    assert_eq!(result["pass"], true);
    assert_eq!(result["input_valid"], true);
...
fn bridge_raw_export_honors_selected_data_families() {
    ...
    let db = tempdir.path().join("open_vitals.sqlite");
    seed_fixture_database(&db);
    let export_dir = tempdir.path().join("selected-export.openvitalsbundle");
...
fn bridge_raw_export_honors_metric_and_algorithm_filters() {
    ...
    let db = tempdir.path().join("open_vitals.sqlite");
    seed_fixture_database(&db);
    let export_dir = tempdir.path().join("filtered-export.openvitalsbundle");
    ...
    let response = request(serde_json::json!({
        ...
        "args": {
            ...
            "algorithm_ids": ["open_vitals.hrv.v0"],
...
fn bridge_raw_export_honors_packet_and_sensor_filters() {
    ...
    let export_dir = tempdir
        .path()
        .join("packet-signal-filtered-export.openvitalsbundle");
...
fn bridge_raw_export_can_omit_raw_bytes() {
    ...
    let db = tempdir.path().join("open_vitals.sqlite");
    seed_fixture_database(&db);
    let export_dir = tempdir.path().join("hash-only-export.openvitalsbundle");
...
fn bridge_scaffolds_local_health_validation_manifest_from_database() {
        ...
        connection
            .execute(
                ...
                INSERT INTO decoded_frames (
                    ...
                    parser_version,
                    warnings_json
                ) VALUES (?1, ?2, 'OpenVitals', 2, 0, 2, '0000', '', 1, 1, ?3, 'DATA', ?4, NULL, ?5, 'test', '[]')
                "#,
                (
    ...
    assert_eq!(
        manifest["schema"],
        "open_vitals.local-health-validation-manifest.v1"
    );
    ...
    assert_eq!(
        runbook["schema"],
        "open_vitals.local-health-validation-runbook.v1"
    );
    assert_eq!(
    ...
    assert!(markdown.contains("open-vitals-local-health-validation-suite"));
...
fn c_abi_bridge_roundtrips_json_and_allows_freeing_results() {
    let version_ptr = open_vitals_core_version_json();
    ...
    unsafe { open_vitals_bridge_free_string(version_ptr) };
    ...
    .unwrap();
    let response_ptr = unsafe { open_vitals_bridge_handle_json(request.as_ptr()) };
    ...
    assert!(response.ok, "{:?}", response.error);
    unsafe { open_vitals_bridge_free_string(response_ptr) };

    let null_response_ptr = unsafe { open_vitals_bridge_handle_json(std::ptr::null()) };
    ...
    unsafe { open_vitals_bridge_free_string(null_response_ptr) };
}
    ...
    .enumerate()
    {
        ...
        assert!(
            store
                .insert_algorithm_run(&AlgorithmRunRecord {
                    run_id: run_id.clone(),
                    algorithm_id: "open_vitals.sleep.v1".to_string(),
                    version: "0.1.0".to_string(),
                    start_time: start_time.clone(),
                    end_time,
                    output_json,
