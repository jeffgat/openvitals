use open_vitals_core::{
    ...
    store::{CaptureSessionInput, OpenVitalsStore, RawEvidenceInput},
};
...
fn local_health_validation_suite_accepts_raw_export_directory_bundle() {
    ...
    let db = bundle_dir.join("data/open_vitals.sqlite");
...
fn local_health_validation_suite_accepts_raw_export_zip_bundle() {
    ...
    assert!(extracted_database_path.contains("open-vitals-local-health-validation"));
    ...
    assert_eq!(
        ...
        "OpenVitals Raw Export 2026-06-02/manifest.json"
    );
    assert_eq!(
        report["database_source"]["raw_export_manifest"]["official_labels_are_labels"],
        true
...
fn local_health_validation_suite_scaffolds_manifest_from_raw_export_bundle() {
    ...
    assert!(runbook.contains("open-vitals-local-health-validation-suite"));
...
fn local_health_validation_manifest_review_flags_missing_required_labels() {
    let manifest = json!({
        "schema": "open_vitals.local-health-validation-manifest.v1",
        ...
        connection
            .execute(
                ...
                INSERT INTO decoded_frames (
                    ...
                    parser_version,
                    warnings_json
                ) VALUES (?1, ?2, 'OpenVitals', 2, 0, 2, '0000', '', 1, 1, 18, 'DATA', ?3, NULL, ?4, 'test', '[]')
                "#,
                (
                    format!("frame-{evidence_id}"),
                    evidence_id,
        ...
        report["next_actions"]
            ...
            .any(|action| {
                action["case_id"] == "database_source"
                    ...
                    && action["action"]
                        .as_str()
                        .is_some_and(|action| action.contains("data/open_vitals.sqlite"))
            })
    );
}
...
fn local_health_validation_suite_runs_step_energy_and_recovery_sensor_cases() {
    ...
    assert_eq!(
        report["schema"],
        "open_vitals.local-health-validation-suite-report.v1"
    );
    ...
    assert_eq!(
        ...
        "open_vitals.energy.local_estimate.v0"
    );
    assert_eq!(
        ...
        connection
            .execute(
                ...
                INSERT OR IGNORE INTO decoded_frames (
                    ...
                    parser_version,
                    warnings_json
                ) VALUES (?1, ?2, 'OpenVitals', 2, 0, 2, '0000', '', 1, 1, 11, 'DATA', ?3, NULL, ?4, 'test', '[]')
                "#,
                (
                    format!("frame-{evidence_id}"),
                    evidence_id,
...
fn seed_open_vitals_database(path: &Path) {
    fs::create_dir_all(path.parent().unwrap()).unwrap();
    let store = OpenVitalsStore::open(path).unwrap();
...
fn zip_open_vitals_database(zip_path: &Path, database_path: &Path, archive_path: &str) -> String {
