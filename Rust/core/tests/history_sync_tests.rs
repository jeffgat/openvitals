...
fn historical_sync_validator_cli_writes_template_and_validates_evidence() {
    ...
    let template_output =
        std::process::Command::new(env!("CARGO_BIN_EXE_open-vitals-historical-sync-validator"))
            .args([
                "--template",
                "--generation",
                "gen5",
...
fn stale_preflight_rejects_unsupported_schema_before_any_commands() {
    let mut input = base_input(HistoricalSyncGeneration::Gen5, true, vec![]);
    input.schema = "open_vitals.historical-sync-dry-run.v0".to_string();
    ...
    assert_eq!(
        report.issues,
        vec!["unsupported schema open_vitals.historical-sync-dry-run.v0".to_string()]
    );
    assert_eq!(report.next_actions.len(), 1);
    assert_eq!(
        report.next_actions[0].reason,
...
fn base_input(
    ...
    HistoricalSyncDryRunInput {
        schema: "open_vitals.historical-sync-dry-run.v1".to_string(),
        generation,
...
fn step<'a>(
    report: &'a open_vitals_core::historical_sync::HistoricalSyncDryRunReport,
    kind: HistoricalSyncPlanStepKind,
) -> &'a open_vitals_core::historical_sync::HistoricalSyncPlanStep {
    report
        .steps
        .iter()
