...
#[test]
fn clean_openvitalsbundle_export_passes_privacy_lint() {
    ...
    assert!(
        lint.files
            .iter()
            .any(|file| file.path == "data/open_vitals.sqlite" && file.skipped)
    );
}

#[test]
fn clean_zipped_openvitalsbundle_export_passes_privacy_lint() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("export.openvitalsbundle");
...
fn privacy_lint_rejects_tokens_private_api_material_and_identifiers() {
    ...
    assert!(
        lint.next_actions
            .iter()
            .any(|action| action.action.contains("OpenVitals-owned pseudonyms")),
...
fn privacy_lint_scans_zip_entries() {
    let tempdir = tempfile::tempdir().unwrap();
    let zip_path = tempdir.path().join("bad.openvitalsbundle.zip");
    let file = File::create(&zip_path).unwrap();
    let mut zip = ZipWriter::new(file);
    let options = FileOptions::default().compression_method(CompressionMethod::Stored);
    zip.start_file("data/debug_events.jsonl", options).unwrap();
    zip.write_all(br#"{"data_json":"ws://127.0.0.1/open-vitals-debug/stream?token=secret"}"#)
        .unwrap();
    zip.finish().unwrap();
...
fn assert_rule(report: &open_vitals_core::privacy_lint::PrivacyLintReport, rule: &str) {
    assert!(
        report
            .files
            .iter()
...
fn assert_next_action(report: &open_vitals_core::privacy_lint::PrivacyLintReport, reason: &str) {
    assert!(
        report
            .next_actions
            .iter()
