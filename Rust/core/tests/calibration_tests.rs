use open_vitals_core::{
    ...
    store::{AlgorithmDefinitionRecord, CalibrationRunTimes, OpenVitalsStore},
};
...
fn calibration_run_persists_to_sqlite() {
    ...
    assert_eq!(saved.algorithm_id, "open_vitals.recovery.v0");
...
#[test]
fn applies_passed_calibration_model_to_local_open_vitals_score() {
    let dataset: CalibrationDataset = serde_json::from_str(FIXTURE).unwrap();
    let report = evaluate_linear_calibration(&dataset, &default_options());
    assert!(report.pass);
    let record = calibration_run_record("calibration-run-1", &report).unwrap();
...
fn apply_calibration_rejects_failed_or_mismatched_calibration_runs() {
    let failed_record = open_vitals_core::store::CalibrationRunRecord {
    ...
    assert!(
        application
            .issues
            .iter()
            .any(|issue| issue.contains("calibration run targets open_vitals.sleep.v0"))
    );
    assert!(
        application
            .issues
