...
fn validates_manifest_file_checksum_and_required_shape() {
    ...
    fs::write(
        ...
        format!(
            r#"{{
  "schema_version": "open_vitals.export.v1",
  "app_version": "0.1.0",
  "core_version": "0.1.0",
...
fn rejects_sqlite_manifest_when_raw_bytes_are_redacted() {
    ...
    fs::write(tempdir.path().join("data/open_vitals.sqlite"), sqlite_bytes).unwrap();
  ...
  "official_labels_are_labels": true,
  "files": [{{"path": "data/open_vitals.sqlite", "sha256": "{checksum}", "kind": "sqlite"}}]
}}"#
        ),
    )
    .unwrap();
...
fn exports_sqlite_timeframe_to_jsonl_csv_and_sqlite_bundle() {
    ...
    let export_dir = tempdir.path().join("export.openvitalsbundle");
    ...
    let hrv_result = open_vitals_hrv_v0(&HrvInput {
        ...
        input_ids: vec!["synthetic.open_vitals.v5.get_hello_frame".to_string()],
    });
    ...
    store
        .upsert_algorithm_definition(&AlgorithmDefinitionRecord {
            algorithm_id: "open_vitals.recovery.v0".to_string(),
            version: "0.1.0".to_string(),
            metric_family: "recovery".to_string(),
            display_name: "OpenVitals Recovery v0".to_string(),
            implementation: "rust".to_string(),
            license: "UNLICENSED".to_string(),
            input_schema: "open_vitals.recovery-input.v1".to_string(),
            output_schema: "open_vitals.recovery-output.v1".to_string(),
    ...
    store
        .insert_debug_command(&DebugCommandRow {
            ...
            args_json: r#"{"url":"ws://127.0.0.1/open-vitals-debug/stream?token=secret-token"}"#
                .to_string(),
            dry_run: false,
    ...
    store
        .insert_debug_event(&DebugEventRow {
            ...
            data_json: r#"{"bind_url":"ws://127.0.0.1/open-vitals-debug/stream?token=secret-token&client=agent"}"#
                .to_string(),
        })
        .unwrap();
    ...
    let report = export_raw_timeframe(
        ...
        RawExportOptions {
            ...
            app_version: "open-vitals-app/test",
            core_version: "open-vitals-core/test",
    ...
    assert!(
        report
            ...
            .files
            .iter()
            .any(|file| file.path == "data/open_vitals.sqlite")
    );
    assert!(
        report
            .manifest
    ...
    assert!(export_dir.join("data/open_vitals.sqlite").exists());
    let exported_sqlite = Connection::open(export_dir.join("data/open_vitals.sqlite")).unwrap();
    let schema_version: i64 = exported_sqlite
        .query_row(
            "SELECT version FROM open_vitals_schema_migrations ORDER BY version DESC LIMIT 1",
            [],
            |row| row.get(0),
        )
        .unwrap();
    ...
    assert!(metric_features.contains("open_vitals.motion-feature-report.v1"));
...
fn raw_export_sqlite_family_snapshots_live_wal_database() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("wal-snapshot.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    ...
    assert!(report.pass, "{:?}", report.issues);
    let exported_path = export_dir.join("data/open_vitals.sqlite");
    assert!(exported_path.exists());
    assert!(!export_dir.join("data/open_vitals.sqlite-wal").exists());
    assert!(!export_dir.join("data/open_vitals.sqlite-shm").exists());
...
fn raw_export_can_select_sensor_samples_only() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("sensor-samples.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    let fixture_root = Path::new("fixtures");
    let index = build_fixture_index(fixture_root).unwrap();
    let import_report = import_fixture_index(
...
fn raw_export_sensor_samples_store_sample_time_separate_from_capture_time() {
    ...
    let export_dir = tempdir
        .path()
        .join("timestamped-sensor-samples.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
...
fn raw_export_sensor_samples_reject_invalid_device_timestamp_subseconds() {
    ...
    let export_dir = tempdir
        .path()
        .join("invalid-subsecond-sensor-samples.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
...
fn raw_export_can_select_metric_feature_reports_only() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("metric-features.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    let fixture_root = Path::new("fixtures");
    let index = build_fixture_index(fixture_root).unwrap();
    let import_report = import_fixture_index(
...
fn raw_export_can_select_metric_outputs_only() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("metric-outputs.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    let definition = built_in_algorithm_definitions()
        .into_iter()
        .find(|definition| definition.algorithm_id == "open_vitals.hrv.v0")
        .unwrap();
    store.upsert_algorithm_definition(&definition).unwrap();
    let hrv_result = open_vitals_hrv_v0(&HrvInput {
        start_time: "2026-05-27T00:15:00Z".to_string(),
...
#[test]
fn raw_export_preserves_sleep_v1_output_components_and_open_vitals_provenance() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("sleep-v1-export.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    let definition = built_in_algorithm_definitions()
        .into_iter()
    ...
    let sleep_result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
    ...
    assert_eq!(algorithm_run["algorithm_id"], OPENVITALS_SLEEP_V1_ID);
    ...
    for row in component_rows {
        assert_eq!(row["algorithm_id"], OPENVITALS_SLEEP_V1_ID);
        assert_eq!(
...
fn raw_export_can_export_and_validate_activity_families() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("activity.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
...
fn raw_export_can_export_and_validate_local_health_metric_family() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("local-health.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();

    let daily_inputs = serde_json::json!({
        "algorithm": "open_vitals.energy.local_estimate.v0",
        "raw_payload_hex": "aa11",
        "nested": {
            "frame_hex": "bb22"
        }
    ...
    let unavailable_recovery_provenance = serde_json::json!({
        "algorithm": "open_vitals.recovery.unavailable_status.v0",
    ...
    let unavailable_activity_provenance = serde_json::json!({
        "algorithm": "open_vitals.activity.unavailable_status.v0",
        "source_kind": "unavailable",
        "metric_id": "steps",
    ...
    let unavailable_energy_provenance = serde_json::json!({
        "algorithm": "open_vitals.energy.unavailable_status.v0",
    ...
    assert!(
        store
            .insert_metric_provenance(MetricProvenanceInput {
                ...
                source_kind: "local_estimate",
                source_detail: "open_vitals.energy.local_estimate.v0",
    ...
    assert!(
        store
            .insert_metric_provenance(MetricProvenanceInput {
                ...
                source_kind: "device_counter",
                source_detail: "open_vitals.step_counter_hourly_rollup.v0",
    ...
    assert!(
        store
            .insert_metric_provenance(MetricProvenanceInput {
                ...
                source_kind: "device_sensor",
                source_detail: "open_vitals.resting_hr_daily_rollup.v0",
...
fn export_validator_rejects_algorithm_runs_with_untrusted_provided_vitals() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("bad-algorithm-run.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    ...
    store
        .insert_algorithm_run(&AlgorithmRunRecord {
            ...
            output_json: r#"{"algorithm_id":"open_vitals.recovery.v0","algorithm_version":"0.1.0","score_0_to_100":72.0,"components":[]}"#
                .to_string(),
...
fn export_validator_rejects_failed_or_malformed_calibration_runs() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("bad-calibration-run.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    for definition in built_in_algorithm_definitions() {
        store.upsert_algorithm_definition(&definition).unwrap();
    }
...
fn export_validator_rejects_malformed_debug_rows() {
    let tempdir = tempfile::tempdir().unwrap();
    fs::create_dir(tempdir.path().join("data")).unwrap();
    let session_rows = br#"{"session_id":"debug-session-1","started_at_unix_ms":1,"bridge_url":"ws://0.0.0.0:49152/open-vitals-debug/stream?token=secret","bind_host":"0.0.0.0","token_required":false,"token_present":false,"remote_bind_enabled":true,"visible_remote_bind_toggle":false}
"#;
    let command_rows = br#"{"command_id":"debug-command-1","session_id":"missing-session","schema":"bad.schema","command":"export.raw_timeframe","args_json":"{\"frame_hex\":\"aa\",\"url\":\"ws://127.0.0.1/open-vitals-debug/stream?token=secret\"}","dry_run":false,"received_at_unix_ms":2}
"#;
    let event_rows = br#"{"session_id":"debug-session-1","sequence":1,"schema":"open_vitals.debug.event.v1","time_unix_ms":3,"source":"app","level":"info","topic":"export.started","message":"stream token=secret","command_id":"missing-command","data_json":"{\"payload_hex\":\"aa\",\"url\":\"ws://127.0.0.1/open-vitals-debug/stream?token=secret\"}"}
...
"#;
  ...
  "files": [
    ...
    assert!(report.issues.iter().any(|issue| {
        issue.contains("debug command debug-command-1 schema must be open_vitals.debug.command.v1")
    }));
    assert!(report.issues.iter().any(|issue| {
...
fn raw_export_filters_algorithm_outputs_and_labels() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("filtered-metrics.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    for definition in built_in_algorithm_definitions() {
        store.upsert_algorithm_definition(&definition).unwrap();
    }
    ...
    let report = export_raw_timeframe(
        ...
        RawExportOptions {
            ...
            filters: RawExportFilters {
                ...
                algorithm_ids: vec!["open_vitals.hrv.v0".to_string()],
...
fn raw_export_filters_metric_feature_reports_by_metric_family() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("filtered-features.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    let fixture_root = Path::new("fixtures");
    let index = build_fixture_index(fixture_root).unwrap();
    let import_report = import_fixture_index(
    ...
    for session_id in ["capture-session-a", "capture-session-b"] {
        store
            .start_capture_session(CaptureSessionInput {
                ...
                device_model: "WHOOP 5.0 OpenVitals",
...
fn raw_export_can_omit_raw_bytes_but_keep_hashes_and_decoded_samples() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("hash-only.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
...
fn raw_export_rejects_sqlite_when_raw_bytes_are_omitted() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("hash-only-sqlite.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
...
fn raw_export_rejects_unknown_or_unavailable_data_families() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let unknown_export_dir = tempdir.path().join("unknown.openvitalsbundle");
    let sqlite_export_dir = tempdir.path().join("sqlite.openvitalsbundle");
    let store = OpenVitalsStore::open(&db_path).unwrap();
...
#[test]
fn exports_and_validates_zipped_openvitalsbundle_without_extracting() {
    let tempdir = tempfile::tempdir().unwrap();
    let db_path = tempdir.path().join("open_vitals.sqlite");
    let export_dir = tempdir.path().join("export.openvitalsbundle");
    let zip_path = tempdir.path().join("export.openvitalsbundle.zip");
    let store = OpenVitalsStore::open(&db_path).unwrap();
    let fixture_root = Path::new("fixtures");
    let index = build_fixture_index(fixture_root).unwrap();
    let import_report = import_fixture_index(
...
fn seed_command_validation_record(store: &OpenVitalsStore) {
