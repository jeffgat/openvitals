...
#[test]
fn hrv_feature_extraction_builds_open_vitals_hrv_score_from_trusted_r17_samples() {
    let store = OpenVitalsStore::open_in_memory().unwrap();
    import_r17_frame_at(
        &store,
        "user-owned-live-notification",
...
fn sleep_feature_score_report_smooths_short_non_wake_stage_islands() {
    ...
    assert_eq!(
        ...
        "open_vitals_sleep_stage_heuristic_v1_transition_smoothed"
    );
    assert_eq!(
    ...
    let report = run_recovery_feature_score_report_for_store(
        ...
        RecoveryFeatureScoreOptions {
            ...
            provided_vitals_provenance_json: Some(
                r#"{"source_kind":"device_sensor","decoder":"open_vitals_packet_decoder","packet_family":"vital_event","source":"metric_features_test"}"#.to_string(),
            ),
        },
    )
    .unwrap();
...
fn import_motion_frame(store: &OpenVitalsStore, sensitivity: &str) {
    import_motion_frame_at(store, sensitivity, "2026-05-27T12:00:00Z");
}

fn import_recovery_feature_inputs(store: &OpenVitalsStore) {
...
fn import_motion_frame_at(store: &OpenVitalsStore, sensitivity: &str, captured_at: &str) {
...
fn import_temperature_event(store: &OpenVitalsStore, sensitivity: &str, body: &[u8]) {
    let frames = vec![CapturedFrameInput {
