...
fn seed_legacy_capture_metric_database(db_path: &Path) {
    ...
    conn.execute_batch(
        ...
        CREATE TABLE open_vitals_schema_migrations (
            version INTEGER PRIMARY KEY,
            applied_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))
        );
        INSERT INTO open_vitals_schema_migrations(version) VALUES (1);
        INSERT INTO open_vitals_schema_migrations(version) VALUES (2);
        INSERT INTO open_vitals_schema_migrations(version) VALUES (3);
        INSERT INTO open_vitals_schema_migrations(version) VALUES (4);
        INSERT INTO open_vitals_schema_migrations(version) VALUES (5);
        INSERT INTO open_vitals_schema_migrations(version) VALUES (6);
...
fn migrates_legacy_capture_metric_database_and_keeps_activity_tables_empty() {
    ...
    assert_eq!(store.schema_version().unwrap(), CURRENT_SCHEMA_VERSION);
    assert_eq!(
        store.table_count("open_vitals_schema_migrations").unwrap(),
...
fn stores_raw_evidence_and_decoded_frame_with_provenance_link() {
    ...
    let frame_inserted = store
        .insert_decoded_frame(DecodedFrameInput {
            ...
            evidence_id: "synthetic-frame-1",
            parsed: &parsed,
            parser_version: "open-vitals-core/0.1.0",
        })
        .unwrap();
...
fn daily_metric_rollups_round_trip_with_source_kind_provenance() {
    ...
    let activity = DailyActivityMetricInput {
        ...
        provenance_json: r#"{"owner":"user","algorithm":"open_vitals.activity_totals.v0"}"#,
    };
    assert!(
        store
    ...
    let hourly_activity = HourlyActivityMetricInput {
        ...
        provenance_json: r#"{"owner":"user","algorithm":"open_vitals.activity_hourly.v0"}"#,
    };
    assert!(
        store
    ...
    let recovery = DailyRecoveryMetricInput {
        ...
        provenance_json: r#"{"owner":"user","algorithm":"open_vitals.recovery_daily.v0"}"#,
    };
    assert!(
        store
    ...
    assert!(
        store
            .insert_daily_activity_metric(DailyActivityMetricInput {
                ...
                provenance_json: r#"{"owner":"user","algorithm":"open_vitals.activity.unavailable_status.v0"}"#,
            })
            .unwrap()
    );
    ...
    assert!(
        store
            .insert_daily_activity_metric(DailyActivityMetricInput {
                ...
                provenance_json: r#"{"algorithm":"open_vitals.steps.raw_motion_estimate.v0"}"#,
            })
            .unwrap()
    );
...
fn migrates_v12_daily_recovery_source_unique_table_for_separate_device_metrics() {
    ...
    conn.execute_batch(
        ...
        INSERT INTO daily_recovery_metrics (
            ...
            '{"algorithm":"open_vitals.resting_heart_rate.device_sensor.v0"}'
        );
        "#,
    )
    .unwrap();
    ...
    assert!(
        store
            .insert_daily_recovery_metric(DailyRecoveryMetricInput {
                ...
                provenance_json: r#"{"algorithm":"open_vitals.hrv.device_sensor.v0"}"#,
            })
            .unwrap()
    );
...
fn daily_recovery_metric_upsert_refreshes_same_day_rollup() {
    ...
    let first = DailyRecoveryMetricInput {
        ...
        provenance_json: r#"{"algorithm":"open_vitals.resting_heart_rate.device_sensor.v0"}"#,
    };
    ...
    let refreshed = DailyRecoveryMetricInput {
        ...
        quality_flags_json: r#"["daily_rhr_lowest_quartile_hr"]"#,
        provenance_json: r#"{"algorithm":"open_vitals.resting_heart_rate.device_sensor.v0","refreshed":true}"#,
    };
...
fn daily_activity_metric_upsert_refreshes_same_day_energy_rollup() {
    ...
    let first = DailyActivityMetricInput {
        ...
        provenance_json: r#"{"algorithm":"open_vitals.energy.local_estimate.v0"}"#,
    };
    ...
    let refreshed = DailyActivityMetricInput {
        ...
        inputs_json: r#"{"heart_rate_sample_count":8}"#,
        quality_flags_json: r#"["local_energy_estimate"]"#,
        provenance_json: r#"{"algorithm":"open_vitals.energy.local_estimate.v0","refreshed":true}"#,
    };
...
fn metric_debug_features_round_trip_with_source_kind_and_confidence() {
    ...
    let feature = MetricDebugFeatureInput {
        ...
        provenance_json: r#"{"owner":"user","report":"open_vitals.step-capture-validation-report.v1"}"#,
    };
...
fn algorithm_preferences_select_primary_algorithms_by_scope_and_family() {
    ...
    let recovery = store
        ...
        .unwrap()
        .unwrap();
    assert_eq!(recovery.algorithm_id, "open_vitals.recovery.v0");
    ...
    let override_preference = AlgorithmPreferenceRecord {
        ...
        algorithm_id: "open_vitals.sleep.v0".to_string(),
