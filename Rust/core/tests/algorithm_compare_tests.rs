...
fn hrv_comparison_reports_zero_deltas_for_shared_time_domain_fields() {
    let report = compare_hrv_open_vitals_to_reference(&HrvInput {
    ...
    assert!(report.open_vitals_output_ready);
...
fn sleep_comparison_reports_shared_window_and_actigraphy_summary_fields() {
    let report = compare_sleep_open_vitals_to_reference(&SleepInput {
    ...
    assert_close(report.deltas[0].open_vitals_value, 480.0);
    assert_close(report.deltas[0].reference_value, 480.0);
    assert_close(report.deltas[1].open_vitals_value, 420.0);
    assert_close(report.deltas[1].reference_value, 420.0);
    assert_close(report.deltas[2].open_vitals_value, 60.0);
    assert_close(report.deltas[2].reference_value, 60.0);
    assert_close(report.deltas[3].open_vitals_value, 0.875);
    assert_close(report.deltas[3].reference_value, 0.875);
    assert_close(report.deltas[4].open_vitals_value, 60.0);
    assert_close(report.deltas[4].reference_value, 60.0);
    assert_close(report.deltas[5].open_vitals_value, 4.0);
    assert_close(report.deltas[5].reference_value, 4.0);
    assert_close(report.deltas[6].open_vitals_value, 4.0 / 7.0);
...
fn sleep_v1_comparison_passes_reference_sleep_wake_summary_fields() {
    let report = compare_sleep_v1_open_vitals_to_reference(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
            sleep_duration_minutes: 420.0,
...
fn sleep_comparison_accepts_external_reference_report_output() {
    ...
    let report = compare_sleep_open_vitals_to_external_reference_report(
        &SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
            sleep_duration_minutes: 420.0,
...
fn sleep_v1_comparison_accepts_external_reference_report_output() {
    ...
    let report = compare_sleep_v1_open_vitals_to_external_reference_report(
        &SleepV1Input {
            sleep: SleepInput {
                start_time: "2026-05-27T22:30:00Z".to_string(),
                end_time: "2026-05-28T06:30:00Z".to_string(),
...
fn strain_comparison_reports_edwards_zone_load_delta() {
    let report = compare_strain_open_vitals_to_reference(&StrainInput {
    ...
    assert_close(report.deltas[0].open_vitals_value, 140.0);
...
fn stress_comparison_reports_shared_hr_and_hrv_proxy_deltas() {
    let report = compare_stress_open_vitals_to_reference(&StressInput {
        start_time: "2026-05-28T12:00:00Z".to_string(),
    ...
    assert_close(report.deltas[0].open_vitals_value, 50.0);
    assert_close(report.deltas[0].reference_value, 50.0);
    assert_close(report.deltas[0].absolute_delta, 0.0);
    assert_close(report.deltas[1].open_vitals_value, 50.0);
...
fn comparison_fails_when_both_algorithms_lack_comparable_outputs() {
    ...
    assert!(
        report
            .errors
            .contains(&"openVitals:not_enough_valid_rr_intervals".to_string())
    );
    assert!(
        report
            .errors
