...
fn command_definitions_cover_generated_protocol_command_map_ids() {
    ...
    let open_vitals_ids: std::collections::BTreeSet<u16> = COMMAND_DEFINITIONS
        .iter()
...
fn load_command_evidence_accepts_exported_top_level_json_report() {
    ...
    let path = tempdir.path().join("open-vitals-command-evidence.json");
    let payload = serde_json::json!({
        "schema": "open_vitals.command-evidence.v1",
        "generated_by": "whoop-reversing.open_vitals_command_evidence",
...
fn command_validator_cli_can_emit_capture_plan_for_selected_commands() {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../fixtures/command-evidence/whoop-emulator-command-evidence.json");
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_open-vitals-command-validator"))
        .arg("--evidence")
        .arg(path)
        .arg("--capture-plan")
        .arg("--commands")
...
fn load_command_evidence_accepts_jsonl_rows_from_file_pickers() {
    let tempdir = tempfile::tempdir().unwrap();
    let path = tempdir.path().join("open-vitals-command-evidence.jsonl");
...
fn load_command_local_frame_candidates_accepts_wrapper_and_jsonl_aliases() {
    ...
    let wrapper_path = tempdir
        .path()
        .join("open-vitals-command-local-frame-candidates.json");
    let jsonl_path = tempdir
        .path()
        .join("open-vitals-command-local-frame-candidates.jsonl");
    ...
    std::fs::write(
        ...
        format!(
            r#"{{
              "schema":"open_vitals.command-local-frame-candidates.v1",
              "candidates":[{{
    ...
    std::fs::write(
        &whoop_rev_path,
        format!(
            r#"{{"command":"GET_HELLO","command_id":145,"sequence":1,"payload_hex":"23019101","frame_hex":"{frame}","device_type":"OPENVITALS","send_allowed":false}}"#
        ),
    )
    .unwrap();
    std::fs::write(
...
fn emulator_log_evidence_conversion_keeps_local_frame_match_explicit() {
    ...
    assert_eq!(evidence_report.schema, "open_vitals.command-evidence.v1");
...
fn command_validator_cli_accepts_builder_directory_for_local_frame_candidates() {
    ...
    std::fs::write(
        ...
        format!(
            r#"{{"command":"GET_HELLO","command_id":145,"sequence":1,"frame_hex":"{get_hello_frame}","device_type":"OPENVITALS","send_allowed":false}}"#
        ),
    )
    .unwrap();
    std::fs::write(
        ...
        format!(
            r#"{{"command":"GET_BATTERY_LEVEL","command_id":26,"sequence":2,"frame_hex":"{get_battery_level_frame}","device_type":"OPENVITALS","send_allowed":false}}"#
        ),
    )
    .unwrap();
...
#[test]
fn local_frame_match_promotion_unlocks_exact_open_vitals_dry_run_bytes() {
    let frame = hex::encode(build_v5_command_frame(1, COMMAND_GET_HELLO, &[1]));
    let response = command_response_frame_hex(COMMAND_GET_HELLO);
...
fn ready_get_hello_gate() -> open_vitals_core::commands::CommandDirectSendGate {
    let report = validate_commands(&[ready_get_hello_evidence()]);
    let ready = report
        .commands
        .iter()
