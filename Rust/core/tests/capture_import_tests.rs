...
fn imports_indexed_frame_fixture_into_sqlite_raw_and_decoded_tables() {
    ...
    let event = report
        .fixtures
        .iter()
        .find(|fixture| fixture.id == "synthetic.open_vitals.v5.temperature_event")
        .unwrap();
    ...
    let motion = report
        .fixtures
        .iter()
        .find(|fixture| fixture.id == "synthetic.open_vitals.v5.k10_motion_summary_short")
        .unwrap();
    assert_eq!(motion.parsed_payload_kind.as_deref(), Some("data_packet"));
    let decoded_motion = store
        .decoded_frame("synthetic.open_vitals.v5.k10_motion_summary_short.frame.0")
        .unwrap()
        .unwrap();
...
#[test]
fn imports_processed_capture_sqlite_into_owned_open_vitals_session() {
    let tempdir = tempfile::tempdir().unwrap();
