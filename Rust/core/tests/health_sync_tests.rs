use open_vitals_core::activity_identity::{ActivityIdentityInput, activity_idempotency_key};
...
fn health_sync_maps_steps_and_active_energy_where_platform_supported() {
    ...
    assert!(health_connect.planned_writes.iter().any(|write| {
        write.destination_type == "StepsRecord"
            && write.open_vitals_marker == "openVitals:activity:open_vitals.test.v0:steps-2"
    }));
    assert!(
        health_connect
            .planned_writes
...
#[test]
fn stale_open_vitals_records_inside_backfill_are_planned_for_delete() {
    ...
    let input = HealthSyncDryRunInput {
        ...
        existing_records: vec![
            existing_record(
                ...
                "heartRate",
                "openVitals:HealthKit:heartRate:old:2026-05-27T01:00:00Z:2026-05-27T01:05:00Z",
                "openVitals:heart_rate:open_vitals.test.v0:old",
            ...
            existing_record(
                ...
                "heartRate",
                "openVitals:HealthKit:heartRate:current:2026-05-27T00:00:00Z:2026-05-27T00:05:00Z",
                "openVitals:heart_rate:open_vitals.test.v0:current",
    ...
    assert_eq!(
        ...
        "stale_open_vitals_record_in_backfill"
    );
}

#[test]
fn delete_planning_blocks_non_open_vitals_external_or_unsupported_records() {
    let input = HealthSyncDryRunInput {
        delete_policy: HealthSyncDeletePolicy::StaleInBackfill,
        existing_records: vec![
            existing_record(
            ...
            existing_record(
                ...
                "WorkoutRoute",
                "openVitals:HealthKit:WorkoutRoute:old:2026-05-27T01:00:00Z:2026-05-27T01:05:00Z",
                "openVitals:route:open_vitals.test.v0:old",
                "2026-05-27T01:00:00Z",
                "2026-05-27T01:05:00Z",
            ),
            existing_record(
                ...
                "heartRate",
                "openVitals:HealthKit:heartRate:outside:2026-05-28T01:00:00Z:2026-05-28T01:05:00Z",
                "openVitals:heart_rate:open_vitals.test.v0:outside",
...
fn dry_run_report_next_actions_cover_report_and_cleanup_blockers() {
    let input = HealthSyncDryRunInput {
        schema: "open_vitals.health-sync-dry-run.v1".to_string(),
    ...
    assert!(blocked_delete.next_actions.iter().any(|action| {
        action.reason == "not_open_vitals_owned" && action.action.contains("Do not delete external")
    }));
    assert!(report.next_actions.iter().any(|action| {
        action.scope == "external-platform-record"
            && action.reason == "not_open_vitals_owned"
...
fn health_sync_cli_report_separates_valid_input_from_partial_plan() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_open-vitals-health-sync-dry-run"))
        .arg("--input")
        .arg(input_path)
        .output()
        .unwrap();
...
fn activity_input(
    ...
    ActivityHealthSyncDryRunInput {
        schema: "open_vitals.activity-health-sync-dry-run.v1".to_string(),
        platform,
        permission_grants,
        backfill: HealthSyncWindow {
            start: "2026-05-27T00:00:00Z".to_string(),
...
fn candidate(
    ...
    HealthSyncCandidate {
        ...
        value: 60.0,
        unit: unit.to_string(),
        algorithm_id: Some("open_vitals.test.v0".to_string()),
...
fn existing_record(
    ...
    ExistingHealthRecord {
        ...
        open_vitals_marker: open_vitals_marker.to_string(),
        start_time: start_time.to_string(),
        end_time: end_time.to_string(),
