...
#[test]
fn open_vitals_hrv_v0_computes_hand_derived_time_domain_metrics() {
    let result = open_vitals_hrv_v0(&HrvInput {
    ...
    assert_eq!(output.algorithm_id, OPENVITALS_HRV_V0_ID);
    assert_eq!(output.algorithm_version, OPENVITALS_HRV_V0_VERSION);
...
#[test]
fn open_vitals_hrv_v0_pnn50_uses_strictly_greater_than_50_ms() {
    let result = open_vitals_hrv_v0(&HrvInput {
        start_time: "2026-05-27T00:00:00Z".to_string(),
        end_time: "2026-05-27T00:01:00Z".to_string(),
...
#[test]
fn open_vitals_hrv_v0_drops_nonphysiological_intervals_and_flags_quality() {
    let result = open_vitals_hrv_v0(&HrvInput {
        start_time: "2026-05-27T00:00:00Z".to_string(),
        end_time: "2026-05-27T00:01:00Z".to_string(),
...
#[test]
fn open_vitals_hrv_v0_reports_insufficient_data_without_output() {
    let result = open_vitals_hrv_v0(&HrvInput {
        start_time: "2026-05-27T00:00:00Z".to_string(),
        end_time: "2026-05-27T00:01:00Z".to_string(),
...
fn hrv_definition_and_run_persist_to_sqlite() {
    ...
    let saved = store
        .algorithm_definition(OPENVITALS_HRV_V0_ID, OPENVITALS_HRV_V0_VERSION)
        .unwrap()
        .unwrap();
    ...
    assert_eq!(saved_run.algorithm_id, OPENVITALS_HRV_V0_ID);
...
#[test]
fn built_in_registry_includes_flagship_open_vitals_score_family() {
    ...
    assert_eq!(ids[0], OPENVITALS_HRV_V0_ID);
    assert!(ids.contains(&OPENVITALS_SLEEP_V0_ID));
    assert!(ids.contains(&OPENVITALS_SLEEP_V1_ID));
    assert!(ids.contains(&OPENVITALS_STRAIN_V0_ID));
    assert!(ids.contains(&OPENVITALS_RECOVERY_V0_ID));
    assert!(ids.contains(&OPENVITALS_STRESS_V0_ID));
...
fn built_in_sleep_v0_remains_stable_default_while_sleep_v1_is_experimental() {
    ...
    let sleep_v0 = definitions
        .iter()
        .find(|definition| definition.algorithm_id == OPENVITALS_SLEEP_V0_ID)
        .unwrap();
    let sleep_v1 = definitions
        .iter()
        .find(|definition| definition.algorithm_id == OPENVITALS_SLEEP_V1_ID)
        .unwrap();
    ...
    assert_eq!(sleep_preference.algorithm_id, OPENVITALS_SLEEP_V0_ID);
    assert_eq!(sleep_preference.version, sleep_v0.version);
}

#[test]
fn open_vitals_sleep_v0_computes_hand_derived_component_score() {
    let result = open_vitals_sleep_v0(&SleepInput {
    ...
    let output = result.output.unwrap();
    assert_eq!(output.algorithm_id, OPENVITALS_SLEEP_V0_ID);
    assert_close(output.sleep_debt_minutes, 60.0);
    assert_close(output.efficiency_fraction, 0.875);
    assert_close(output.score_0_to_100, 86.83789515021726);
    assert!(result.errors.is_empty());
}

#[test]
fn open_vitals_sleep_v0_reports_sleep_architecture_latency_and_hr_dip() {
    let result = open_vitals_sleep_v0(&SleepInput {
        start_time: "2026-05-27T22:30:00Z".to_string(),
        end_time: "2026-05-28T06:30:00Z".to_string(),
        sleep_duration_minutes: 420.0,
    ...
    let output = result.output.unwrap();
    assert_close(output.score_0_to_100, 87.33789515021726);
...
#[test]
fn open_vitals_sleep_v0_reports_invalid_inputs_without_output() {
    let result = open_vitals_sleep_v0(&SleepInput {
        start_time: "2026-05-27T22:30:00Z".to_string(),
        end_time: "2026-05-28T06:30:00Z".to_string(),
...
fn sleep_model_status_reports_learning_for_first_packet_derived_night() {
    let report = evaluate_sleep_model_status(&SleepModelStatusInput {
        sleep_permission_granted: true,
        trusted_open_vitals_sleep_nights: 1,
        motion_coverage_fraction: Some(0.92),
        heart_rate_coverage_fraction: Some(0.80),
        ..Default::default()
    });
    ...
    assert_eq!(report.nights_until_open_vitals_training, 6);
    assert!(report.can_show_provisional_score);
...
fn sleep_model_status_saturates_malformed_history_counts() {
    let report = evaluate_sleep_model_status(&SleepModelStatusInput {
        sleep_permission_granted: true,
        trusted_open_vitals_sleep_nights: u32::MAX,
    ...
    assert_eq!(report.nights_until_open_vitals_training, 0);
...
#[test]
fn sleep_model_status_requires_open_vitals_night_for_final_score_visibility() {
    let report = evaluate_sleep_model_status(&SleepModelStatusInput {
        sleep_permission_granted: true,
        imported_platform_sleep_nights: 12,
        trusted_open_vitals_sleep_nights: 0,
        motion_coverage_fraction: Some(0.90),
        heart_rate_coverage_fraction: Some(0.75),
        ..Default::default()
    });
    ...
    assert!(
        report.next_actions.contains(
            &"Complete one OpenVitals packet-derived sleep night before showing a final Sleep V1 score."
                .to_string()
        )
    );
}
...
#[test]
fn sleep_model_status_requires_open_vitals_nights_for_trained() {
    let report = evaluate_sleep_model_status(&SleepModelStatusInput {
        sleep_permission_granted: true,
        imported_platform_sleep_nights: 30,
        trusted_open_vitals_sleep_nights: 7,
...
fn sleep_model_status_names_packet_nights_before_training() {
    let report = evaluate_sleep_model_status(&SleepModelStatusInput {
        sleep_permission_granted: true,
        imported_platform_sleep_nights: 12,
        trusted_open_vitals_sleep_nights: 3,
        motion_coverage_fraction: Some(0.88),
        heart_rate_coverage_fraction: Some(0.70),
        calibration_label_count: 20,
        holdout_validation_passed: true,
    ...
    assert_eq!(report.status, SleepModelStatus::BaselineReady);
    assert_eq!(report.report_state, "final");
    assert_eq!(report.nights_until_open_vitals_training, 4);
    assert_eq!(report.nights_until_training, 0);
    assert!(!report.can_show_trained_score);
    assert!(report.next_actions.contains(
        &"Collect 4 more OpenVitals packet-derived sleep nights before training.".to_string()
    ));
}

#[test]
fn sleep_model_status_does_not_train_on_imported_history_without_open_vitals_nights() {
    let report = evaluate_sleep_model_status(&SleepModelStatusInput {
        sleep_permission_granted: true,
        imported_platform_sleep_nights: 30,
        trusted_open_vitals_sleep_nights: 0,
    ...
    assert_eq!(report.status, SleepModelStatus::BaselineReady);
    assert_eq!(report.report_state, "provisional");
    assert_eq!(report.nights_until_open_vitals_training, 7);
    assert_eq!(report.nights_until_training, 0);
    assert!(!report.can_show_final_score);
    assert!(!report.can_show_trained_score);
    assert!(
...
fn sleep_model_status_blocks_when_timestamps_are_untrusted() {
    let report = evaluate_sleep_model_status(&SleepModelStatusInput {
        sleep_permission_granted: true,
        trusted_open_vitals_sleep_nights: 8,
...
#[test]
fn open_vitals_sleep_v1_computes_hand_derived_component_score() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
            sleep_duration_minutes: 420.0,
        ...
        model_status: SleepModelStatusInput {
            sleep_permission_granted: true,
            imported_platform_sleep_nights: 10,
            trusted_open_vitals_sleep_nights: 2,
    ...
    assert!(result.errors.is_empty(), "{:?}", result.errors);
    assert_eq!(result.algorithm_id, OPENVITALS_SLEEP_V1_ID);
    let output = result.output.unwrap();
    ...
    assert_close(output.score_0_to_100, 84.6449667343507);
...
#[test]
fn open_vitals_sleep_v1_caps_confidence_when_heart_rate_coverage_is_low() {
    ...
    let high_coverage = open_vitals_sleep_v1(&input).output.unwrap();
    input.model_status.heart_rate_coverage_fraction = Some(0.20);
    let low_coverage_result = open_vitals_sleep_v1(&input);
...
#[test]
fn open_vitals_sleep_v1_derives_stage_minutes_from_confident_segments() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_stage_confidence_is_duration_weighted() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_architecture_confidence_uses_stage_probability_uncertainty() {
    let mut input = SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
    ...
    let uncertain_output = open_vitals_sleep_v1(&input).output.unwrap();
    assert_close(
    ...
    let confident_output = open_vitals_sleep_v1(&input).output.unwrap();
    assert!(
        confident_output
            .sleep_architecture_confidence_0_to_1
            .unwrap()
...
#[test]
fn open_vitals_sleep_v1_rejects_invalid_stage_segment_confidence_and_probabilities() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_rejects_unrecognized_current_stage_minutes() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_rejects_impossible_stage_segment_timeline() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_rejects_impossible_sleep_window_math() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_rejects_invalid_sleep_window_timestamps() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "not-a-time".to_string(),
            end_time: "2026-05-28T02:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_rejects_nonexistent_calendar_dates() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
...
#[test]
fn open_vitals_sleep_v1_rejects_impossible_prior_night_duration_math() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_rejects_impossible_prior_night_stage_math() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_returns_baseline_from_prior_nights() {
...
#[test]
fn open_vitals_sleep_v1_ignores_prior_nights_after_current_sleep_start() {
...
#[test]
fn open_vitals_sleep_v1_uses_personal_baseline_for_architecture_and_hr_components() {
...
#[test]
fn open_vitals_sleep_v1_blends_stage_priors_by_baseline_maturity_and_confidence() {
    let mut prior_nights = (0..7)
...
#[test]
fn open_vitals_sleep_v1_scores_overnight_hr_trend_against_personal_baseline() {
    let mut prior_nights = (0..14)
        .map(|index| sleep_history_night(index, 420.0, 0.90))
        .collect::<Vec<_>>();
    for night in &mut prior_nights {
    ...
    let rising = open_vitals_sleep_v1(&input).output.unwrap();
    ...
    let falling = open_vitals_sleep_v1(&input).output.unwrap();
...
#[test]
fn open_vitals_sleep_v1_uses_overnight_hr_trend_before_personal_baseline() {
    let mut input = SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_uses_pre_sleep_awake_hr_in_cardiovascular_recovery() {
    let mut input = SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
    ...
    let elevated = open_vitals_sleep_v1(&input).output.unwrap();
    ...
    let recovered = open_vitals_sleep_v1(&input).output.unwrap();
...
#[test]
fn open_vitals_sleep_v1_guardrails_very_short_and_fragmented_sleep() {
    let result = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
...
#[test]
fn open_vitals_sleep_v1_edge_cases_all_awake_no_hr_missing_stages_and_timestamp_blocked() {
    let all_awake = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
    ...
    let missing_stages = open_vitals_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
            start_time: "2026-05-27T22:30:00Z".to_string(),
            end_time: "2026-05-28T06:30:00Z".to_string(),
            sleep_duration_minutes: 390.0,
...
#[test]
fn open_vitals_sleep_v1_input_and_output_round_trip_json() {
    ...
    let output = open_vitals_sleep_v1(&input).output.unwrap();
...
#[test]
fn open_vitals_strain_v0_computes_hand_derived_zone_and_hr_reserve_score() {
    let result = open_vitals_strain_v0(&StrainInput {
    ...
    assert_close(output.score_0_to_21, 8.682697400506427);
}

#[test]
fn open_vitals_strain_v0_is_monotonic_when_minutes_move_to_higher_zone() {
    let easy = open_vitals_strain_v0(&StrainInput {
        start_time: "2026-05-28T12:00:00Z".to_string(),
        end_time: "2026-05-28T13:00:00Z".to_string(),
        duration_minutes: 60.0,
        resting_hr_bpm: 60.0,
    ...
    .output
    .unwrap();
    let hard = open_vitals_strain_v0(&StrainInput {
        start_time: "2026-05-28T12:00:00Z".to_string(),
        end_time: "2026-05-28T13:00:00Z".to_string(),
        duration_minutes: 60.0,
        resting_hr_bpm: 60.0,
...
#[test]
fn open_vitals_recovery_v0_computes_hand_derived_interpretable_composite() {
    let result = open_vitals_recovery_v0(&RecoveryInput {
        ...
        resting_hr_baseline_bpm: Some(60.0),
        ...
        prior_strain_0_to_21: Some(10.5),
    ...
    assert_close(output.score_0_to_100, 78.8);
    assert_eq!(output.score_status, "complete");
    assert_close(output.component_coverage, 1.0);
    assert_close(output.recovery_confidence, 0.90);
    assert_eq!(output.components.len(), 5);
}

#[test]
fn open_vitals_recovery_v0_flags_low_sleep_and_high_prior_strain() {
    let result = open_vitals_recovery_v0(&RecoveryInput {
        start_time: "2026-05-28T00:00:00Z".to_string(),
        end_time: "2026-05-28T08:00:00Z".to_string(),
        hrv_rmssd_ms: Some(45.0),
        hrv_baseline_rmssd_ms: Some(50.0),
        ...
        sleep_score_0_to_100: Some(55.0),
        prior_strain_0_to_21: Some(15.0),
        input_ids: Vec::new(),
        ..Default::default()
    });
...
#[test]
fn open_vitals_stress_v0_computes_hand_derived_hr_and_hrv_score() {
    let result = open_vitals_stress_v0(&StressInput {
        start_time: "2026-05-28T12:00:00Z".to_string(),
    ...
    let output = result.output.unwrap();
    assert_eq!(output.algorithm_id, OPENVITALS_STRESS_V0_ID);
...
#[test]
fn open_vitals_stress_v0_lowers_hr_contribution_when_motion_explains_elevation() {
    let still = open_vitals_stress_v0(&StressInput {
        start_time: "2026-05-28T12:00:00Z".to_string(),
        end_time: "2026-05-28T12:05:00Z".to_string(),
        heart_rate_bpm: 90.0,
        resting_hr_bpm: 60.0,
    ...
    .output
    .unwrap();
    let moving = open_vitals_stress_v0(&StressInput {
        start_time: "2026-05-28T12:00:00Z".to_string(),
        end_time: "2026-05-28T12:05:00Z".to_string(),
        heart_rate_bpm: 90.0,
        resting_hr_bpm: 60.0,
...
fn score_family_run_record_persists_to_sqlite() {
    ...
    assert_eq!(saved_run.algorithm_id, OPENVITALS_SLEEP_V0_ID);
...
fn flagship_score_fixtures_match_hand_derived_expected_outputs() {
    let sleep: SleepInput = serde_json::from_str(include_str!(
        "../fixtures/synthetic/sleep_open_vitals_v0_hand_derived.json"
    ))
    .unwrap();
    assert_close(
        open_vitals_sleep_v0(&sleep).output.unwrap().score_0_to_100,
        86.83789515021726,
    );
    ...
    let stress: StressInput = serde_json::from_str(include_str!(
        "../fixtures/synthetic/stress_open_vitals_v0_hand_derived.json"
    ))
    .unwrap();
    assert_close(
