...
`open-vitals-reference-algo-runner`. They must print `open_vitals.external-reference-output.v1`
...
python3 tools/reference/neurokit_hrv.py \
  --input fixtures/synthetic/hrv_open_vitals_v0_hand_derived.json \
  --family hrv \
  --provider external.neurokit2.hrv \
  --output-format open_vitals.external-reference-output.v1
```
...
CI can test the OpenVitals contract without external Python science packages.
