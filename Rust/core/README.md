# OpenVitals Rust Core

This is the first runnable OpenVitals core scaffold. It starts with the evidence and
...
- `metrics`: built-in algorithm registry plus `open_vitals.hrv.v0` time-domain HRV
  metrics and initial `open_vitals.sleep.v0`, `open_vitals.strain.v0`,
  `open_vitals.recovery.v0`, and `open_vitals.stress.v0` local scores with component
...
- `algorithm_compare`: OpenVitals-vs-reference comparison reports with explicit
...
- `open_vitals_protocol`: frame, command, event, and data-packet parsing.
- `open_vitals_store`: SQLite schema, inserts, queries, migrations.
- `open_vitals_metrics`: local metric calculations and provenance records.
- `open_vitals_bridge`: FFI-safe request/response structs for Flutter.
- `open_vitals_debug`: event envelope and debug command model shared with the app.
...
```sh
cargo test
cargo run --bin open-vitals-fixture-index -- --fixtures fixtures --output fixtures/index.json
cargo run --bin open-vitals-capture-sanitize -- --input ../../captures/sessions/example --output /tmp/open-vitals-sanitized-capture
cargo run --bin open-vitals-parser-fixture-runner -- --fixtures fixtures --index fixtures/index.json
cargo run --bin open-vitals-capture-import -- --fixtures fixtures --index fixtures/index.json --db /tmp/open_vitals.sqlite
cargo run --bin open-vitals-capture-correlation -- --fixtures fixtures --index fixtures/index.json
cargo run --bin open-vitals-command-validator -- --template
cargo run --bin open-vitals-command-validator -- --evidence ../fixtures/command-evidence/whoop-emulator-command-evidence.json --capture-plan --commands toggle_realtime_hr,start_firmware_load_new
cargo run --bin open-vitals-command-validator -- --emulator-log ../../../captures/ble/whoop-emulator.log --emulator-evidence-output ../../../captures/ble/open-vitals-command-evidence.json --visible-user-intent --triggering-ui-action "Official app screen/button used during capture"
cargo run --bin open-vitals-command-validator -- --emulator-log ../../../captures/ble/emulator-command-capture.jsonl --emulator-mirror-local-frame --visible-user-intent --commands get_hello --capture-plan
cargo run --bin open-vitals-reference-algo-runner -- --family hrv --input fixtures/synthetic/hrv_open_vitals_v0_hand_derived.json --db /tmp/open_vitals.sqlite --run-id reference-hrv-demo-run
cargo run --bin open-vitals-reference-algo-runner -- --family sleep --input fixtures/synthetic/sleep_open_vitals_v0_hand_derived.json --db /tmp/open_vitals.sqlite --run-id reference-sleep-demo-run
cargo run --bin open-vitals-reference-algo-runner -- --family strain --input fixtures/synthetic/strain_open_vitals_v0_hand_derived.json --db /tmp/open_vitals.sqlite --run-id reference-strain-demo-run
cargo run --bin open-vitals-reference-algo-runner -- --family stress --input fixtures/synthetic/stress_open_vitals_v0_hand_derived.json --db /tmp/open_vitals.sqlite --run-id reference-stress-demo-run
cargo run --bin open-vitals-algo-benchmark -- --input fixtures/synthetic/hrv_open_vitals_v0_hand_derived.json --db /tmp/open_vitals.sqlite --run-id hrv-demo-run
cargo run --bin open-vitals-algo-benchmark -- --algorithm open_vitals.sleep.v0 --input fixtures/synthetic/sleep_open_vitals_v0_hand_derived.json --label-value 86.0 --label-unit score_0_to_100 --label-source manual --label-provenance-json '{"entry":"typed_by_user","official_labels_are_labels":true}' --max-absolute-error 2.0
cargo run --bin open-vitals-algo-benchmark -- --compare-reference --family hrv --input fixtures/synthetic/hrv_open_vitals_v0_hand_derived.json
cargo run --bin open-vitals-algo-benchmark -- --compare-reference --family sleep --input fixtures/synthetic/sleep_open_vitals_v0_hand_derived.json
cargo run --bin open-vitals-algo-benchmark -- --compare-reference --family sleep --input fixtures/synthetic/sleep_open_vitals_v0_hand_derived.json --reference-report /tmp/open-vitals-external-sleep-reference.json
cargo run --bin open-vitals-algo-benchmark -- --compare-reference --family strain --input fixtures/synthetic/strain_open_vitals_v0_hand_derived.json
cargo run --bin open-vitals-algo-benchmark -- --compare-reference --family stress --input fixtures/synthetic/stress_open_vitals_v0_hand_derived.json
cargo run --bin open-vitals-algo-benchmark -- --algorithm open_vitals.recovery.v0 --input fixtures/synthetic/recovery_open_vitals_v0_hand_derived.json
cargo run --bin open-vitals-calibration-evaluator -- --input fixtures/synthetic/recovery_calibration_linear.json --db /tmp/open_vitals.sqlite --run-id calibration-demo-run --split-at 2026-05-04T00:00:00Z
cargo run --bin open-vitals-health-sync-dry-run -- --input fixtures/synthetic/health_sync_dry_run_healthkit.json
cargo run --bin open-vitals-storage-check -- --db /tmp/open_vitals.sqlite --self-test
cargo run --bin open-vitals-debug-ws-contract -- --input fixtures/synthetic/debug_ws_contract_valid.json
cargo run --bin open-vitals-debug-ws-serve -- --db /tmp/open_vitals.sqlite --session-id debug-session-1 --token session-token --port 49152
cargo run --bin open-vitals-ui-coverage-audit -- --input ../apk-ui-inventory/coverage-map.json
cargo run --bin open-vitals-raw-export -- --db /tmp/open_vitals.sqlite --output-dir /tmp/open_vitals.openvitalsbundle --zip-output /tmp/open_vitals.openvitalsbundle.zip --start 2026-05-01T00:00:00Z --end 2026-05-28T00:00:00Z
cargo run --bin open-vitals-export-validator -- --bundle path/to/unpacked-export
cargo run --bin open-vitals-privacy-lint -- --input path/to/unpacked-or-zipped-export
cargo run --bin open-vitals-property-test-suite -- --cases 128 --seed 7453298449734135857
cargo run --bin open-vitals-perf-budget -- --scale 256
```
...
cargo run --bin open-vitals-calibration-evaluator -- --input fixtures/synthetic/recovery_calibration_linear.json --db /tmp/open_vitals.sqlite --run-id calibration-demo-run --split-at 2026-05-04T00:00:00Z
cargo run --bin open-vitals-raw-export -- --db /tmp/open_vitals.sqlite --output-dir /tmp/open_vitals.openvitalsbundle --zip-output /tmp/open_vitals.openvitalsbundle.zip --start 2026-05-01T00:00:00Z --end 2026-05-28T00:00:00Z
cargo run --bin open-vitals-export-validator -- --bundle /tmp/open_vitals.openvitalsbundle
cargo run --bin open-vitals-export-validator -- --bundle /tmp/open_vitals.openvitalsbundle.zip
cargo run --bin open-vitals-privacy-lint -- --input /tmp/open_vitals.openvitalsbundle
cargo run --bin open-vitals-privacy-lint -- --input /tmp/open_vitals.openvitalsbundle.zip
```

## App Bridge

The Rust core builds as `rlib`, `staticlib`, and `cdylib`. The first C ABI
surface is declared in `include/open_vitals_core_bridge.h`:

- `open_vitals_core_version_json()`
- `open_vitals_bridge_handle_json(request_json)`
- `open_vitals_bridge_free_string(value)`

Bridge requests use `open_vitals.bridge.request.v1` and responses use
`open_vitals.bridge.response.v1`. Errors are returned as JSON responses, not native
panics.

The OpenVitals Swift app builds this core for iOS through
...
crate at `Rust/iphonesimulator/libopen_vitals_core.a` and
`Rust/iphoneos/libopen_vitals_core.a`; they should not be committed.
...
- `metrics.open_vitals_hrv_v0`
- `metrics.open_vitals_sleep_v0`
- `metrics.open_vitals_strain_v0`
- `metrics.open_vitals_recovery_v0`
- `metrics.open_vitals_stress_v0`
...
strings alone as
enough: the official frame must parse as a OpenVitals
...
result code.
`open-vitals-command-validator --emulator-log` can now parse the macOS spoofed
peripheral log directly into `open_vitals.command-evidence.v1` rows and validate them
...
bytes OpenVitals intends to show/send. The same CLI can now take that separate
...
blocked comparisons, so an official-app emulator session plus a OpenVitals dry-run
...
`015-safe-read-baseline-builders/`, `open_vitals.command-local-frame-candidates.v1`
wrappers, and JSONL rows; CLI style command names such as `GET_HELLO` and
numeric `command_id` values are normalized to OpenVitals command ids before
comparison.
...
`commands.promote_local_frame_matches` takes official evidence plus OpenVitals
...
`open-vitals-fixture-index` and `open-vitals-parser-fixture-runner` reports include
...
`open_vitals.captured-frame-batch.v1` fixtures model sanitized CoreBluetooth-style
...
`open-vitals-capture-import` flattens each frame into raw evidence and decoded-frame
rows through the same path used by app-captured frame batches.

`open-vitals-capture-sanitize` currently preserves UTF-8 text, JSON, and JSONL
...
`open-vitals-raw-export` writes an unpacked `.openvitalsbundle` directory and, when
`--zip-output` is provided, a `.openvitalsbundle.zip` archive with the same manifest
...
- `data/open_vitals.sqlite` when the `sqlite` family is selected and exporting from a
...
generated bundle paths. `open-vitals-export-validator` validates both unpacked
...
`open-vitals-capture-correlation` emits `open_vitals.capture-correlation-report.v1` for
...
`metrics.input_readiness` emits `open_vitals.metric-input-readiness-report.v1` from
...
`capture.arrival_plan` emits `open_vitals.capture-arrival-plan-report.v1` from the app
...
`metrics.motion_features` emits `open_vitals.motion-feature-report.v1` from decoded
...
`metrics.heart_rate_features` emits `open_vitals.heart-rate-feature-report.v1` from
...
`metrics.vital_event_features` emits `open_vitals.vital-event-feature-report.v1` from
...
`metrics.hrv_features` emits `open_vitals.hrv-feature-report.v1` from decoded R17
...
`rr_intervals_ms` candidates, rejects implausible samples, runs `open_vitals.hrv.v0`
...
`metrics.window_features` emits `open_vitals.metric-window-feature-report.v1` by
...
`open_vitals.resting-heart-rate-feature-report.v1` by reusing trusted heart-rate
...
`open_vitals.strain-feature-score-report.v1` by composing trusted resting-HR and
window feature reports into a `open_vitals.strain.v0` input. When no user/profile max
...
`open_vitals.sleep-feature-score-report.v1` by deriving a preliminary sleep window
from trusted low-motion raw-motion features, then feeding duration, time in bed,
midpoint deviation, and disturbance count into `open_vitals.sleep.v0`.

`metrics.recovery_score_from_features` emits
`open_vitals.recovery-feature-score-report.v1` by composing trusted HRV, HRV baseline,
resting-HR, resting-HR baseline, sleep-score, and prior-strain reports into
`open_vitals.recovery.v0`. Respiratory rate and skin-temperature inputs must be
explicitly provided until packet decoders exist, and the report flags that basis.

`metrics.stress_score_from_features` emits
`open_vitals.stress-feature-score-report.v1` by composing trusted current HR, motion,
resting-HR, current HRV, and HRV-baseline reports into a local
`open_vitals.stress.v0` input/output. It blocks the local score when the current HRV
...
`open-vitals-privacy-lint` should pass on both unpacked and zipped exports before a
...
OpenVitals-generated outputs.

## Property Stress Suite

`open-vitals-property-test-suite` emits `open_vitals.property-test-report.v1` JSON and
...
- bounds and quality invariants for `open_vitals.hrv.v0`, `open_vitals.sleep.v0`,
  `open_vitals.strain.v0`, `open_vitals.recovery.v0`, and `open_vitals.stress.v0`
...
`open-vitals-perf-budget` emits `open_vitals.perf-budget-report.v1` JSON and fails when a
...
- OpenVitals v0 score calculation for HRV, sleep, strain, recovery, and stress
- raw `.openvitalsbundle` plus `.openvitalsbundle.zip` export from a synthetic SQLite
  store
...
`open_vitals.hrv.v0` computes time-domain HRV from RR intervals in milliseconds:
...
- `open_vitals.sleep.v0`: weighted duration, efficiency, consistency, and
  disturbance components on a 0-100 scale.
- `open_vitals.strain.v0`: HR-zone load plus average HR reserve on a 0-21 scale.
- `open_vitals.recovery.v0`: interpretable HRV, RHR, respiratory, temperature,
  sleep, and prior-strain readiness composite on a 0-100 scale.
- `open_vitals.stress.v0`: HR elevation and HRV suppression with motion-context
  discounting on a 0-100 scale.

Benchmark-only references are registered separately from OpenVitals defaults through
`metrics.reference_definitions` and `open-vitals-reference-algo-runner`:
...
These are versioned OpenVitals-owned reference reports, not official WHOOP labels.
They are designed to benchmark local formulas while preserving component
breakdowns.

OpenVitals-vs-reference comparisons use `open_vitals.algorithm-comparison-report.v1`.
...
Defaults select the built-in OpenVitals v0 algorithm for `hrv`, `sleep`, `strain`,
...
`open-vitals-reference-algo-runner` supports four benchmark-only internal
providers:
...
```bash
cargo run --bin open-vitals-reference-algo-runner -- --family hrv --provider external.neurokit2.hrv --input fixtures/synthetic/hrv_open_vitals_v0_hand_derived.json --external-command python3 --external-arg tools/reference/neurokit_hrv.py --db /tmp/open_vitals.sqlite --run-id reference-neurokit-hrv-demo
```

The command receives `--input`, `--family`, `--provider`, and `--output-format`
arguments plus matching `OPENVITALS_REFERENCE_*` environment variables. It must emit
`open_vitals.external-reference-output.v1` JSON on stdout with algorithm id/version,
provider version, source, license, parameters, output units, quality gates,
provenance, and errors. OpenVitals validates that contract, records input/stdout
...
`open-vitals-algo-benchmark` can run a built-in OpenVitals algorithm from JSON input, or
run `--compare-reference` for HRV, sleep, strain, and stress. Comparison mode
emits the
same `open_vitals.algorithm-comparison-report.v1` used by the bridge, including
OpenVitals output, reference output, shared-field deltas, quality flags, runtime,
...
`open_vitals_output_ready`, `reference_output_ready`, and `shared_fields_ready` so a
...
`open_vitals.reference-algo-report.v1` or `open_vitals.external-reference-output.v1` report,
so pyActigraphy/GGIR adapter output can be compared against OpenVitals sleep inputs
without copying external formulas into the mobile runtime.

Built-in algorithm mode emits `open_vitals.algo-benchmark-report.v1` with runtime,
...
`open-vitals-calibration-evaluator` reads `open_vitals.calibration-dataset.v1` records and
...
`calibration.apply` applies a stored, passing calibration run to a local OpenVitals
...
result as `open_vitals_calibrated_local_score`. Application reports expose
...
provenance; they are not re-emitted as OpenVitals outputs.

## Health Sync Dry Run

`open-vitals-health-sync-dry-run` validates platform mappings before any HealthKit or
...
- OpenVitals marker for later identification
- stale OpenVitals-owned delete planning inside the backfill window
...
`open-vitals-storage-check` validates the local SQLite store before app or Debug-tab
features trust it.
...
- every known OpenVitals table exists
...
`open-vitals-debug-ws-contract` validates the stream contract that the Debug tab
...
`open-vitals-debug-ws-serve` is the first local stream transport. It binds to
loopback, requires `/open-vitals-debug/stream?token=...`, streams persisted
`open_vitals.debug.event.v1` envelopes from SQLite in sequence order, and exits after
...
- bridge URL uses a WebSocket scheme and `/open-vitals-debug/stream`
...
- command envelopes use `open_vitals.debug.command.v1`
- event envelopes use `open_vitals.debug.event.v1`
...
`open-vitals-ui-coverage-audit` validates the generated APK UI inventory against
`projects/openVitals/apk-ui-inventory/coverage-map.json`.

Current checks:

- navigation destinations resolve to an explicit OpenVitals status
