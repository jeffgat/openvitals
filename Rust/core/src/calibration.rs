...
pub fn evaluate_linear_calibration(
    ...
    if dataset.schema != "open_vitals.calibration-dataset.v1" {
    ...
    CalibrationReport {
        schema: "open_vitals.calibration-report.v1".to_string(),
        generated_by: "open-vitals-calibration-evaluator".to_string(),
...
pub fn apply_calibration(input: &CalibrationApplicationInput) -> CalibrationApplicationReport {
    ...
    CalibrationApplicationReport {
        schema: "open_vitals.calibrated-score.v1".to_string(),
        generated_by: "open-vitals-calibration-apply".to_string(),
        ...
        applied_model,
        output_kind: "open_vitals_calibrated_local_score".to_string(),
...
fn calibration_issue_action(issue: &str) -> (&'static str, &'static str) {
    if issue.starts_with("unsupported dataset schema ") {
        (
            ...
            "Convert the labels to open_vitals.calibration-dataset.v1 before evaluating calibration.",
        )
        ...
        || issue == "holdout rows must be at or after split_at"
    {
        ...
        (
            ...
            "Add calibration rows with varied OpenVitals predictions before fitting a linear calibration model.",
        )
...
fn calibration_application_issue_action(issue: &str) -> (&'static str, &'static str) {
    if issue == "raw_score must be finite" {
        (
            "raw_score_invalid",
            "Recompute the local OpenVitals score and apply calibration only to a finite raw score.",
        )
    } else if issue == "score_min must be finite and less than score_max" {
        (
            "score_range_invalid",
...
pub fn calibration_run_record(
...
) -> OpenVitalsResult<CalibrationRunRecord> {
    ...
    .map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize calibration metrics: {error}"))
    })?;
    ...
    .map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize calibration params: {error}"))
    })?;
...
fn fit_linear_model(records: &[CalibrationRecord]) -> OpenVitalsResult<LinearCalibrationModel> {
