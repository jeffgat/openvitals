...
pub const PRIVACY_LINT_REPORT_SCHEMA: &str = "open_vitals.privacy-lint-report.v1";
...
pub fn lint_privacy_path(path: &Path) -> OpenVitalsResult<PrivacyLintReport> {
    if !path.exists() {
    ...
    Ok(PrivacyLintReport {
        ...
        generated_by: "open-vitals-privacy-lint".to_string(),
...
fn privacy_lint_action_for_rule(rule: &str) -> &'static str {
    match rule {
        ...
        "private_whoop_api_material" => {
            "Remove private WHOOP API replay material from OpenVitals artifacts; keep official-app traces only as redacted capture fixtures with provenance."
        }
        "direct_identifier" => {
            "Replace direct user/device identifiers with OpenVitals-owned pseudonyms before export and keep any mapping outside shareable artifacts."
        }
        "email" => {
    ...
    if is_zip_path(path) {
        ...
        for index in 0..archive.len() {
            ...
            entry.read_to_end(&mut bytes).map_err(|source| {
                OpenVitalsError::message(format!("cannot read zip bytes: {source}"))
            })?;
