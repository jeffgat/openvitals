...
impl DeviceType {
    pub fn header_len(self) -> usize {
        match self {
            DeviceType::Gen4 => 4,
            DeviceType::Maverick | DeviceType::Puffin | DeviceType::OpenVitals => 8,
        }
    }
    ...
    pub fn expected_frame_len(self, buffer: &[u8]) -> Option<usize> {
        match self {
            ...
            DeviceType::Maverick | DeviceType::Puffin | DeviceType::OpenVitals => {
                if buffer.len() < 8 {
                    None
                } else {
...
pub fn parse_frame_hex(device_type: DeviceType, hex_value: &str) -> OpenVitalsResult<ParsedFrame> {
    let raw = decode_hex_with_whitespace(hex_value)?;
    parse_frame(device_type, &raw)
}

pub fn parse_frame(device_type: DeviceType, frame: &[u8]) -> OpenVitalsResult<ParsedFrame> {
    if frame.first().copied() != Some(FRAME_START) {
        return Err(OpenVitalsError::message("frame does not start with 0xaa"));
    }
...
pub fn decode_hex_with_whitespace(hex_value: &str) -> OpenVitalsResult<Vec<u8>> {
