...
pub const OPENVITALS_RECOVERY_SENSOR_DEVICE_SENSOR_V0_VERSION: &str = "0.1.0";
...
pub fn rollup_resting_heart_rate_day_for_store(
...
) -> OpenVitalsResult<RestingHeartRateDailyRollupReport> {
    validate_options(&options)?;
    let start_time_unix_ms = parse_rfc3339_utc_unix_ms(options.start)
        .ok_or_else(|| OpenVitalsError::message("start must be an RFC3339 UTC timestamp"))?;
    let end_time_unix_ms = parse_rfc3339_utc_unix_ms(options.end)
        .ok_or_else(|| OpenVitalsError::message("end must be an RFC3339 UTC timestamp"))?;
    if end_time_unix_ms <= start_time_unix_ms {
        return Err(OpenVitalsError::message("end must be after start"));
    }
    ...
    if pass && options.write_metric {
        ...
        let quality_flags_json = serde_json::to_string(&quality_flags).map_err(|error| {
            OpenVitalsError::message(format!("cannot serialize RHR quality flags: {error}"))
        })?;
        let provenance_json = json!({
            "algorithm": OPENVITALS_RESTING_HEART_RATE_DEVICE_SENSOR_V0_ID,
            "algorithm_version": OPENVITALS_RESTING_HEART_RATE_DEVICE_SENSOR_V0_VERSION,
    ...
    Ok(RestingHeartRateDailyRollupReport {
        ...
        generated_by: "open-vitals-resting-heart-rate-daily-rollup".to_string(),
        pass,
...
pub fn validate_resting_heart_rate_capture_for_store(
...
) -> OpenVitalsResult<RestingHeartRateCaptureValidationReport> {
    ...
    Ok(RestingHeartRateCaptureValidationReport {
        ...
        generated_by: "open-vitals-resting-heart-rate-capture-validator".to_string(),
        ...
        algorithm_id: OPENVITALS_RESTING_HEART_RATE_DEVICE_SENSOR_V0_ID.to_string(),
        algorithm_version: OPENVITALS_RESTING_HEART_RATE_DEVICE_SENSOR_V0_VERSION.to_string(),
        resting_hr_rollup,
...
pub fn rollup_recovery_unavailable_daily_status_for_store(
...
) -> OpenVitalsResult<RecoveryUnavailableDailyStatusReport> {
    ...
    Ok(RecoveryUnavailableDailyStatusReport {
        ...
        generated_by: "open-vitals-recovery-unavailable-daily-status".to_string(),
        pass: issues.is_empty(),
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start: options.start.to_string(),
...
pub fn rollup_recovery_sensor_daily_for_store(
...
) -> OpenVitalsResult<RecoverySensorDailyRollupReport> {
    validate_recovery_sensor_daily_options(&options)?;
    let start_time_unix_ms = parse_rfc3339_utc_unix_ms(options.start)
        .ok_or_else(|| OpenVitalsError::message("start must be an RFC3339 UTC timestamp"))?;
    let end_time_unix_ms = parse_rfc3339_utc_unix_ms(options.end)
        .ok_or_else(|| OpenVitalsError::message("end must be after start"))?;
    if end_time_unix_ms <= start_time_unix_ms {
        return Err(OpenVitalsError::message("end must be after start"));
    }
    ...
    Ok(RecoverySensorDailyRollupReport {
        ...
        generated_by: "open-vitals-recovery-sensor-daily-rollup".to_string(),
        pass: issues.is_empty(),
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start: options.start.to_string(),
...
fn recovery_unavailable_metric_status_for_widget(
...
) -> OpenVitalsResult<RecoveryUnavailableMetricStatus> {
    ...
    if options.write_metric {
        ...
        let provenance_json = json!({
            "algorithm": OPENVITALS_RECOVERY_UNAVAILABLE_STATUS_V0_ID,
            "algorithm_version": OPENVITALS_RECOVERY_UNAVAILABLE_STATUS_V0_VERSION,
...
fn recovery_sensor_daily_metric_status_for_widget(
    ...
    start_time_unix_ms: i64,
    end_time_unix_ms: i64,
) -> OpenVitalsResult<RecoverySensorDailyMetricStatus> {
    let metric_name = recovery_metric_name(&widget.metric_id).to_string();
    ...
    if options.write_metric && promotable_value {
        ...
        let provenance_json = json!({
            "algorithm": OPENVITALS_RECOVERY_SENSOR_DEVICE_SENSOR_V0_ID,
            "algorithm_version": OPENVITALS_RECOVERY_SENSOR_DEVICE_SENSOR_V0_VERSION,
            "source_kind": "device_sensor",
            "metric_id": widget.metric_id,
            "metric_name": metric_name,
            "date_key": options.date_key,
...
) -> OpenVitalsResult<RollingAverage> {
...
fn validate_options(options: &RestingHeartRateDailyRollupOptions<'_>) -> OpenVitalsResult<()> {
    ...
    if options.timezone.trim().is_empty() {
        return Err(OpenVitalsError::message("timezone is required"));
    }
