...
pub fn validate_export_bundle(path: &Path) -> OpenVitalsResult<ExportValidationReport> {
    if path
        .extension()
    ...
    let manifest_raw = fs::read_to_string(&manifest_path)
        .map_err(|source| OpenVitalsError::io(&manifest_path, source))?;
    let manifest: ExportManifest = serde_json::from_str(&manifest_raw)
        .map_err(|source| OpenVitalsError::json(&manifest_path, source))?;
    ...
    let content = validate_export_contents(&manifest, |relative_path| {
        fs::read_to_string(base_dir.join(relative_path))
            .map_err(|source| OpenVitalsError::io(base_dir.join(relative_path), source))
    });
...
pub fn export_raw_timeframe(
...
) -> OpenVitalsResult<RawExportReport> {
    ...
    let debug_window = debug_window
        .transpose()
        .map_err(OpenVitalsError::message)?
        .unwrap_or((0, 0));
    ...
    fs::create_dir_all(options.output_dir)
        .map_err(|source| OpenVitalsError::io(options.output_dir, source))?;
    fs::create_dir_all(options.output_dir.join("data"))
        .map_err(|source| OpenVitalsError::io(options.output_dir.join("data"), source))?;
    ...
    let manifest = ExportManifest {
        schema_version: "open_vitals.export.v1".to_string(),
    ...
    let manifest_json = serde_json::to_vec_pretty(&manifest)
        .map_err(|error| OpenVitalsError::message(format!("cannot serialize manifest: {error}")))?;
    fs::write(options.output_dir.join("manifest.json"), manifest_json)
        .map_err(|source| OpenVitalsError::io(options.output_dir.join("manifest.json"), source))?;
    ...
    Ok(RawExportReport {
        schema: "open_vitals.raw-export-report.v1".to_string(),
        generated_by: "open-vitals-raw-export".to_string(),
    ...
    for file in &manifest.files {
        ...
        match export_manifest_file_family(&file.path) {
            ...
            None => issues.push(format!(
                "{} is not a recognized OpenVitals export artifact path",
                file.path
            )),
        }
    }
...
fn snapshot_sqlite_database(source_path: &Path, target_path: &Path) -> OpenVitalsResult<()> {
    fs::metadata(source_path).map_err(|source| OpenVitalsError::io(source_path, source))?;
    if target_path.exists() {
        fs::remove_file(target_path).map_err(|source| OpenVitalsError::io(target_path, source))?;
    }
...
fn debug_commands_for_raw_byte_policy(
    ...
    filters: &RawExportFilters,
) -> OpenVitalsResult<Vec<ExportDebugCommandRow>> {
    if filters.include_raw_bytes {
        return Ok(rows.to_vec());
    }
    rows.iter()
...
fn debug_events_for_raw_byte_policy(
    ...
    filters: &RawExportFilters,
) -> OpenVitalsResult<Vec<ExportDebugEventRow>> {
    if filters.include_raw_bytes {
        return Ok(rows.to_vec());
    }
    rows.iter()
...
fn raw_byte_policy_json_text(text: &str) -> OpenVitalsResult<String> {
    let value: Value = serde_json::from_str(text).map_err(|error| {
        OpenVitalsError::message(format!("cannot parse JSON for raw-byte policy: {error}"))
    })?;
    serde_json::to_string(&raw_byte_policy_json_value(value)).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize raw-byte policy JSON: {error}"))
    })
}

fn raw_byte_policy_json_text_if_valid(text: &str) -> OpenVitalsResult<String> {
...
fn write_raw_csv(path: &Path, rows: &[RawEvidenceRow]) -> OpenVitalsResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
        "evidence_id,source,captured_at,device_model,payload_hex,sha256,sensitivity,capture_session_id"
    )
    .map_err(|error| OpenVitalsError::message(format!("cannot write CSV header: {error}")))?;
    for row in rows {
        write_csv_row(
            &mut bytes,
            &[
...
fn write_decoded_csv(path: &Path, rows: &[DecodedFrameRow]) -> OpenVitalsResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_packet_timeline_csv(path: &Path, rows: &[PacketTimelineRow]) -> OpenVitalsResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
) -> OpenVitalsResult<Vec<ExportSensorSampleRow>> {
        ...
        && plausible_unix_timestamp_seconds(seconds)
    {
        let millis = match timestamp_subseconds {
            Some(subseconds) if subseconds > 999 => {
                ...
                push_quality_flag_once(quality_flags, "device_timestamp_subseconds_ignored");
                0
            }
...
) -> OpenVitalsResult<Vec<ExportMetricFeatureReportRow>> {
    let mut rows = Vec::new();
...
fn push_metric_feature_report(
    ...
    let report_json = serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize metric feature report: {error}"))
    })?;
...
) -> OpenVitalsResult<(Vec<ExportMetricValueRow>, Vec<ExportMetricComponentRow>)> {
...
fn write_algorithm_runs_csv(path: &Path, rows: &[AlgorithmRunRecord]) -> OpenVitalsResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_activity_labels_csv(path: &Path, rows: &[ActivityLabelRow]) -> OpenVitalsResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_debug_events_csv(path: &Path, rows: &[ExportDebugEventRow]) -> OpenVitalsResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_csv_row(output: &mut Vec<u8>, fields: &[&str]) -> OpenVitalsResult<()> {
...
) -> OpenVitalsResult<Vec<ExportCommandValidationRow>> {
    rows.into_iter()
        .map(|row| {
...
fn validate_zipped_export_bundle(path: &Path) -> OpenVitalsResult<ExportValidationReport> {
    let zip_file = File::open(path).map_err(|source| OpenVitalsError::io(path, source))?;
...
fn validate_export_contents(
    manifest: &ExportManifest,
    mut read_file: impl FnMut(&str) -> OpenVitalsResult<String>,
...
fn read_typed_export_rows<T: DeserializeOwned>(
    read_file: &mut impl FnMut(&str) -> OpenVitalsResult<String>,
    manifest: &ExportManifest,
    family: &str,
    path: &str,
    issues: &mut Vec<String>,
    ...
    for (index, line) in text.lines().enumerate() {
        ...
        match serde_json::from_str(line) {
            ...
            Err(error) => issues.push(format!(
                "{path} line {} cannot be re-imported as OpenVitals data: {error}",
                index + 1
            )),
        }
    }
...
fn validate_debug_command_reimport(
    ...
    for row in rows {
        ...
        if row.schema != "open_vitals.debug.command.v1" {
            issues.push(format!(
                "debug command {} schema must be open_vitals.debug.command.v1",
                row.command_id
            ));
        }
        if let Some(session_ids) = session_ids {
...
fn validate_debug_event_reimport(
    ...
    for row in rows {
        ...
        if row.schema != "open_vitals.debug.event.v1" {
            issues.push(format!(
                "debug event {}:{} schema must be open_vitals.debug.event.v1",
                row.session_id, row.sequence
            ));
        }
        if let Some(session_ids) = session_ids {
...
fn export_manifest_file_family(path: &str) -> Option<&'static str> {
    match path {
        ...
        "data/open_vitals.sqlite" => Some(RAW_EXPORT_SQLITE_FAMILY),
        _ => None,
    }
}
...
fn required_export_artifact_paths_for_family(family: &str) -> &'static [&'static str] {
    match family {
        ...
        RAW_EXPORT_SQLITE_FAMILY => &["data/open_vitals.sqlite"],
        _ => &[],
    }
}
...
fn write_export_zip(
    ...
    writer
        .finish()
        .map_err(|error| OpenVitalsError::message(format!("cannot finish zip bundle: {error}")))?;
    Ok(())
}
...
fn write_zip_file(
    ...
    let bytes = fs::read(source_path).map_err(|source| OpenVitalsError::io(source_path, source))?;
    writer.start_file(archive_path, options).map_err(|error| {
        OpenVitalsError::message(format!("cannot add {archive_path} to zip: {error}"))
    })?;
    writer.write_all(&bytes).map_err(|error| {
        OpenVitalsError::message(format!("cannot write {archive_path} to zip: {error}"))
    })?;
    Ok(())
}
...
fn read_zip_entry(archive: &mut ZipArchive<File>, path: &str) -> OpenVitalsResult<Vec<u8>> {
    validate_safe_relative_path(path)?;
    let mut entry = archive
        .by_name(path)
        .map_err(|error| OpenVitalsError::message(format!("{path}: {error}")))?;
    let mut bytes = Vec::new();
    entry
        .read_to_end(&mut bytes)
        .map_err(|error| OpenVitalsError::message(format!("cannot read {path}: {error}")))?;
    Ok(bytes)
}

fn validate_safe_relative_path(path: &str) -> OpenVitalsResult<()> {
...
fn export_validation_issue_action(default_scope: &str, issue: &str) -> ExportValidationNextAction {
    ...
    } else if issue_lower.contains("cannot be re-imported as openVitals data") {
        (
            "typed_reimport_failed",
            "Regenerate the typed export row from OpenVitals SQLite or update the typed parser/schema before revalidating.",
        )
        ...
        .contains("sqlite data family cannot be exported when include_raw_bytes is false")
    {
        ...
        (
            "sqlite_source_path",
            "Provide the OpenVitals SQLite source path or deselect the sqlite family before exporting.",
        )
        ...
        || issue_lower.contains("duplicate family")
    {
        (
            ...
            "Regenerate the export manifest with only supported OpenVitals data families selected once.",
        )
    } else if issue_lower.contains("unselected data family") {
        (
            "unselected_data_family_artifact",
            "Regenerate the export so files and row counts exist only for selected OpenVitals data families.",
        )
    } else if issue_lower.contains("not a recognized openVitals export artifact path") {
        (
            "unknown_manifest_file",
            "Regenerate the export so the manifest lists only canonical OpenVitals artifact paths.",
        )
        ...
        || issue_lower.contains("official labels")
    {
        (
            ...
            "Mark official WHOOP comparison values as labels, not OpenVitals outputs, and re-export calibration labels.",
        )
        ...
        || issue_lower.contains("duplicate ")
    {
        ...
        (
            "invalid_hex",
            "Regenerate the row with valid hex and checksum fields from OpenVitals storage.",
        )
    } else if issue_lower.contains("is required for re-import") {
        (
            "required_field",
            "Regenerate the export from normalized OpenVitals rows so required fields are populated.",
        )
        ...
        || issue_lower.contains("json must be")
    {
        (
            "json_field",
            "Regenerate the row with valid JSON field content from OpenVitals storage.",
        )
        ...
        || issue_lower.contains("activity label")
    {
        (
            ...
            "Regenerate activity session, metric, interval, and label rows from OpenVitals storage so timestamps, enums, linkage, and provenance fields stay consistent.",
        )
        ...
        || issue_lower.contains("debug event")
    {
        ...
        (
            ...
            "Regenerate command-validation rows from OpenVitals storage after importing trusted official-app command evidence.",
        )
...
fn report(
    ...
    ExportValidationReport {
        schema: "open_vitals.export-validation-report.v1".to_string(),
        generated_by: "open-vitals-export-validator".to_string(),
