...
) -> OpenVitalsResult<StepPacketDiscoveryReport> {
...
pub fn run_step_packet_discovery(
    ...
    Ok(StepPacketDiscoveryReport {
        ...
        generated_by: "open-vitals-step-packet-discovery".to_string(),
...
) -> OpenVitalsResult<StepCaptureValidationReport> {
    let decoded_rows = store.decoded_frames_between(start, end)?;
...
pub fn run_step_capture_validation(
    ...
    Ok(StepCaptureValidationReport {
        ...
        generated_by: "open-vitals-step-capture-validator".to_string(),
...
fn parsed_payload_json(row: &DecodedFrameRow) -> OpenVitalsResult<Value> {
    ...
    for issue in issues {
        let (summary, action): (&str, String) = match issue.as_str() {
            ...
            "no_step_discovery_frames" => (
                ...
                "Import the controlled capture into a OpenVitals SQLite store before validating step deltas.".to_string(),
            ),
            "no_step_or_pedometer_fields_in_decoded_frames" => (
                "Decoded frames did not expose step, cadence, activity, or pedometer fields",
