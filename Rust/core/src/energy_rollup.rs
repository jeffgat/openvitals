...
pub const ENERGY_DAILY_ROLLUP_REPORT_SCHEMA: &str = "open_vitals.energy-daily-rollup-report.v1";
pub const ENERGY_HOURLY_ROLLUP_REPORT_SCHEMA: &str = "open_vitals.energy-hourly-rollup-report.v1";
...
pub fn rollup_energy_day_for_store(
...
) -> OpenVitalsResult<EnergyDailyRollupReport> {
    ...
    if pass && options.write_metric {
        ...
        let quality_flags_json = serde_json::to_string(&quality_flags).map_err(|error| {
            OpenVitalsError::message(format!("cannot serialize energy quality flags: {error}"))
        })?;
        let provenance_json = json!({
            "algorithm": OPENVITALS_ENERGY_LOCAL_ESTIMATE_V0_ID,
            "algorithm_version": OPENVITALS_ENERGY_LOCAL_ESTIMATE_V0_VERSION,
    ...
    Ok(EnergyDailyRollupReport {
        ...
        generated_by: "open-vitals-energy-daily-rollup".to_string(),
        pass,
...
pub fn rollup_energy_unavailable_daily_status_for_store(
...
) -> OpenVitalsResult<EnergyUnavailableDailyStatusReport> {
    ...
    Ok(EnergyUnavailableDailyStatusReport {
        ...
        generated_by: "open-vitals-energy-unavailable-daily-status".to_string(),
        pass: true,
...
fn energy_unavailable_metric_status_for_rollup(
...
) -> OpenVitalsResult<EnergyUnavailableMetricStatus> {
    ...
    if write_metric {
        ...
        let provenance_json = json!({
            "algorithm": OPENVITALS_ENERGY_UNAVAILABLE_STATUS_V0_ID,
            "algorithm_version": OPENVITALS_ENERGY_UNAVAILABLE_STATUS_V0_VERSION,
...
pub fn rollup_energy_hour_for_store(
...
) -> OpenVitalsResult<EnergyHourlyRollupReport> {
    ...
    Ok(EnergyHourlyRollupReport {
        ...
        generated_by: "open-vitals-energy-hourly-rollup".to_string(),
        pass,
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start: options.start.to_string(),
...
pub fn validate_energy_capture_for_store(
...
) -> OpenVitalsResult<EnergyCaptureValidationReport> {
    ...
    Ok(EnergyCaptureValidationReport {
        ...
        generated_by: "open-vitals-energy-capture-validator".to_string(),
        ...
        algorithm_id: OPENVITALS_ENERGY_LOCAL_ESTIMATE_V0_ID.to_string(),
        algorithm_version: OPENVITALS_ENERGY_LOCAL_ESTIMATE_V0_VERSION.to_string(),
        energy_rollup,
...
) -> OpenVitalsResult<StepCadenceSupport> {
...
fn validate_options(options: &EnergyDailyRollupOptions<'_>) -> OpenVitalsResult<()> {
    ...
    if let Some(resting_hr_bpm) = options.resting_hr_bpm {
        if !resting_hr_bpm.is_finite() || resting_hr_bpm <= 0.0 {
            return Err(OpenVitalsError::message("resting_hr_bpm must be positive"));
        }
    }
    if let Some(max_hr_bpm) = options.max_hr_bpm {
        if !max_hr_bpm.is_finite() || max_hr_bpm <= 0.0 {
            return Err(OpenVitalsError::message("max_hr_bpm must be positive"));
        }
    }
...
fn validate_hourly_options(options: &EnergyHourlyRollupOptions<'_>) -> OpenVitalsResult<()> {
    if options.date_key.trim().is_empty() {
        return Err(OpenVitalsError::message("date_key is required"));
    }
    if options.timezone.trim().is_empty() {
