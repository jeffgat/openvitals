...
pub const COMMAND_CAPTURE_PLAN_REPORT_SCHEMA: &str = "open_vitals.command-capture-plan-report.v1";
...
const OPENVITALS_COMMAND_SERVICE_UUID: &str = "fd4b0001-cce1-4033-93ce-002d5875f58a";
const OPENVITALS_COMMAND_TO_STRAP_UUID: &str = "fd4b0002-cce1-4033-93ce-002d5875f58a";
const OPENVITALS_COMMAND_FROM_STRAP_UUID: &str = "fd4b0003-cce1-4033-93ce-002d5875f58a";
const MAX_DIRECT_SEND_OVERRIDE_WINDOW_MS: u64 = 30_000;

pub fn load_command_evidence(path: &Path) -> OpenVitalsResult<Vec<CommandEvidence>> {
...
pub fn load_command_local_frame_candidates(
    path: &Path,
) -> OpenVitalsResult<Vec<CommandLocalFrameCandidate>> {
    if path.is_dir() {
...
fn load_command_local_frame_candidates_dir(
    ...
    let mut files = fs::read_dir(path)
        .map_err(|source| OpenVitalsError::io(path, source))?
...
fn load_command_evidence_jsonl(path: &Path, raw: &str) -> OpenVitalsResult<Vec<CommandEvidence>> {
...
) -> OpenVitalsResult<CommandEmulatorLogEvidenceReport> {
    let raw = fs::read_to_string(path).map_err(|source| OpenVitalsError::io(path, source))?;
...
pub fn command_evidence_from_emulator_log_text(
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        if let Some(write) = emulator_command_write_record(line_no, message, options) {
            let parsed = match parse_frame_hex(DeviceType::OpenVitals, &write.frame_hex) {
                Ok(parsed) => parsed,
                Err(error) => {
                    issues.push(format!(
        ...
        let parsed = match parse_frame_hex(DeviceType::OpenVitals, &response.frame_hex) {
            Ok(parsed) => parsed,
            Err(error) => {
                issues.push(format!(
    ...
    Ok(CommandEmulatorLogEvidenceReport {
        schema: "open_vitals.command-evidence.v1".to_string(),
        generated_by: "open-vitals-command-validator emulator-log".to_string(),
        ...
        notes: vec![
            ...
            "The official app produced these writes; OpenVitals did not send BLE commands during conversion.".to_string(),
...
fn emulator_log_next_actions(
    ...
    if official_capture_ready && !local_frame_match_ready {
        push_emulator_log_next_action(
            ...
            "Import OpenVitals dry-run local frame candidates and run local-frame promotion before validating direct sends.",
        );
    }
    actions
}
...
fn emulator_log_issue_next_action(issue: &str) -> (&'static str, String) {
    if issue.starts_with("emulator_write_parse_failed:") {
        ...
        (
            ...
            "Recapture or repair the strap response frame so OpenVitals can parse it.".to_string(),
        )
...
pub fn command_evidence_with_local_frame_matches(
    ...
    for row in evidence {
        let Some(definition) = COMMAND_DEFINITIONS
            ...
            comparisons.push(CommandLocalFrameComparison {
                ...
                source: None,
                warnings: vec!["command is not in OpenVitals command definitions".to_string()],
            });
            continue;
        };
    ...
    CommandLocalFrameMatchReport {
        schema: "open_vitals.command-local-frame-match-report.v1".to_string(),
        generated_by: "open-vitals-command-local-frame-match".to_string(),
...
fn local_frame_match_next_actions(
    ...
    if evidence.is_empty() {
        push_local_frame_next_action(
            ...
            "Import official-app command evidence from the real strap or macOS BLE emulator before matching OpenVitals dry-run bytes.",
        );
    }
    if candidates.is_empty() {
        push_local_frame_next_action(
            ...
            "Import OpenVitals dry-run local frame candidates from the APK/firmware-derived builder or whoop-rev command builder.",
        );
    }
...
fn local_frame_match_action_for_reason(command: &str, reason: &str) -> String {
    match reason {
        "unknown_command" => {
            "Map this evidence row to a known OpenVitals command definition before comparing bytes."
                .to_string()
        }
        "local_candidate_missing" | "local_frame_missing" => format!(
            "Build or import a OpenVitals dry-run frame candidate for {command} before promotion."
        ),
        ...
        "official_frame_parse_failed" | "official_frame_crc_invalid" => format!(
            "Recapture or repair the official {command} frame so OpenVitals can parse it with valid CRCs."
        ),
        ...
        "local_frame_parse_failed" | "local_frame_crc_invalid" => format!(
            "Regenerate the OpenVitals dry-run frame for {command} so it parses with valid CRCs."
        ),
        ...
        "local_frame_command_number_mismatch" => format!(
            "Fix the OpenVitals dry-run builder so the local frame parses as the same command number as {command}."
        ),
        "frame_bytes_differ" => format!(
            "Replay or adjust the OpenVitals dry-run builder until the local {command} bytes exactly match the official frame."
        ),
...
pub fn validate_commands(evidence: &[CommandEvidence]) -> CommandValidationReport {
    ...
    CommandValidationReport {
        schema: "open_vitals.command-validation-report.v1".to_string(),
        generated_by: "open-vitals-command-validator".to_string(),
...
pub fn direct_send_gate_from_result(
    ...
    match result {
        Some(result) => CommandDirectSendGate {
            schema: "open_vitals.command-direct-send-gate.v1".to_string(),
        ...
        None => CommandDirectSendGate {
            ...
            next_capture_actions: vec![CommandNextCaptureAction {
                ...
                action: "Import or validate official command evidence so OpenVitals can persist a validation record for this command.".to_string(),
            }],
...
pub fn command_capture_plan_from_results(
    ...
    CommandCapturePlanReport {
        ...
        generated_by: "open-vitals-command-capture-plan".to_string(),
        pass,
...
pub fn direct_send_preflight_from_gate(
    ...
    CommandDirectSendPreflight {
        schema: "open_vitals.command-direct-send-preflight.v1".to_string(),
...
fn emulator_command_write_record(
    ...
    Some(EmulatorCommandWriteRecord {
        line_no,
        frame_hex,
        service_uuid: OPENVITALS_COMMAND_SERVICE_UUID.to_string(),
        characteristic_uuid: OPENVITALS_COMMAND_TO_STRAP_UUID.to_string(),
...
fn structured_emulator_command_write_record(
    ...
    .map(normalize_ble_endpoint_value)
    .filter(|value| !value.is_empty())
    .unwrap_or_else(|| OPENVITALS_COMMAND_SERVICE_UUID.to_string());
    ...
    .map(normalize_ble_endpoint_value)
    .filter(|value| !value.is_empty())
    .unwrap_or_else(|| OPENVITALS_COMMAND_TO_STRAP_UUID.to_string());
...
fn structured_emulator_command_response_record(
    ...
    if !structured_emulator_role_matches(
        &value,
        "command_from_strap",
        OPENVITALS_COMMAND_FROM_STRAP_UUID,
    ) {
        return None;
    }
...
fn next_capture_action_for_requirement(
    ...
    match requirement {
        ...
        "local_ble_service_uuid" => {
            "Set the OpenVitals dry-run service UUID to the official captured command service.".to_string()
        }
        ...
        "local_ble_characteristic_uuid" => {
            "Set the OpenVitals dry-run characteristic UUID to the official captured command characteristic.".to_string()
        }
        ...
        "local_ble_write_type" => {
            "Set the OpenVitals dry-run write type to the official captured write type.".to_string()
        }
        "local_ble_write_type_valid" => {
            "Normalize the OpenVitals dry-run write type to with_response or without_response.".to_string()
        }
        "ble_service_uuid_matches_official_capture" => {
            "Update the OpenVitals dry-run endpoint so its service UUID exactly matches the official capture.".to_string()
        }
        "ble_characteristic_uuid_matches_official_capture" => {
            "Update the OpenVitals dry-run endpoint so its characteristic UUID exactly matches the official capture.".to_string()
        }
        "ble_write_type_matches_official_capture" => {
            "Update the OpenVitals dry-run endpoint so its write type exactly matches the official capture.".to_string()
        }
        "local_frame_matches_official_frame" => format!(
            "Use the APK/firmware-derived builder or raw replay args until OpenVitals dry-run bytes exactly match the official {command} frame."
        ),
        "official_frame_parseable" => {
            "Preserve the complete official app write frame and verify OpenVitals can parse it.".to_string()
        }
        ...
        "official_response_frame_parseable" => {
            "Preserve the complete success response frame and verify OpenVitals can parse it.".to_string()
        }
        ...
        "event_logging" => {
            "Persist a OpenVitals event-log entry for dry-run, preflight, and send attempts before enabling this command.".to_string()
        }
        "timeout_behavior" => {
            "Capture or document the official app timeout/retry behavior and encode the OpenVitals timeout path for this command.".to_string()
        }
        "failure_parser" => {
        ...
        "official_failure_response_frame_parseable" => {
            "Preserve the complete failure response frame and verify OpenVitals can parse it.".to_string()
        }
...
fn promote_local_frame_candidate(
    ...
    let official_parsed = match parse_frame_hex(DeviceType::OpenVitals, official) {
        Ok(parsed) => parsed,
        Err(error) => {
    ...
    let local_parsed = match parse_frame_hex(DeviceType::OpenVitals, local) {
        Ok(parsed) => parsed,
        Err(error) => {
    ...
    match_object.insert(
        ...
        serde_json::json!(
            candidate
                ...
                .map(str::trim)
                .filter(|value| !value.is_empty())
                .unwrap_or("open_vitals_local_dry_run")
        ),
    );
    match_object.insert(
...
fn validate_official_command_frame(
    ...
    let parsed = match parse_frame_hex(DeviceType::OpenVitals, official_frame_hex) {
        Ok(parsed) => parsed,
        Err(error) => {
...
fn validate_official_response_frame(
    ...
    let parsed = match parse_frame_hex(DeviceType::OpenVitals, response_frame_hex) {
        Ok(parsed) => parsed,
        Err(error) => {
