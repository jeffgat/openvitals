...
use crate::{OpenVitalsError, OpenVitalsResult};

pub const UI_COVERAGE_AUDIT_SCHEMA: &str = "open_vitals.ui-coverage-audit.v1";
...
pub fn run_ui_coverage_audit(
    ...
    base_dir: &Path,
) -> OpenVitalsResult<UiCoverageAuditReport> {
    ...
    Ok(UiCoverageAuditReport {
        schema: "open_vitals.ui-coverage-audit-report.v1".to_string(),
        generated_by: "open-vitals-ui-coverage-audit".to_string(),
        inventory_valid,
        coverage_map_valid,
...
fn record_deferred_surface(
    ...
    bucket.deferred_surfaces.push(DeferredUiSurface {
        ...
        open_vitals_area: open_vitals_area.to_string(),
...
fn ui_coverage_issue_next_action(issue: &str) -> UiCoverageNextAction {
    ...
    let action = match issue_kind {
        "unsupported_schema" => {
            "Update the coverage-map schema to open_vitals.ui-coverage-audit.v1, then rerun UI coverage audit."
        }
        ...
        "navigation_rule_open_vitals_area_required"
        | "layout_rule_open_vitals_area_required"
        | "ui_resource_rule_open_vitals_area_required"
        | "source_class_rule_open_vitals_area_required" => {
            "Set open_vitals_area to the OpenVitals screen or area responsible for this surface, then rerun UI coverage audit."
        }
        ...
        | "source_class_rule_target_level_required" => {
            "Set target_level for the implemented, approximate, or debug-only OpenVitals coverage target, then rerun UI coverage audit."
        }
        ...
        _ if issue_kind.ends_with("_missing_coverage") => {
            "Add or update a coverage-map rule for this APK surface with a status, OpenVitals area, target level or reason, then rerun UI coverage audit."
        }
        _ => {
...
fn file_sha256(path: &Path) -> OpenVitalsResult<String> {
