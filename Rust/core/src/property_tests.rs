...
pub const PROPERTY_TEST_REPORT_SCHEMA: &str = "open_vitals.property-test-report.v1";
...
pub fn run_property_suite(options: PropertySuiteOptions) -> OpenVitalsResult<PropertySuiteReport> {
...
pub fn property_suite_report_from_groups(
    ...
    PropertySuiteReport {
        ...
        generated_by: "open-vitals-property-test-suite".to_string(),
        seed,
        cases_per_group,
        pass,
        input_valid,
...
fn property_failure_action(group_name: &str, property: &str, reason: &str) -> &'static str {
    match reason {
        ...
        "algorithm_output_failure" => {
            "Fix the generated OpenVitals score input or quality gate so valid generated inputs produce an output, then add a deterministic algorithm regression."
        }
        "algorithm_bounds_failure" => {
            "Clamp or correct the OpenVitals score calculation so finite generated inputs stay within documented bounds and component counts remain consistent."
        }
        "algorithm_metamorphic_failure" => {
            "Review the OpenVitals score formula for this monotonic relationship and add a hand-derived regression before changing expected behavior."
        }
        _ => {
    ...
    for case_index in 0..cases {
        ...
        if corrupted.len() > 12 {
            ...
            match catch_unwind(AssertUnwindSafe(|| {
                parse_frame(DeviceType::OpenVitals, &corrupted)
            })) {
                Ok(Ok(parsed)) => {
                    group.check(
                        case_index,
...
fn algorithm_metamorphic_properties(cases: usize) -> PropertyGroupReport {
    ...
    for case_index in 0..cases {
        let sleep_short = open_vitals_sleep_v0(&SleepInput {
        ...
        let sleep_long = open_vitals_sleep_v0(&SleepInput {
        ...
        let sleep_v1_short = open_vitals_sleep_v1(&SleepV1Input {
            sleep: SleepInput {
                sleep_duration_minutes: 300.0,
        ...
        let sleep_v1_long = open_vitals_sleep_v1(&SleepV1Input {
            sleep: SleepInput {
                sleep_duration_minutes: 360.0,
                wake_after_sleep_onset_minutes: 30.0,
                wake_episode_count: 2,
        ...
        let sleep_v1_low_waso = open_vitals_sleep_v1(&SleepV1Input {
            sleep: SleepInput {
        ...
        .output
        .expect("valid sleep v1 input");
        let sleep_v1_high_waso = open_vitals_sleep_v1(&SleepV1Input {
            sleep: SleepInput {
        ...
        let low_strain = open_vitals_strain_v0(&StrainInput {
            hr_zone_minutes: vec![60.0, 0.0, 0.0, 0.0, 0.0],
            ..base_strain_input()
        })
        .output
        .expect("valid strain input");
        let high_strain = open_vitals_strain_v0(&StrainInput {
        ...
        let low_motion_stress = open_vitals_stress_v0(&StressInput {
            motion_intensity_0_to_1: 0.0,
            ..base_stress_input()
        })
        .output
        .expect("valid stress input");
        let high_motion_stress = open_vitals_stress_v0(&StressInput {
        ...
        let hrv = open_vitals_hrv_v0(&HrvInput {
            start_time: "2026-05-28T00:00:00Z".to_string(),
...
fn check_sleep_v1_bounds(group: &mut GroupBuilder, rng: &mut DeterministicRng, case_index: usize) {
    ...
    let result = open_vitals_sleep_v1(&SleepV1Input {
        ...
        model_status: SleepModelStatusInput {
            ...
            trusted_open_vitals_sleep_nights: rng.usize(12) as u32,
