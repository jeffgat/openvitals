...
pub const CAPTURE_SANITIZE_REPORT_SCHEMA: &str = "open_vitals.capture-sanitize-report.v1";
pub const CAPTURE_SANITIZE_MANIFEST_SCHEMA: &str = "open_vitals.capture-sanitize-manifest.v1";
...
pub fn sanitize_capture_path(
...
) -> OpenVitalsResult<CaptureSanitizeReport> {
    if options.salt.trim().is_empty() {
        return Err(OpenVitalsError::message("sanitize salt must not be empty"));
    }
    ...
    Ok(CaptureSanitizeReport {
        ...
        generated_by: "open-vitals-capture-sanitize".to_string(),
...
fn sanitize_one_file(
    ...
    output_root: &Path,
    salt: &str,
) -> OpenVitalsResult<SanitizedFileReport> {
    let bytes = fs::read(input_path).map_err(|source| OpenVitalsError::io(input_path, source))?;
    ...
    let text = String::from_utf8(bytes.clone()).map_err(|source| {
        OpenVitalsError::message(format!("{} is not UTF-8: {source}", input_path.display()))
    })?;
        ...
        CaptureFileFormat::Json => match serde_json::from_str::<serde_json::Value>(&text) {
            Ok(mut value) => {
                ...
                serde_json::to_string_pretty(&value).map_err(|source| {
                    OpenVitalsError::message(format!("cannot serialize sanitized JSON: {source}"))
                })? + "\n"
            }
            Err(source) => {
            ...
            Ok(mut value) => {
                *redactions += sanitize_json_value(&mut value, salt);
                output.push(serde_json::to_string(&value).map_err(|source| {
                    OpenVitalsError::message(format!("cannot serialize sanitized JSONL: {source}"))
                })?);
            }
            Err(source) => {
                warnings.push(format!(
...
fn collect_files(root: &Path, files: &mut Vec<PathBuf>) -> OpenVitalsResult<()> {
...
fn reject_output_inside_input(input_path: &Path, output_path: &Path) -> OpenVitalsResult<()> {
