...
use crate::{
    ...
    store::{DebugCommandRow, DebugEventRow, DebugSessionRow, OpenVitalsStore},
};

pub const DEBUG_WS_CONTRACT_SCHEMA: &str = "open_vitals.debug-ws-contract.v1";
pub const DEBUG_EVENT_SCHEMA: &str = "open_vitals.debug.event.v1";
pub const DEBUG_COMMAND_SCHEMA: &str = "open_vitals.debug.command.v1";
...
pub fn validate_debug_ws_contract(input: &DebugWsContractInput) -> DebugWsContractReport {
    ...
    DebugWsContractReport {
        schema: "open_vitals.debug-ws-contract-report.v1".to_string(),
        generated_by: "open-vitals-debug-ws-contract".to_string(),
        pass,
        input_valid,
        bridge_valid,
        commands_valid,
...
) -> OpenVitalsResult<DebugSessionSnapshot> {
...
pub fn finish_debug_command(
    ...
    let command = store.debug_command(&input.command_id)?.ok_or_else(|| {
        OpenVitalsError::message(format!("debug command {} not found", input.command_id))
    })?;
...
) -> OpenVitalsResult<DebugEventEnvelope> {
    validate_required("session_id", &input.session_id)?;
    if store.debug_session(&input.session_id)?.is_none() {
        return Err(OpenVitalsError::message(format!(
            "debug session {} not found",
    ...
    if let Some(command_id) = input.command_id.as_deref() {
        if !command_id.trim().is_empty() {
            let command = store.debug_command(command_id)?.ok_or_else(|| {
                OpenVitalsError::message(format!("debug command {command_id} not found"))
            })?;
            if command.session_id != input.session_id {
                return Err(OpenVitalsError::message(format!(
...
pub fn debug_session_snapshot(
    ...
    let session = store
        ...
        .ok_or_else(|| OpenVitalsError::message(format!("debug session {session_id} not found")))?;
    ...
    Ok(DebugSessionSnapshot {
        schema: "open_vitals.debug-session-snapshot.v1".to_string(),
...
fn validate_bridge(bridge: &DebugBridgeConfig, issues: &mut Vec<String>) {
    ...
    if !bridge.url.contains("/open-vitals-debug/stream") {
...
fn debug_ws_next_action(issue: &str) -> DebugWsNextAction {
    ...
    let action = match reason {
        "unsupported_schema" => {
            "Update the debug stream contract input to open_vitals.debug-ws-contract.v1, then rerun the contract validator."
        }
        ...
        "bridge_url_missing_debug_stream_path" => {
            "Bind the debug stream on /open-vitals-debug/stream so agents and tests connect to the expected path."
        }
        ...
        "command_schema_invalid" => {
            "Emit debug commands with schema open_vitals.debug.command.v1, then rerun the contract validator."
        }
        "command_id_required" => {
        ...
        "event_schema_invalid" => {
            "Emit debug events with schema open_vitals.debug.event.v1, then rerun the contract validator."
        }
        ...
        "event_sequence_not_strictly_increasing" => {
            "Append events through OpenVitals storage so sequence numbers strictly increase before streaming."
        }
        ...
        _ => {
            "Resolve the debug stream contract issue, then rerun open-vitals-debug-ws-contract or refresh the Debug snapshot."
        }
    };
    DebugWsNextAction {
        scope,
...
fn object_value(value: serde_json::Value, name: &str) -> OpenVitalsResult<serde_json::Value> {
    if value.is_object() {
        Ok(value)
    } else {
...
fn u64_to_i64(name: &str, value: u64) -> OpenVitalsResult<i64> {
    i64::try_from(value).map_err(|_| OpenVitalsError::message(format!("{name} is too large")))
}

fn i64_to_u64(name: &str, value: i64) -> OpenVitalsResult<u64> {
    u64::try_from(value).map_err(|_| OpenVitalsError::message(format!("{name} is negative")))
}
