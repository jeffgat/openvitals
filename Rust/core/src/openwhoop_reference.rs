...
pub fn whoop_generation_from_device_type(device_type: DeviceType) -> Option<WhoopGeneration> {
    match device_type {
        ...
        DeviceType::Maverick | DeviceType::OpenVitals => Some(WhoopGeneration::Gen5),
        DeviceType::Puffin => None,
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OpenVitalsSummaryStatus {
    Matched,
    Candidate,
    Conflicting,
    NotDecoded,
...
impl Display for OpenVitalsSummaryStatus {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str(self.as_str())
    }
}
...
pub const OPENVITALS_SUMMARIES_NONE: [&str; 0] = [];
pub const OPENVITALS_SUMMARIES_NORMAL_HISTORY: [&str; 1] = ["normal_history"];
pub const OPENVITALS_SUMMARIES_R17: [&str; 1] = ["r17_optical_or_labrador_filtered"];
pub const OPENVITALS_SUMMARIES_RAW_MOTION: [&str; 2] = ["raw_motion_k10", "raw_motion_k21"];
pub const OPENVITALS_SUMMARIES_EVENT_TEMPERATURE: [&str; 1] = ["event_temperature_level"];
...
pub struct HistoryFieldReference {
    ...
    pub gen4: bool,
    pub gen5: bool,
    pub open_vitals_summary_kinds: &'static [&'static str],
    pub status: OpenVitalsSummaryStatus,
    pub note: &'static str,
}
...
pub const OPENWHOOP_HISTORY_FIELD_REFERENCES: [HistoryFieldReference; 11] = [
    HistoryFieldReference {
        ...
        gen4: true,
        gen5: true,
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_NORMAL_HISTORY,
        status: OpenVitalsSummaryStatus::Matched,
        note: "OpenVitals already promotes the normal_history heart-rate marker into heart_rate_bpm; the byte-level source is not the same, but the summary-level concept matches.",
    },
    HistoryFieldReference {
        field: OpenWhoopHistoryField::Rr,
        gen4: true,
        gen5: true,
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_R17,
        status: OpenVitalsSummaryStatus::Candidate,
        note: "OpenVitals currently treats RR intervals as preliminary candidates from r17_optical_or_labrador_filtered rather than a fully decoded strap-history field.",
    },
    HistoryFieldReference {
        field: OpenWhoopHistoryField::Imu,
        gen4: true,
        gen5: true,
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_RAW_MOTION,
        status: OpenVitalsSummaryStatus::Matched,
        note: "OpenVitals already exposes raw motion summaries for K10/K21 motion payloads, which are the closest direct match to OpenWhoop IMU samples.",
    },
    HistoryFieldReference {
        ...
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_R17,
        status: OpenVitalsSummaryStatus::Candidate,
        note: "OpenVitals keeps the optical stream as r17_optical_or_labrador_filtered candidates, but does not yet expose a dedicated PPG history field.",
    },
    HistoryFieldReference {
        field: OpenWhoopHistoryField::RawSpo2RedIr,
        gen4: true,
        gen5: true,
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_NONE,
        status: OpenVitalsSummaryStatus::NotDecoded,
        note: "OpenVitals does not currently expose a raw SpO2 red/IR decoder or summary.",
    },
    HistoryFieldReference {
        field: OpenWhoopHistoryField::RawSkinTemp,
        gen4: true,
        gen5: true,
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_EVENT_TEMPERATURE,
        status: OpenVitalsSummaryStatus::Candidate,
        note: "OpenVitals has an event_temperature_level candidate path, but metric_readiness still blocks skin-temperature extraction until the units and semantics are verified.",
    },
    HistoryFieldReference {
        ...
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_NONE,
        status: OpenVitalsSummaryStatus::NotDecoded,
        note: "OpenVitals's metric readiness explicitly marks respiratory-rate extraction as not implemented.",
    },
    HistoryFieldReference {
        ...
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_NONE,
        status: OpenVitalsSummaryStatus::NotDecoded,
        note: "OpenVitals does not currently surface a signal-quality summary for this field.",
    },
    HistoryFieldReference {
        ...
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_NONE,
        status: OpenVitalsSummaryStatus::NotDecoded,
        note: "OpenVitals does not currently surface a skin-contact summary for this field.",
    },
    HistoryFieldReference {
        ...
        gen5: true,
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_RAW_MOTION,
        status: OpenVitalsSummaryStatus::Conflicting,
        note: "OpenVitals raw motion summaries expose signed axes rather than the derived gravity vector that OpenWhoop stores here, so the semantics do not line up cleanly.",
    },
    HistoryFieldReference {
        ...
        open_vitals_summary_kinds: &OPENVITALS_SUMMARIES_NONE,
        status: OpenVitalsSummaryStatus::NotDecoded,
        note: "OpenWhoop marks this as a Gen5-only field, but OpenVitals does not yet have a summary or decoder for it.",
    },
];
...
pub fn openwhoop_history_field_open_vitals_summary_kinds(
    field: OpenWhoopHistoryField,
) -> Option<&'static [&'static str]> {
    openwhoop_history_field_reference(field).map(|reference| reference.open_vitals_summary_kinds)
}
...
mod tests {
    ...
    fn service_uuid_lookup_is_generation_aware() {
        ...
        assert_eq!(
            whoop_generation_from_device_type(DeviceType::OpenVitals),
            Some(WhoopGeneration::Gen5)
        );
    ...
    fn history_field_table_marks_open_vitals_statuses() {
        ...
        assert!(!spo2.gen4);
        assert!(spo2.gen5);
        assert_eq!(spo2.status, OpenVitalsSummaryStatus::NotDecoded);
    }
