...
fn partial_plan_next_actions(scope: &str, state: PartialPlanState) -> Vec<HealthSyncNextAction> {
    if state.confirmation_required {
        return vec![HealthSyncNextAction {
            ...
            action: "Ask the user to confirm syncing only the planned OpenVitals rows; blocked rows must remain unwritten.".to_string(),
        }];
    }
...
pub fn run_health_sync_dry_run(input: &HealthSyncDryRunInput) -> HealthSyncDryRunReport {
    ...
    if input.schema != "open_vitals.health-sync-dry-run.v1" {
    ...
    for candidate in &input.candidates {
        ...
        if reasons.is_empty() {
            ...
            planned_writes.push(PlannedHealthWrite {
                ...
                idempotency_key,
                open_vitals_marker: open_vitals_marker(candidate),
    ...
    HealthSyncDryRunReport {
        schema: "open_vitals.health-sync-dry-run-report.v1".to_string(),
        generated_by: "open-vitals-health-sync-dry-run".to_string(),
...
pub fn run_activity_health_sync_dry_run(
    ...
    let mut issues = Vec::new();
    if input.schema != "open_vitals.activity-health-sync-dry-run.v1" {
        issues.push(format!("unsupported schema {}", input.schema));
    }
    let backfill_times = validate_backfill_window(&input.backfill, &mut issues);
        ...
        if reasons.is_empty() {
            ...
            if !seen_keys.insert(idempotency_key.clone()) {
                ...
                planned_sessions.push(PlannedActivityHealthWrite {
                    ...
                    idempotency_key,
                    open_vitals_marker: activity_open_vitals_marker(session),
    ...
    next_actions = dedupe_health_next_actions(next_actions);
    ActivityHealthSyncDryRunReport {
        schema: "open_vitals.activity-health-sync-dry-run-report.v1".to_string(),
        generated_by: "open-vitals-activity-health-sync-dry-run".to_string(),
        platform: input.platform,
        pass: input_valid,
        input_valid,
        partial_plan_policy: input.partial_plan_policy,
...
fn activity_open_vitals_marker(session: &ActivitySyncCandidate) -> String {
    format!(
        "openVitals:{}:{}:{}",
    ...
    for existing in &input.existing_records {
        ...
        if reasons.is_empty() {
            planned_deletes.push(PlannedHealthDelete {
                ...
                open_vitals_marker: existing.open_vitals_marker.clone(),
                start_time: existing.start_time.clone(),
                end_time: existing.end_time.clone(),
                reason: "stale_open_vitals_record_in_backfill".to_string(),
            });
        } else {
            reasons.sort();
            reasons.dedup();
...
fn validate_existing_record_for_delete(
    ...
    for (name, value) in [
        ...
        ("open_vitals_marker", existing.open_vitals_marker.as_str()),
    ...
    if !existing.open_vitals_marker.starts_with("openVitals:")
        || !existing.idempotency_key.starts_with("openVitals:")
    {
        reasons.push("not_open_vitals_owned".to_string());
    }
...
fn health_sync_reason_action(
    ...
    match reason {
        ...
        "unsupported_delete_mapping" => {
            "Leave this platform record untouched or add a supported OpenVitals cleanup mapping."
                .to_string()
        }
        "outside_backfill_window" => {
        ...
        "unsafe_source_kind" => {
            "Use a OpenVitals-owned decoded, derived, user-confirmed, or manual source kind."
                .to_string()
        }
        "unsupported_activity_type_mapping" => {
            "Use a supported OpenVitals activity, workout, or sleep type before planning this session."
                .to_string()
        }
        ...
        "benchmark_only_algorithm_not_syncable" => {
            "Use a OpenVitals-owned primary algorithm output instead of a benchmark/reference output."
                .to_string()
        }
        "missing_provenance" => {
            "Attach non-empty OpenVitals provenance linking this value to owned data or an approved algorithm run."
                .to_string()
        }
        "provenance_must_be_object" => {
        ...
        "private_api_provenance_not_syncable" => {
            "Replace private API provenance with user-owned capture/import provenance; OpenVitals must not sync private API replay material."
                .to_string()
        }
        "official_whoop_label_not_syncable" => {
            "Keep official WHOOP labels as labels only; sync OpenVitals outputs or decoded owned values instead."
                .to_string()
        }
        "platform_import_not_syncable" => {
            "Keep HealthKit and Health Connect values out of OpenVitals local metrics; only sync OpenVitals-owned decoded or derived outputs."
                .to_string()
        }
        "healthkit_rmssd_must_not_be_written_as_sdnn" => {
            "Do not write RMSSD to HealthKit SDNN; sync SDNN only if OpenVitals can calculate SDNN, or use Health Connect RMSSD."
                .to_string()
        }
        "health_connect_has_no_sdnn_record" => {
        ...
        "not_open_vitals_owned" => {
            "Do not delete external platform records; cleanup is limited to OpenVitals-owned records."
                .to_string()
        }
...
fn idempotency_key(
    ...
    format!(
        "openVitals:{platform:?}:{destination_type}:{}:{}:{}",
        candidate.record_id, candidate.start_time, candidate.end_time
    )
}

fn open_vitals_marker(candidate: &HealthSyncCandidate) -> String {
    format!(
        "openVitals:{}:{}:{}",
