...
pub fn serve_debug_ws_once(options: DebugWsServerOptions) -> OpenVitalsResult<DebugWsServeReport> {
    let listener = bind_debug_ws_listener(&options)?;
    serve_debug_ws_listener_once(listener, options)
}

pub fn bind_debug_ws_listener(options: &DebugWsServerOptions) -> OpenVitalsResult<TcpListener> {
    validate_server_options(options)?;
    let mut addresses = (options.bind_host.as_str(), options.port)
        .to_socket_addrs()
        .map_err(|error| {
            OpenVitalsError::message(format!("cannot resolve debug bind address: {error}"))
        })?;
...
) -> OpenVitalsResult<DebugWsServeReport> {
    validate_server_options(&options)?;
    ...
    loop {
        let rows = store.debug_events_after_sequence(
            ...
            i64::try_from(last_sequence)
                .map_err(|_| OpenVitalsError::message("last_sequence is too large"))?,
            Some(100),
        )?;
        if rows.is_empty() {
        ...
        for row in rows {
            ...
            let event_json = serde_json::to_string(&event).map_err(|error| {
                OpenVitalsError::message(format!("cannot serialize debug event: {error}"))
            })?;
            websocket
                .send(Message::Text(event_json.into()))
                .map_err(|error| {
                    OpenVitalsError::message(format!("cannot write websocket event: {error}"))
                })?;
            events_sent += 1;
...
fn validate_server_options(options: &DebugWsServerOptions) -> OpenVitalsResult<()> {
    if options.session_id.trim().is_empty() {
        return Err(OpenVitalsError::message("session_id is required"));
    }
    ...
    let issues = debug_bridge_config_issues(&DebugBridgeConfig {
        url: format!(
            "ws://{}:{}/open-vitals-debug/stream?token={}",
...
fn validate_handshake_request(request: &Request, token: &str) -> Result<(), ErrorResponse> {
    if request.uri().path() != "/open-vitals-debug/stream" {
        return Err(error_response(
            StatusCode::NOT_FOUND,
            "expected /open-vitals-debug/stream",
        ));
    }
...
fn report(
    ...
    DebugWsServeReport {
        schema: "open_vitals.debug-ws-serve-report.v1".to_string(),
        generated_by: "open-vitals-debug-ws-serve".to_string(),
        bind_url,
        session_id,
...
fn debug_ws_serve_issue_action(issue: &str) -> String {
    if issue.contains("websocket handshake failed") {
        return "Connect with path /open-vitals-debug/stream and the current per-session token."
            .to_string();
    }
    match issue {
