...
pub const BRIDGE_REQUEST_SCHEMA: &str = "open_vitals.bridge.request.v1";
pub const BRIDGE_RESPONSE_SCHEMA: &str = "open_vitals.bridge.response.v1";
pub const CAPTURE_ARRIVAL_PLAN_REPORT_SCHEMA: &str = "open_vitals.capture-arrival-plan-report.v1";
...
struct ReferenceCompareArgs {
    ...
    #[serde(default)]
    open_vitals_algorithm_id: Option<String>,
}
...
pub fn core_version_payload() -> serde_json::Value {
    json!({
        ...
        "crate_name": option_env!("CARGO_PKG_NAME").unwrap_or("open-vitals-core"),
...
pub fn openwhoop_reference_report_payload() -> serde_json::Value {
    ...
    let history_fields = openwhoop_history_field_references()
        ...
        .map(|reference| {
            json!({
                ...
                "gen4": reference.gen4,
                "gen5": reference.gen5,
                "open_vitals_summary_kinds": reference.open_vitals_summary_kinds,
    ...
    json!({
        "schema": "open_vitals.openwhoop-reference-report.v1",
...
fn handle_bridge_request_inner(request: BridgeRequest) -> BridgeResponse {
    ...
    match request.method.as_str() {
        ...
        "metrics.built_in_definitions" => serde_json::to_value(built_in_algorithm_definitions())
            .map_err(|error| OpenVitalsError::message(error.to_string()))
        ...
        "metrics.open_vitals_hrv_v0" => request_args::<HrvInput>(&request)
            .and_then(|input| metric_result_to_value(open_vitals_hrv_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.open_vitals_sleep_v0" => request_args::<SleepInput>(&request)
            .and_then(|input| metric_result_to_value(open_vitals_sleep_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.open_vitals_sleep_v1" => request_args::<SleepV1Input>(&request)
            .and_then(|input| metric_result_to_value(open_vitals_sleep_v1(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.open_vitals_strain_v0" => request_args::<StrainInput>(&request)
            .and_then(|input| metric_result_to_value(open_vitals_strain_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.open_vitals_recovery_v0" => request_args::<RecoveryInput>(&request)
            .and_then(|input| metric_result_to_value(open_vitals_recovery_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.open_vitals_stress_v0" => request_args::<StressInput>(&request)
            .and_then(|input| metric_result_to_value(open_vitals_stress_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
...
#[unsafe(no_mangle)]
pub extern "C" fn open_vitals_core_version_json() -> *mut c_char {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn open_vitals_bridge_free_string(value: *mut c_char) {
    if value.is_null() {
        return;
    }
...
fn storage_check_bridge(args: StorageCheckArgs) -> OpenVitalsResult<serde_json::Value> {
    if args.database_path.trim().is_empty() {
        return Err(OpenVitalsError::message("database_path is required"));
    }
...
fn apply_default_preferences_bridge(
    ...
    serde_json::to_value(preferences)
        .map_err(|error| OpenVitalsError::message(format!("cannot serialize preferences: {error}")))
}

fn set_algorithm_preference_bridge(args: SetPreferenceArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn get_algorithm_preference_bridge(args: GetPreferenceArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn evaluate_calibration_dataset_bridge(
    ...
    let mut value = serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize calibration report: {error}"))
    })?;
...
fn evaluate_stored_calibration_labels_bridge(
    ...
    if args.start.trim().is_empty() {
        return Err(OpenVitalsError::message("start is required"));
    }
    ...
    if let Some(object) = value.as_object_mut() {
        ...
        object.insert(
            ...
            json!("open_vitals.calibration-dataset.v1"),
        );
        object.insert(
...
fn import_calibration_labels_bridge(
    ...
    for label in args.labels {
        let provenance_json = serde_json::to_string(&label.provenance).map_err(|error| {
            OpenVitalsError::message(format!("cannot serialize label provenance: {error}"))
        })?;
    ...
    Ok(json!({
        "schema": "open_vitals.calibration-label-import-report.v1",
        "generated_by": "open-vitals-bridge",
        "pass": true,
...
fn list_calibration_labels_bridge(
    ...
    let labels = store.calibration_labels_between(&args.start, &args.end)?;
    Ok(json!({
        "schema": "open_vitals.calibration-label-list.v1",
        "generated_by": "open-vitals-bridge",
        "start": args.start,
        "end": args.end,
...
fn apply_calibration_bridge(args: ApplyCalibrationArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    let calibration_run = match args.calibration_run_id.as_deref() {
        Some(calibration_run_id) if !calibration_run_id.trim().is_empty() => {
            store.calibration_run(calibration_run_id)?.ok_or_else(|| {
                OpenVitalsError::message(format!("calibration run {calibration_run_id} not found"))
            })?
        }
    ...
    serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize calibration application: {error}"))
    })
}
...
fn maybe_persist_calibration_report(
    ...
    let calibration_run_id = calibration_run_id
        ...
        .ok_or_else(|| {
            OpenVitalsError::message("calibration_run_id is required when persist is true")
        })?;
...
fn stored_calibration_dataset(
    ...
    (
        CalibrationDataset {
            schema: "open_vitals.calibration-dataset.v1".to_string(),
            records,
        },
        matched_records,
        issues,
...
fn raw_export_bridge(args: RawExportArgs) -> OpenVitalsResult<serde_json::Value> {
    if args.output_dir.trim().is_empty() {
        return Err(OpenVitalsError::message("output_dir is required"));
    }
    let store = open_bridge_store(&args.database_path)?;
...
fn local_health_validation_manifest_runbook_bridge(
    ...
    if !args.manifest.is_object() {
        return Err(OpenVitalsError::message("manifest object is required"));
    }
    ...
    let manifest_schema = args
        ...
        .unwrap_or("unknown");
    Ok(serde_json::json!({
        "schema": "open_vitals.local-health-validation-runbook.v1",
...
fn privacy_lint_bridge(args: PrivacyLintArgs) -> OpenVitalsResult<serde_json::Value> {
    if args.path.trim().is_empty() {
        return Err(OpenVitalsError::message("path is required"));
    }
    let report = lint_privacy_path(Path::new(&args.path))?;
    serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize privacy lint report: {error}"))
    })
}
...
fn capture_sanitize_bridge(args: CaptureSanitizeArgs) -> OpenVitalsResult<serde_json::Value> {
    if args.input_path.trim().is_empty() {
        return Err(OpenVitalsError::message("input_path is required"));
    }
    if args.output_path.trim().is_empty() {
        return Err(OpenVitalsError::message("output_path is required"));
    }
    ...
    serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize capture sanitize report: {error}"))
    })
}

fn ui_coverage_audit_bridge(args: UiCoverageAuditArgs) -> OpenVitalsResult<serde_json::Value> {
    let input_path = args
        .coverage_map_path
...
fn perf_budget_bridge(args: PerfBudgetArgs) -> OpenVitalsResult<serde_json::Value> {
    let report = run_perf_budget(PerfBudgetOptions {
        scale: args.scale,
        budgets: PerfBudgets::default(),
    })?;
    serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize perf budget report: {error}"))
    })
}

fn property_suite_bridge(args: PropertySuiteArgs) -> OpenVitalsResult<serde_json::Value> {
    let report = run_property_suite(PropertySuiteOptions {
        seed: args.seed,
        cases_per_group: args.cases_per_group,
    })?;
    serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize property suite report: {error}"))
    })
}

fn reference_compare_bridge(args: ReferenceCompareArgs) -> OpenVitalsResult<serde_json::Value> {
    let report = match args.family.as_str() {
        "hrv" => {
            let input: HrvInput = serde_json::from_value(args.input)
                .map_err(|error| OpenVitalsError::message(format!("invalid HRV input: {error}")))?;
            compare_hrv_open_vitals_to_reference(&input)?
        }
        "sleep" => {
            let use_sleep_v1 = args
                .open_vitals_algorithm_id
                .as_deref()
                .is_some_and(|id| id == crate::metrics::OPENVITALS_SLEEP_V1_ID)
                || args
                    .input
                    .get("sleep")
...
fn motion_features_bridge(args: MotionFeaturesArgs) -> OpenVitalsResult<serde_json::Value> {
    ...
    serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize motion feature report: {error}"))
    })
}

fn heart_rate_features_bridge(args: HeartRateFeaturesArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn step_counter_ingest_bridge(args: StepCounterIngestArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn energy_daily_rollup_bridge(args: EnergyDailyRollupArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn hrv_features_bridge(args: HrvFeaturesArgs) -> OpenVitalsResult<serde_json::Value> {
    validate_requested_primary_algorithm(
        ...
        OPENVITALS_HRV_V0_ID,
        OPENVITALS_HRV_V0_VERSION,
    )?;
    let store = open_bridge_store(&args.database_path)?;
    ...
    let mut value = serde_json::to_value(&report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize HRV feature report: {error}"))
    })?;
...
fn sleep_v1_input_from_feature_score(
    ...
    history_import_in_progress: bool,
) -> OpenVitalsResult<SleepV1Input> {
    let prior_history_end_unix_ms = sleep_time_unix_ms(&sleep_input.start_time)
        .ok_or_else(|| OpenVitalsError::message("sleep_v1_input_start_time_invalid"))?;
    ...
    let trusted_open_vitals_sleep_nights = u32::from(
        report
            .sleep_window
            .as_ref()
    ...
    Ok(SleepV1Input {
        ...
        model_status: SleepModelStatusInput {
            ...
            trusted_open_vitals_sleep_nights,
...
fn external_sleep_history_nights_for_sleep_v1(
    ...
    before_unix_ms: i64,
) -> OpenVitalsResult<Vec<SleepNightHistoryInput>> {
...
fn sleep_feature_score_bridge(args: SleepFeatureScoreArgs) -> OpenVitalsResult<serde_json::Value> {
    let requested_algorithm_id = args
        .algorithm_id
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or(OPENVITALS_SLEEP_V0_ID);
    let requested_algorithm_version = args
        .algorithm_version
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or(if requested_algorithm_id == OPENVITALS_SLEEP_V1_ID {
            OPENVITALS_SLEEP_V1_VERSION
        } else {
            OPENVITALS_SLEEP_V0_VERSION
        });
    let sleep_v1_requested = match (requested_algorithm_id, requested_algorithm_version) {
        (OPENVITALS_SLEEP_V0_ID, OPENVITALS_SLEEP_V0_VERSION) => false,
        (OPENVITALS_SLEEP_V1_ID, OPENVITALS_SLEEP_V1_VERSION) => true,
        _ => {
            return Err(OpenVitalsError::message(format!(
                "unsupported primary algorithm {requested_algorithm_id}@{requested_algorithm_version} for sleep; this packet-derived scorer currently supports {OPENVITALS_SLEEP_V0_ID}@{OPENVITALS_SLEEP_V0_VERSION} and {OPENVITALS_SLEEP_V1_ID}@{OPENVITALS_SLEEP_V1_VERSION}"
            )));
        }
    };
    let store = open_bridge_store(&args.database_path)?;
    ...
    if sleep_v1_requested {
        if let Some(sleep_input) = report.sleep_input.as_ref() {
            ...
            value["sleep_v1_input"] = serde_json::to_value(&sleep_v1_input).map_err(|error| {
                OpenVitalsError::message(format!("cannot serialize sleep v1 input: {error}"))
            })?;
...
fn health_sync_dry_run_bridge(input: HealthSyncDryRunInput) -> OpenVitalsResult<serde_json::Value> {
...
fn capture_import_frame_batch_bridge(
    ...
    serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize capture import report: {error}"))
    })
}
...
fn overnight_mirror_counts_bridge(
    ...
    serde_json::to_value(counts).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize overnight mirror counts: {error}"))
    })
}

fn capture_timeline_bridge(args: CaptureTimelineArgs) -> OpenVitalsResult<serde_json::Value> {
    if args.start.trim().is_empty() {
        return Err(OpenVitalsError::message("start is required"));
    }
    if args.end.trim().is_empty() {
...
fn capture_observability_timeline_bridge(
    ...
    serde_json::to_value(rows).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize observability timeline: {error}"))
    })
}
...
fn capture_start_session_bridge(
    ...
    let session = store.capture_session(&args.session_id)?.ok_or_else(|| {
        OpenVitalsError::message(format!("capture session {} not found", args.session_id))
    })?;
    serde_json::to_value(json!({
        "schema": "open_vitals.capture-session-result.v1",
        "inserted": inserted,
        "session": session,
    }))
...
fn activity_create_session_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.activity-session-result.v1",
        "generated_by": "open-vitals-bridge",
        "inserted": inserted,
        "session": session,
    }))
...
fn activity_list_sessions_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.activity-session-list.v1",
        "generated_by": "open-vitals-bridge",
        "start_time_unix_ms": args.start_time_unix_ms,
        "end_time_unix_ms": args.end_time_unix_ms,
        "session_count": sessions.len(),
...
fn activity_list_sessions_with_metrics_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.activity-session-list-with-metrics.v1",
        "generated_by": "open-vitals-bridge",
        "start_time_unix_ms": args.start_time_unix_ms,
        "end_time_unix_ms": args.end_time_unix_ms,
        "session_count": sessions.len(),
...
fn activity_correction_plans_bridge() -> OpenVitalsResult<serde_json::Value> {
    let plans = activity_session_correction_plans();
    Ok(json!({
        "schema": "open_vitals.activity-correction-plans.v1",
        "generated_by": "open-vitals-bridge",
...
fn activity_apply_correction_bridge(
    ...
    if !args.details.is_object() {
        return Err(OpenVitalsError::message("details must be a JSON object"));
    }
    if !args.provenance.is_object() {
        return Err(OpenVitalsError::message("provenance must be a JSON object"));
    }
    ...
    match args.kind {
        ...
        ActivitySessionCorrectionKind::TrimEnd => {
            end_time_unix_ms = args.end_time_unix_ms.ok_or_else(|| {
                OpenVitalsError::message("end_time_unix_ms is required for trim_end corrections")
            })?;
        }
    ...
    Ok(json!({
        "schema": "open_vitals.activity-correction-result.v1",
        "generated_by": "open-vitals-bridge",
        "session_id": args.session_id,
        "kind": args.kind,
        "updated": updated,
...
fn activity_delete_session_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.activity-session-delete-result.v1",
        "generated_by": "open-vitals-bridge",
        "session_id": args.session_id,
        "deleted": deleted,
    }))
...
fn activity_attach_metric_bridge(
    ...
    let metric = store.activity_metric(&args.metric_id)?.ok_or_else(|| {
        OpenVitalsError::message(format!("activity metric {} not found", args.metric_id))
    })?;
    Ok(json!({
        "schema": "open_vitals.activity-metric-result.v1",
        "generated_by": "open-vitals-bridge",
        "inserted": inserted,
        "metric": metric,
    }))
...
fn activity_attach_metrics_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.activity-metric-batch-result.v1",
        "generated_by": "open-vitals-bridge",
...
fn activity_list_metrics_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.activity-metric-list.v1",
        "generated_by": "open-vitals-bridge",
        "activity_session_id": args.activity_session_id,
        "metric_count": metrics.len(),
        "metrics": metrics,
...
fn activity_metrics_for_session_in_window_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.activity-metric-window.v1",
        "generated_by": "open-vitals-bridge",
        "activity_session_id": args.activity_session_id,
        "start_time_unix_ms": args.start_time_unix_ms,
        "end_time_unix_ms": args.end_time_unix_ms,
...
fn activity_attach_interval_bridge(
    ...
    let interval = store.activity_interval(&args.interval_id)?.ok_or_else(|| {
        OpenVitalsError::message(format!("activity interval {} not found", args.interval_id))
    })?;
    Ok(json!({
        "schema": "open_vitals.activity-interval-result.v1",
        "generated_by": "open-vitals-bridge",
        "inserted": inserted,
        "interval": interval,
    }))
...
fn activity_list_intervals_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.activity-interval-list.v1",
        "generated_by": "open-vitals-bridge",
        "activity_session_id": args.activity_session_id,
...
fn external_sleep_history_import_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.external-sleep-history-import-result.v1",
        "generated_by": "open-vitals-bridge",
...
fn sleep_correction_label_bridge(
    ...
    let label = store
        ...
        .ok_or_else(|| OpenVitalsError::message("sleep correction label was not stored"))?;
    Ok(json!({
        "schema": "open_vitals.sleep-correction-label-result.v1",
        "generated_by": "open-vitals-bridge",
        "inserted": inserted,
        "label": label,
...
fn sleep_correction_label_list_bridge(
    ...
    let distinct_sleep_window_sleep_id_count = labels
        ...
        .len();
    Ok(json!({
        "schema": "open_vitals.sleep-correction-label-list.v1",
        "generated_by": "open-vitals-bridge",
        "label_count": labels.len(),
...
fn capture_correlation_bridge(args: CaptureCorrelationArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn capture_arrival_plan_bridge(
    ...
    let report = CaptureArrivalPlanReport {
        ...
        generated_by: "open-vitals-capture-arrival-plan".to_string(),
        pass,
        start: args.start,
        end: args.end,
        min_owned_captures,
    ...
    serde_json::to_value(report).map_err(|error| {
        OpenVitalsError::message(format!("cannot serialize capture arrival plan: {error}"))
    })
}
...
fn capture_arrival_physical_rows(
    ...
    rows.push(capture_arrival_physical_row(
        ...
        "Record start, end, pauses, sport/activity type, confidence, and whether type came from WHOOP bytes, app metadata, or OpenVitals inference.",
...
) -> OpenVitalsResult<(Vec<CaptureSessionRow>, Vec<ActivitySessionRow>)> {
...
fn command_import_validation_records_bridge(
    ...
    Ok(json!({
        "schema": "open_vitals.command-validation-import-report.v1",
        "generated_by": "open-vitals-command-validation-import",
    ...
    for result in results {
        store.upsert_command_validation_record(&CommandValidationRecord {
            ...
            report_json: serde_json::to_string(result).map_err(|error| {
                OpenVitalsError::message(format!("cannot serialize command result: {error}"))
            })?,
        })?;
    }
    Ok(())
...
fn debug_start_session_bridge(args: DebugStartSessionArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn debug_start_command_bridge(args: DebugStartCommandArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn debug_record_event_bridge(args: DebugRecordEventArgs) -> OpenVitalsResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
) -> OpenVitalsResult<Option<crate::store::CalibrationRunRecord>> {
...
fn open_bridge_store(database_path: &str) -> OpenVitalsResult<OpenVitalsStore> {
    if database_path.trim().is_empty() {
        return Err(OpenVitalsError::message("database_path is required"));
    }
    OpenVitalsStore::open(Path::new(database_path))
}

fn json_object_string(field_name: &str, value: &serde_json::Value) -> OpenVitalsResult<String> {
    if !value.is_object() {
        return Err(OpenVitalsError::message(format!(
...
fn request_args<T>(request: &BridgeRequest) -> OpenVitalsResult<T>
where
    T: for<'de> Deserialize<'de>,
{
    serde_json::from_value(request.args.clone())
        .map_err(|error| OpenVitalsError::message(format!("invalid args: {error}")))
}

fn parse_device_type(value: &str) -> OpenVitalsResult<DeviceType> {
    match value {
        ...
        "OPENVITALS" | "OpenVitals" | "openVitals" => Ok(DeviceType::OpenVitals),
...
fn default_raw_export_app_version() -> String {
    "open-vitals-app/bridge".to_string()
}
...
fn default_capture_sanitize_salt() -> String {
    "open-vitals-capture-sanitize-v1".to_string()
}
