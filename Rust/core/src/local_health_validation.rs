...
pub fn scaffold_local_health_validation_manifest(
    ...
    manifest.insert(
        "notes".to_string(),
        json!(
            "Generated from OpenVitals-owned packet evidence. Fill manual/official labels before treating labeled cases as acceptance."
        ),
    );
...
pub fn local_health_validation_manifest_runbook_markdown(manifest: &Value) -> String {
    ...
    let command = run_validation
        ...
        .and_then(value_string)
        .unwrap_or_else(|| {
            "open-vitals-local-health-validation-suite --manifest local-health-validation-manifest.json"
                .to_string()
        });
...
pub fn review_local_health_validation_manifest(manifest: &Value) -> Value {
    ...
    json!({
        "schema": "open_vitals.local-health-validation-manifest-review.v1",
        ...
        .map(|blocker| {
            let action = match *blocker {
                "manifest_schema_invalid" => {
                    "Regenerate the validation manifest with open_vitals.local-health-validation-manifest.v1."
                }
                "no_validation_cases" => {
...
fn scaffold_time_window(
    options: &LocalHealthValidationManifestScaffoldOptions,
) -> OpenVitalsResult<(String, String, String)> {
...
) -> OpenVitalsResult<Option<(String, String)>> {
...
) -> OpenVitalsResult<ScaffoldEvidenceSummary> {
    let connection = open_read_only_database(database_path)?;
...
fn open_read_only_database(database_path: &PathBuf) -> OpenVitalsResult<Connection> {
...
fn scaffold_run_validation(options: &LocalHealthValidationManifestScaffoldOptions) -> Value {
    ...
    let mut args = vec!["open-vitals-local-health-validation-suite".to_string()];
    ...
    let command = args
        ...
        .join(" ");
    json!({
        "cli": "open-vitals-local-health-validation-suite",
        "args": args,
