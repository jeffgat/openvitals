...
pub const FIXTURE_INDEX_SCHEMA: &str = "open_vitals.fixture-index.v1";
pub const FRAME_HEX_SCHEMA: &str = "open_vitals.frame.hex.v1";
pub const CAPTURED_FRAME_BATCH_SCHEMA: &str = "open_vitals.captured-frame-batch.v1";
pub const PAYLOAD_HEX_SCHEMA: &str = "open_vitals.payload.hex.v1";
pub const ACTIVITY_SESSION_FIXTURE_SCHEMA: &str = "open_vitals.activity-session-fixtures.v1";
pub const OPENWHOOP_REFERENCE_FIXTURE_SCHEMA: &str = "open_vitals.openwhoop-reference-fixture.v1";
pub const COMMAND_VALIDATION_FIXTURE_SCHEMA: &str = "open_vitals.command-validation-fixtures.v1";
...
pub fn build_fixture_index(root: &Path) -> OpenVitalsResult<FixtureIndexReport> {
    ...
    for sidecar in sidecars {
        let raw =
            fs::read_to_string(&sidecar).map_err(|source| OpenVitalsError::io(&sidecar, source))?;
        let metadata: FixtureMetadata =
            serde_json::from_str(&raw).map_err(|source| OpenVitalsError::json(&sidecar, source))?;
    ...
    Ok(FixtureIndexReport {
        ...
        generated_by: "open-vitals-fixture-index".to_string(),
...
pub fn run_parser_fixtures(root: &Path, index: &FixtureIndexReport) -> ParserFixtureReport {
    ...
    if results.is_empty() {
        issues.push("no open_vitals.frame.hex.v1 fixtures found".to_string());
    }

    let next_actions = parser_fixture_report_next_actions(&issues, &results);

    ParserFixtureReport {
        schema: "open_vitals.parser-fixture-report.v1".to_string(),
        generated_by: "open-vitals-parser-fixture-runner".to_string(),
        fixture_root: root.display().to_string(),
        pass: issues.is_empty(),
        fixtures: results,
        issues,
...
pub fn load_fixture_index(path: &Path) -> OpenVitalsResult<FixtureIndexReport> {
...
fn run_frame_fixture(root: &Path, fixture: &IndexedFixture) -> ParserFixtureResult {
    ...
    let device_type = expected_device_type(&expected).unwrap_or(DeviceType::OpenVitals);
...
fn expected_device_type(expected: &serde_json::Value) -> Option<DeviceType> {
    ...
    match value {
        ...
        "OPENVITALS" => Some(DeviceType::OpenVitals),
        _ => None,
    }
}
...
fn parser_fixture_report_next_actions(
    ...
    for issue in issues {
        if issue == "no open_vitals.frame.hex.v1 fixtures found" {
            actions.push(FixtureNextAction {
                scope: "parser_fixtures".to_string(),
                reason: "no_parser_fixtures".to_string(),
                action: "Add at least one open_vitals.frame.hex.v1, open_vitals.payload.hex.v1, or open_vitals.captured-frame-batch.v1 fixture and regenerate the fixture index.".to_string(),
            });
...
fn parser_fixture_issue_action(issue: &str) -> (&'static str, &'static str) {
    if issue.starts_with("cannot read fixture file:") {
        ...
        (
            ...
            "Update the captured-frame batch schema to open_vitals.captured-frame-batch.v1 or add a migration before trusting it.",
        )
...
fn collect_sidecars(root: &Path, sidecars: &mut Vec<PathBuf>) -> OpenVitalsResult<()> {
    if !root.exists() {
        return Ok(());
    }
    for entry in fs::read_dir(root).map_err(|source| OpenVitalsError::io(root, source))? {
        let entry = entry.map_err(|source| OpenVitalsError::io(root, source))?;
        let path = entry.path();
        if path.is_dir() {
...
fn collect_data_files(root: &Path, files: &mut Vec<PathBuf>) -> OpenVitalsResult<()> {
    if !root.exists() {
        return Ok(());
    }
    for entry in fs::read_dir(root).map_err(|source| OpenVitalsError::io(root, source))? {
