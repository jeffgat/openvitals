...
fn run() -> open_vitals_core::OpenVitalsResult<()> {
    ...
    if flag(&args, "--compare-reference") {
        ...
        let cli_data_coverage = serde_json::to_value(data_coverage_from_report(
            ...
            &report.start_time,
            &report.end_time,
            report.open_vitals_output.is_some() && report.reference_output.is_some(),
    ...
    let algorithm_id = value(&args, "--algorithm")?
        ...
        .unwrap_or_else(|| OPENVITALS_HRV_V0_ID.to_string());
    ...
    let report = AlgoBenchmarkReport {
        schema: "open_vitals.algo-benchmark-report.v1".to_string(),
        generated_by: "open-vitals-algo-benchmark".to_string(),
        family: run.family,
...
) -> open_vitals_core::OpenVitalsResult<AlgorithmComparisonReport> {
    match family {
        "hrv" => {
        ...
        "sleep" => {
            if algorithm_id == Some(OPENVITALS_SLEEP_V1_ID) {
...
fn reject_external_reference_report_for_sleep_v1(
    ...
    if reference_report_path.is_some() {
        return Err(OpenVitalsError::message(
            "--reference-report currently compares external sleep reports against open_vitals.sleep.v0; omit it for open_vitals.sleep.v1 reference comparison",
        ));
    }
    Ok(())
}
...
fn benchmark_label_args(args: &[String]) -> open_vitals_core::OpenVitalsResult<Option<BenchmarkLabelArgs>> {
    ...
    let label_value = value_raw
        .parse::<f64>()
        .map_err(|error| OpenVitalsError::message(format!("invalid --label-value: {error}")))?;
    ...
    let max_absolute_error = value(args, "--max-absolute-error")?
        .map(|raw| {
            raw.parse::<f64>().map_err(|error| {
                OpenVitalsError::message(format!("invalid --max-absolute-error: {error}"))
            })
        })
        .transpose()?;
...
fn read_typed_input<I>(input_path: &PathBuf) -> open_vitals_core::OpenVitalsResult<I>
where
    I: DeserializeOwned,
{
    let input_raw =
...
fn read_json_value(path: &PathBuf) -> open_vitals_core::OpenVitalsResult<serde_json::Value> {
...
fn input_path(args: &[String], algorithm_id: &str) -> open_vitals_core::OpenVitalsResult<PathBuf> {
    if let Some(path) = path_value(args, "--input")? {
        return Ok(path);
    }
    if algorithm_id == OPENVITALS_HRV_V0_ID {
...
fn default_algorithm_for_family(family: &str) -> Option<String> {
    match family {
        "hrv" => Some("open_vitals.hrv.v0".to_string()),
        "sleep" => Some("open_vitals.sleep.v0".to_string()),
        "strain" => Some("open_vitals.strain.v0".to_string()),
        "recovery" => Some("open_vitals.recovery.v0".to_string()),
        "stress" => Some("open_vitals.stress.v0".to_string()),
        _ => None,
    }
}
