    ...
    Ok(value(args, name)?
        .map(|raw| {
            raw.parse::<usize>().map_err(|source| {
                open_vitals_core::OpenVitalsError::message(format!("invalid {name}: {source}"))
            })
        })
        .transpose()?)
}

fn optional_u64(args: &[String], name: &str) -> open_vitals_core::OpenVitalsResult<Option<u64>> {
    Ok(value(args, name)?
        .map(|raw| {
...
fn optional_mib(args: &[String], name: &str) -> open_vitals_core::OpenVitalsResult<Option<u64>> {
