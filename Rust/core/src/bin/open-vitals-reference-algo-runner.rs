...
use open_vitals_core::{
    ...
    store::{AlgorithmDefinitionRecord, AlgorithmRunRecord, OpenVitalsStore},
...
const EXTERNAL_REFERENCE_OUTPUT_SCHEMA: &str = "open_vitals.external-reference-output.v1";
...
fn run() -> open_vitals_core::OpenVitalsResult<()> {
    ...
    let report = ReferenceAlgoReport {
        schema: "open_vitals.reference-algo-report.v1".to_string(),
        generated_by: "open-vitals-reference-algo-runner".to_string(),
        family: run.family,
        provider,
...
fn run_external_reference_algorithm(
    ...
    command
        ...
        .env("OPENVITALS_REFERENCE_INPUT_PATH", input_path)
        .env("OPENVITALS_REFERENCE_INPUT_SHA256", &input_sha256)
        .env("OPENVITALS_REFERENCE_FAMILY", family)
        .env("OPENVITALS_REFERENCE_PROVIDER", provider)
        .env(
            "OPENVITALS_REFERENCE_OUTPUT_SCHEMA",
...
fn reference_next_actions(
    ...
    if !storage_ready {
        actions.push(ReferenceNextAction {
            ...
            action: "Rerun with a writable OpenVitals SQLite database or omit --db for a report-only benchmark.".to_string(),
        });
    }
    for error in errors {
        actions.push(ReferenceNextAction {
...
fn external_reference_definition(
    report: &ExternalReferenceOutput,
) -> open_vitals_core::OpenVitalsResult<AlgorithmDefinitionRecord> {
    Ok(AlgorithmDefinitionRecord {
        ...
        input_schema: report
            .input_schema
            .clone()
            .unwrap_or_else(|| format!("open_vitals.{}-input.v1", report.family)),
...
fn values(args: &[String], name: &str) -> open_vitals_core::OpenVitalsResult<Vec<String>> {
...
fn require_non_empty(field: &str, value: &str) -> open_vitals_core::OpenVitalsResult<()> {
...
fn require_array(field: &str, value: &serde_json::Value) -> open_vitals_core::OpenVitalsResult<()> {
    if !value.is_array() {
        return Err(OpenVitalsError::message(format!(
...
fn reference_input_path(
    args: &[String],
    family: &str,
) -> open_vitals_core::OpenVitalsResult<std::path::PathBuf> {
    match family {
        "hrv" => default_path(
            args,
            "--input",
            "fixtures/synthetic/hrv_open_vitals_v0_hand_derived.json",
        ),
        "sleep" => default_path(
            args,
            "--input",
