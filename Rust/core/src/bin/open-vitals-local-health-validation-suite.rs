...
const MANIFEST_SCHEMA: &str = "open_vitals.local-health-validation-manifest.v1";
const REPORT_SCHEMA: &str = "open_vitals.local-health-validation-suite-report.v1";
...
fn run() -> open_vitals_core::OpenVitalsResult<()> {
    ...
    let manifest_path = path_value(&args, "--manifest")?
        .ok_or_else(|| OpenVitalsError::message("--manifest is required"))?;
    let output = path_value(&args, "--output")?;
    let markdown_output = path_value(&args, "--markdown-output")?;
    let review_output = path_value(&args, "--review-output")?;
...
fn resolve_validation_database(
...
) -> open_vitals_core::OpenVitalsResult<ResolvedValidationDatabase> {
    match (database_path, raw_export_bundle_path) {
        ...
        (Some(_), Some(_)) => Err(OpenVitalsError::message(
            "use either --database or --raw-export-bundle, not both",
        )),
        (None, None) => Err(OpenVitalsError::message(
...
fn resolve_raw_export_bundle_database(
    ...
    if bundle_path.is_dir() {
        let database_path = bundle_path.join("data").join("open_vitals.sqlite");
        ...
        return Err(OpenVitalsError::message(format!(
            "raw export bundle {} is missing data/open_vitals.sqlite",
            bundle_path.display()
        )));
    }
...
fn audit_validation_sqlite(
    ...
    let storage_schema_version = match connection.query_row(
        "SELECT MAX(version) FROM open_vitals_schema_migrations",
        [],
...
fn sqlite_table_exists(
    ...
    connection
        ...
        .map(|value| value != 0)
        .map_err(|error| OpenVitalsError::message(format!("cannot inspect SQLite tables: {error}")))
}
...
fn raw_export_manifest_audit_from_raw(
    ...
    let mut issues = Vec::new();
    if manifest.schema_version != "open_vitals.export.v1" {
        issues.push(format!(
    ...
    let sqlite_files = manifest
        .files
        .iter()
        .filter(|file| file.path == "data/open_vitals.sqlite")
        .collect::<Vec<_>>();
...
fn file_sha256_hex(path: &Path) -> open_vitals_core::OpenVitalsResult<String> {
    let bytes = fs::read(path).map_err(|source| OpenVitalsError::io(path, source))?;
    Ok(sha256_hex(&bytes))
}
...
fn extract_raw_export_sqlite_from_zip(
    ...
    let entry_name = raw_export_sqlite_zip_entry_name(&archive).ok_or_else(|| {
        OpenVitalsError::message(format!(
            "raw export zip {} is missing data/open_vitals.sqlite",
            bundle_path.display()
        ))
    })?;
...
fn run_manifest(
    ...
    LocalHealthValidationSuiteReport {
        ...
        generated_by: "open-vitals-local-health-validation-suite".to_string(),
...
fn write_json_file<T: Serialize>(
    ...
    let json = serde_json::to_string_pretty(report)
        .map_err(|source| OpenVitalsError::message(format!("cannot serialize report: {source}")))?;
    fs::write(path, json.as_bytes()).map_err(|source| OpenVitalsError::io(path, source))
}

fn write_markdown_text(markdown: &str, path: &Path) -> open_vitals_core::OpenVitalsResult<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|source| OpenVitalsError::io(parent, source))?;
    }
    fs::write(path, markdown.as_bytes()).map_err(|source| OpenVitalsError::io(path, source))
}
...
fn run_capture_sqlite_import(
    ...
    let device_model = import
        .device_model
        .clone()
        .unwrap_or_else(|| "WHOOP 5.0 OpenVitals".to_string());
    ...
    let store = match OpenVitalsStore::open(Path::new(database_path)) {
        Ok(store) => store,
        Err(error) => {
            return capture_sqlite_import_error_report(
                import,
...
fn metric_records_for_case(
    ...
    match str_field(result, &["schema"]).as_deref() {
        Some("open_vitals.step-packet-discovery-report.v1") => {
            vec![step_discovery_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("open_vitals.step-capture-validation-report.v1") => {
            vec![step_validation_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("open_vitals.raw-motion-step-estimate-report.v1") => {
            vec![raw_motion_step_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("open_vitals.step-counter-daily-rollup-report.v1") => {
        ...
        Some("open_vitals.step-counter-hourly-rollup-report.v1") => {
            vec![step_counter_rollup_metric_record(
                case_id,
                report,
                method,
        ...
        Some("open_vitals.activity-unavailable-daily-status-report.v1") => {
            activity_unavailable_status_metric_records(case_id, report, method, result)
        }
        Some("open_vitals.energy-capture-validation-report.v1") => {
            energy_validation_metric_records(case_id, report, method, result)
        }
        Some("open_vitals.energy-daily-rollup-report.v1") => {
            energy_rollup_metric_records(case_id, report, method, result, "daily_rollup")
        }
        Some("open_vitals.energy-unavailable-daily-status-report.v1") => {
            energy_unavailable_status_metric_records(case_id, report, method, result)
        }
        Some("open_vitals.energy-hourly-rollup-report.v1") => {
            energy_rollup_metric_records(case_id, report, method, result, "hourly_rollup")
        }
        Some("open_vitals.resting-heart-rate-capture-validation-report.v1") => {
            vec![rhr_validation_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("open_vitals.resting-heart-rate-daily-rollup-report.v1") => {
            vec![rhr_rollup_metric_record(case_id, report, method, result)]
        }
        Some("open_vitals.hrv-capture-validation-report.v1") => {
            vec![hrv_validation_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("open_vitals.respiratory-rate-capture-validation-report.v1") => {
            vec![respiratory_rate_validation_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("open_vitals.oxygen-saturation-capture-validation-report.v1") => {
            vec![oxygen_saturation_validation_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("open_vitals.temperature-capture-validation-report.v1") => {
            vec![temperature_validation_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("open_vitals.recovery-sensor-discovery-report.v1") => {
            recovery_sensor_metric_records(case_id, report, method, result)
        }
        Some("open_vitals.recovery-sensor-daily-rollup-report.v1") => {
            recovery_sensor_daily_rollup_metric_records(case_id, report, method, result)
        }
        Some("open_vitals.recovery-unavailable-daily-status-report.v1") => {
...
fn step_discovery_metric_record(
    ...
    metric_record(
        ...
        None,
        Some("open_vitals.steps.step_packet_discovery.v0".to_string()),
...
fn step_validation_metric_record(
    ...
    metric_record(
        ...
        None,
        Some("open_vitals.steps.device_counter.v0".to_string()),
        Some("0.1.0".to_string()),
...
fn energy_rollup_metric_records(
    ...
    .map(|(metric_name, unit)| {
        ...
        metric_record(
            ...
            f64_field(result, &["confidence"]),
            Some("open_vitals.energy.local_estimate.v0".to_string()),
            Some("0.1.0".to_string()),
            None,
            Some(promotion_status.to_string()),
            string_array(result, &["issues"]),
    ...
    result
        ...
        .map(|status| {
            ...
            metric_record(
                ...
                Some(0.0),
                Some("open_vitals.energy.unavailable_status.v0".to_string()),
                Some("0.1.0".to_string()),
                None,
...
fn rhr_rollup_metric_record(
    ...
    metric_record(
        ...
        f64_field(result, &["confidence"]),
        Some("open_vitals.resting_heart_rate.device_sensor.v0".to_string()),
        Some("0.1.0".to_string()),
        None,
    ...
    result
        ...
        .map(|status| {
            ...
            metric_record(
                ...
                Some("open_vitals.recovery_sensor.device_sensor.v0".to_string()),
                Some("0.1.0".to_string()),
                None,
                str_field(status, &["promotion_status"]),
                Vec::new(),
    ...
    result
        ...
        .map(|status| {
            ...
            metric_record(
                ...
                Some(0.0),
                Some("open_vitals.recovery.unavailable_status.v0".to_string()),
                Some("0.1.0".to_string()),
                None,
                str_field(status, &["promotion_status"]),
                Vec::new(),
    ...
    result
        ...
        .map(|status| {
            metric_record(
                ...
                Some(0.0),
                Some("open_vitals.activity.unavailable_status.v0".to_string()),
                Some("0.1.0".to_string()),
                None,
                str_field(status, &["promotion_status"]),
                Vec::new(),
...
fn scoped_database_path_for_case(
    ...
    {
        let _ = OpenVitalsStore::open(&scoped_path)?;
    }
...
fn copy_capture_session_scope(
    ...
    let connection = Connection::open(scoped_path).map_err(|error| {
        OpenVitalsError::message(format!("cannot open scoped validation database: {error}"))
    })?;
    ...
    let copy_result = (|| -> open_vitals_core::OpenVitalsResult<()> {
        connection
            ...
            .map_err(|error| {
                OpenVitalsError::message(format!("cannot copy scoped packet evidence: {error}"))
            })?;
...
fn source_table_exists(
    ...
    connection
        ...
        .map(|value| value != 0)
        .map_err(|error| OpenVitalsError::message(format!("cannot inspect source tables: {error}")))
}

fn persist_scoped_formatted_metric_writes(
    source_database_path: &str,
    scoped_path: &Path,
) -> open_vitals_core::OpenVitalsResult<usize> {
    ...
    let copy_result = (|| -> open_vitals_core::OpenVitalsResult<usize> {
        ...
        for table in [
            ...
            changed += connection
                ...
                .map_err(|error| {
                    OpenVitalsError::message(format!("cannot copy scoped {table}: {error}"))
                })?;
        }
        Ok(changed)
    })();
...
fn unique_validation_temp_path(token: &str) -> PathBuf {
    ...
    env::temp_dir().join(format!(
        "open-vitals-local-health-validation-{}-{}-{}.sqlite",
        process::id(),
...
fn suite_issue_action(issue: &str) -> &'static str {
    match issue {
        ...
        issue if issue.starts_with("manifest_json_invalid") => {
            "Regenerate manifest.json from open-vitals-raw-export so it matches the OpenVitals Raw Export manifest schema."
        }
        issue if issue.starts_with("manifest_schema_version_unexpected") => {
            "Regenerate the Raw Export bundle with schema_version open_vitals.export.v1."
        }
        ...
        issue if issue.starts_with("case_window_packet_evidence_query_failed") => {
            "Repair or regenerate data/open_vitals.sqlite so raw and decoded packet evidence can be inspected for each validation case window."
        }
        issue
        ...
        {
            "Repair or regenerate data/open_vitals.sqlite so decoded packet families can be inspected for each validation case window."
        }
        issue
        ...
        {
            "Repair or regenerate data/open_vitals.sqlite so packet timestamp bounds can be inspected for each validation case window."
        }
        "sqlite_family_not_declared" => {
            "Regenerate the Raw Export bundle with the sqlite family selected so data/open_vitals.sqlite is declared."
        }
        "sqlite_manifest_file_missing" => {
            "Regenerate the Raw Export bundle so manifest.json includes data/open_vitals.sqlite."
        }
        "sqlite_manifest_file_duplicate" => {
            "Regenerate the Raw Export bundle so manifest.json lists data/open_vitals.sqlite exactly once."
        }
        issue if issue.starts_with("sqlite_manifest_kind_unexpected") => {
            "Regenerate the Raw Export bundle so data/open_vitals.sqlite is declared with kind sqlite."
        }
        "sqlite_manifest_sha256_missing" => {
            "Regenerate the Raw Export bundle so data/open_vitals.sqlite has a manifest SHA-256."
        }
        "sqlite_actual_sha256_unavailable" => {
            "Repair data/open_vitals.sqlite permissions or regenerate the Raw Export bundle before validation."
        }
        "sqlite_manifest_sha256_mismatch" => {
            "Regenerate the Raw Export bundle; manifest.json and data/open_vitals.sqlite do not describe the same SQLite snapshot."
        }
        ...
        issue if issue.starts_with("sqlite_open_failed") => {
            "Regenerate the Raw Export bundle; data/open_vitals.sqlite is not an openable SQLite database."
        }
        issue
            if issue.starts_with("sqlite_schema_version_query_failed")
                || issue == "sqlite_schema_version_missing" =>
        {
            "Regenerate the Raw Export bundle from migrated OpenVitals storage so open_vitals_schema_migrations is present."
        }
        issue if issue.starts_with("sqlite_required_table_missing") => {
            "Regenerate the Raw Export bundle from OpenVitals storage that includes local health evidence and formatted metric tables."
        }
        issue
        ...
        {
            "Repair or regenerate data/open_vitals.sqlite so canonical OpenVitals tables can be inspected."
        }
        ...
        "capture_sqlite_raw_import_incomplete" => {
            "Repair the target OpenVitals DB or raw_evidence constraints, then rerun the capture.sqlite import."
        }
