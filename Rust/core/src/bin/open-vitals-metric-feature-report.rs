...
fn run() -> open_vitals_core::OpenVitalsResult<()> {
    ...
    let method = value(&args, "--method")?
        ...
        .ok_or_else(|| OpenVitalsError::message("--method is required"))?;
    ...
    if !response.ok {
        ...
        return Err(OpenVitalsError::message(message));
    }
    let report = response
        .result
        .ok_or_else(|| OpenVitalsError::message("metric feature report missing result"))?;
...
fn metric_bridge_method(value: &str) -> open_vitals_core::OpenVitalsResult<&'static str> {
    match value.trim() {
...
fn merge_args_json(
    ...
    if let Some(raw) = value(args, "--args-json")? {
        merge_args_value(
            ...
            serde_json::from_str(&raw).map_err(|source| {
                OpenVitalsError::message(format!("invalid --args-json object: {source}"))
            })?,
        )?;
    }
    if let Some(path) = path_value(args, "--args-json-file")? {
        ...
        merge_args_value(
            object,
            serde_json::from_str(&raw).map_err(|source| {
                OpenVitalsError::message(format!("invalid --args-json-file object: {source}"))
            })?,
        )?;
    }
    Ok(())
