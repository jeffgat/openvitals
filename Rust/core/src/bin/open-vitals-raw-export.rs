...
fn run() -> open_vitals_core::OpenVitalsResult<()> {
    ...
    let output_dir = default_path(&args, "--output-dir", "exports/latest.openvitalsbundle")?;
    ...
    let store = OpenVitalsStore::open(&db)?;
...
fn csv_values(args: &[String], key: &str) -> open_vitals_core::OpenVitalsResult<Vec<String>> {
    Ok(value(args, key)?
        .map(|values| {
            values
                .split(',')
