...
pub const STEP_COUNTER_INGEST_REPORT_SCHEMA: &str = "open_vitals.step-counter-ingest-report.v1";
...
pub fn persist_step_counter_discovery(
        ...
        .filter(|candidate| candidate.match_kind == "step_count")
    {
        ...
        let provenance_json = json!({
            "algorithm": "open_vitals.step_counter_ingest.v1",
    ...
    Ok(StepCounterIngestReport {
        ...
        generated_by: "open-vitals-step-counter-ingest".to_string(),
...
pub fn rollup_device_step_counter_day(
...
) -> OpenVitalsResult<StepCounterDailyRollupReport> {
    ...
    if pass && options.write_metric {
        ...
        let quality_flags_json = serde_json::to_string(&quality_flags).map_err(|error| {
            OpenVitalsError::message(format!("cannot serialize quality flags: {error}"))
        })?;
        let provenance_json = json!({
            "algorithm": OPENVITALS_STEPS_DEVICE_COUNTER_V0_ID,
            "algorithm_version": OPENVITALS_STEPS_DEVICE_COUNTER_V0_VERSION,
            "source_kind": "device_counter",
    ...
    Ok(StepCounterDailyRollupReport {
        ...
        generated_by: "open-vitals-step-counter-daily-rollup".to_string(),
        pass,
...
pub fn rollup_device_step_counter_hour(
...
) -> OpenVitalsResult<StepCounterHourlyRollupReport> {
    ...
    Ok(StepCounterHourlyRollupReport {
        ...
        generated_by: "open-vitals-step-counter-hourly-rollup".to_string(),
        pass,
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start_time_unix_ms: options.start_time_unix_ms,
...
pub fn rollup_activity_unavailable_daily_status_for_store(
...
) -> OpenVitalsResult<ActivityUnavailableDailyStatusReport> {
    ...
    Ok(ActivityUnavailableDailyStatusReport {
        ...
        generated_by: "open-vitals-activity-unavailable-daily-status".to_string(),
        pass: true,
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start_time_unix_ms: options.start_time_unix_ms,
...
fn activity_steps_unavailable_status_for_rollup(
    ...
    options: ActivityUnavailableDailyStatusOptions<'_>,
) -> OpenVitalsResult<ActivityUnavailableMetricStatus> {
    ...
    if options.write_metric {
        ...
        let provenance_json = json!({
            "algorithm": OPENVITALS_ACTIVITY_UNAVAILABLE_STATUS_V0_ID,
            "algorithm_version": OPENVITALS_ACTIVITY_UNAVAILABLE_STATUS_V0_VERSION,
...
fn validate_rollup_options(options: &StepCounterDailyRollupOptions<'_>) -> OpenVitalsResult<()> {
