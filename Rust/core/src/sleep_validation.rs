...
use crate::{
    ...
    algorithm_compare::{
        ...
        algorithm_comparison_next_actions, compare_sleep_v1_open_vitals_to_reference,
...
pub const SLEEP_WINDOW_LABEL_VALIDATION_SCHEMA: &str =
    "open_vitals.sleep-window-label-validation-report.v1";
pub const SLEEP_WINDOW_LABEL_VALIDATION_INPUT_SCHEMA: &str =
    "open_vitals.sleep-window-label-validation-input.v1";
pub const SLEEP_STAGE_LABEL_VALIDATION_SCHEMA: &str =
    "open_vitals.sleep-stage-label-validation-report.v1";
...
pub const SLEEP_V1_EXPLANATION_STABILITY_SCHEMA: &str =
    "open_vitals.sleep-v1-explanation-stability-report.v1";
...
pub const SLEEP_V1_RELEASE_GATE_SCHEMA: &str = "open_vitals.sleep-v1-release-gate-report.v1";
pub const SLEEP_V1_EVIDENCE_FOLDER_SCHEMA: &str =
    "open_vitals.sleep-v1-validation-evidence-folder-report.v1";
...
pub fn validate_sleep_v1_evidence_folder(
    evidence_dir: &Path,
) -> OpenVitalsResult<SleepV1EvidenceFolderReport> {
...
pub fn validate_sleep_v1_evidence_folder_with_options(
    ...
    let mut report = SleepV1EvidenceFolderReport {
        ...
        generated_by: "open-vitals-sleep-v1-evidence-folder-validator".to_string(),
        pass,
...
fn validate_sleep_v1_evidence_file(
    ...
    if pass == Some(true) {
        ...
        if filename == "sleep-v1-benchmark.json" {
            for field in ["open_vitals_quality_flags", "reference_quality_flags"] {
...
fn expected_sleep_v1_report_generator(filename: &str) -> Option<&'static str> {
    match filename {
        "historical-sync-validation.json" => Some("open-vitals-historical-sync-physical-validator"),
        "sleep-window-validation.json" => Some("open-vitals-sleep-window-label-validator"),
        "sleep-stage-validation.json" => Some("open-vitals-sleep-stage-label-validator"),
        "sleep-v1-stability.json" => Some("open-vitals-sleep-v1-explanation-stability-validator"),
        "sleep-v1-benchmark.json" => Some("open_vitals.algorithm_compare"),
        "sleep-v1-release-gate.json" => Some("open-vitals-sleep-v1-release-gate-validator"),
        _ => None,
    }
}
...
fn validate_sleep_v1_window_derivation(evidence_dir: &Path) -> SleepV1EvidenceDerivationCheck {
    ...
    let issues = match (
        ...
        (Ok(input), Ok(report)) => {
            ...
            match OpenVitalsStore::open_read_only(&db_path).and_then(|store| {
...
fn validate_sleep_v1_benchmark_derivation(evidence_dir: &Path) -> SleepV1EvidenceDerivationCheck {
    ...
    let issues = match (
        ...
        (Ok(input), Ok(report)) => match compare_sleep_v1_open_vitals_to_reference(&input) {
            Ok(expected)
...
fn algorithm_comparison_reports_equivalent(
    ...
    expected.schema == observed.schema
        ...
        && expected.open_vitals_output_ready == observed.open_vitals_output_ready
        ...
        && expected.open_vitals_algorithm_id == observed.open_vitals_algorithm_id
        && expected.open_vitals_algorithm_version == observed.open_vitals_algorithm_version
        ...
        && expected.open_vitals_quality_flags == observed.open_vitals_quality_flags
        ...
        && json_options_equivalent(&expected.open_vitals_output, &observed.open_vitals_output)
        ...
        && expected
            ...
            .all(|(left, right)| {
                left.field == right.field
                    && left.open_vitals_path == right.open_vitals_path
                    && left.reference_path == right.reference_path
                    && left.unit == right.unit
                    && approx_equal(left.open_vitals_value, right.open_vitals_value)
...
fn sleep_v1_evidence_folder_issue_action(issue: &str) -> String {
    if let Some(filename) = issue.strip_prefix("missing_required_file:") {
        ...
        "Regenerate the report with the current OpenVitals validator so the expected schema matches."
            .to_string()
    } else if issue.starts_with("generated_by_mismatch:") {
        "Regenerate the report with the expected OpenVitals validator instead of editing or substituting the artifact."
            .to_string()
    } else if issue.starts_with("supporting_schema_mismatch:") {
        "Regenerate or replace the supporting JSON with the expected OpenVitals schema.".to_string()
        ...
        || issue == "release_gate_report_benchmark_sample_below_threshold"
    {
        ...
        "Regenerate the release-gate report with the current OpenVitals validator.".to_string()
        ...
        "Regenerate historical-sync-validation.json with the current OpenVitals physical-sync validator."
            .to_string()
        ...
        "Regenerate sleep-window-validation.json with the current OpenVitals sleep-window label validator.".to_string()
        ...
        "Regenerate sleep-stage-validation.json with the current OpenVitals sleep-stage label validator."
            .to_string()
        ...
        || issue == "release_gate_report_stage_label_sample_below_threshold"
    {
        ...
        "Regenerate sleep-v1-benchmark.json with the current OpenVitals benchmark comparator."
            .to_string()
        ...
        "Regenerate the report with a OpenVitals validator that emits an explicit pass field."
            .to_string()
        ...
        "Regenerate the report with the current OpenVitals validator; passing Sleep V1 reports must carry an explicit empty issues list.".to_string()
        ...
        "Regenerate the report with the current OpenVitals validator; passing Sleep V1 reports must carry an explicit empty next_actions list.".to_string()
        ...
        "Regenerate the report with the current OpenVitals validator; passing Sleep V1 reports must carry an explicit empty errors list.".to_string()
        ...
        format!(
            "Regenerate the report with the current OpenVitals validator; passing Sleep V1 reports must carry an explicit empty {field} list."
        )
    } else {
...
pub fn validate_sleep_v1_release_gates(
    ...
    let mut report = SleepV1ReleaseGateReport {
        ...
        generated_by: "open-vitals-sleep-v1-release-gate-validator".to_string(),
        pass,
        physical_historical_sync_pass,
        timestamp_evidence_pass,
        sleep_window_label_pass,
...
fn historical_sync_physical_report_integrity_pass(
    ...
    report.schema == HISTORICAL_SYNC_PHYSICAL_VALIDATION_REPORT_SCHEMA
        && report.generated_by == "open-vitals-historical-sync-physical-validator"
...
fn sleep_window_label_report_integrity_pass(report: &SleepWindowLabelValidationReport) -> bool {
    ...
    report.schema == SLEEP_WINDOW_LABEL_VALIDATION_SCHEMA
        && report.generated_by == "open-vitals-sleep-window-label-validator"
        && report.pass == pass
...
fn sleep_stage_label_report_integrity_pass(report: &SleepStageLabelValidationReport) -> bool {
    ...
    let provenance_valid = sleep_stage_label_thresholds_at_least_default(report)
        ...
        && report
            ...
            .and_then(Value::as_str)
            == Some("official_or_platform_stage_values_are_labels_not_open_vitals_outputs")
        && report
            .provenance
            .get("report_integrity_policy")
            .and_then(Value::as_str)
    ...
    report.schema == SLEEP_STAGE_LABEL_VALIDATION_SCHEMA
        && report.generated_by == "open-vitals-sleep-stage-label-validator"
        && report.pass == pass
        && provenance_valid
        && comparison_rows_valid
        && report.quality_flags.is_empty()
...
fn sleep_feature_report_integrity_pass(report: &SleepFeatureScoreReport) -> bool {
    ...
    report.schema == SLEEP_FEATURE_SCORE_REPORT_SCHEMA
        && report.generated_by == "open-vitals-sleep-feature-score-builder"
        ...
        && score_result.algorithm_id == OPENVITALS_SLEEP_V0_ID
        && score_result.algorithm_version == OPENVITALS_SLEEP_V0_VERSION
        ...
        && score_output.algorithm_id == OPENVITALS_SLEEP_V0_ID
        && score_output.algorithm_version == OPENVITALS_SLEEP_V0_VERSION
...
fn sleep_motion_report_integrity_pass(report: &SleepFeatureScoreReport) -> bool {
    ...
    motion.schema == MOTION_FEATURE_REPORT_SCHEMA
        && motion.generated_by == "open-vitals-motion-feature-extractor"
...
fn sleep_heart_rate_report_integrity_pass(report: &SleepFeatureScoreReport) -> bool {
    ...
    heart_rate.schema == HEART_RATE_FEATURE_REPORT_SCHEMA
        && heart_rate.generated_by == "open-vitals-heart-rate-feature-extractor"
...
fn sleep_v1_benchmark_report_ready(report: &AlgorithmComparisonReport) -> bool {
    report.schema == ALGORITHM_COMPARISON_SCHEMA
        && report.generated_by == "open_vitals.algorithm_compare"
        && report.family == "sleep"
        && report.pass
        ...
        && report.open_vitals_algorithm_id == OPENVITALS_SLEEP_V1_ID
        && report.open_vitals_algorithm_version == OPENVITALS_SLEEP_V1_VERSION
        ...
        && report.open_vitals_output.is_some()
        && report.reference_output.is_some()
        && sleep_v1_benchmark_open_vitals_output_integrity_pass(report)
...
fn sleep_v1_benchmark_data_coverage_integrity_pass(report: &AlgorithmComparisonReport) -> bool {
    report.data_coverage.as_ref().is_some_and(|coverage| {
        coverage
            .get("open_vitals_output_data_coverage_fraction")
            .and_then(Value::as_f64)
            .is_some_and(|value| value.is_finite() && (0.0..=1.0).contains(&value))
    })
}
...
fn sleep_v1_benchmark_open_vitals_output_integrity_pass(
    ...
    output.get("algorithm_id").and_then(Value::as_str)
        ...
        && sleep_v1_benchmark_open_vitals_output_contract_pass(output)
...
fn sleep_v1_output_provenance_integrity_pass(output: &Value) -> bool {
    ...
    provenance.get("score_policy").and_then(Value::as_str)
        ...
        && provenance
            ...
            .and_then(Value::as_str)
            == Some("hand-derived-tests-and-versioned-open-vitals-output")
        && provenance
            .get("input_ids")
            .and_then(Value::as_array)
...
fn sleep_v1_benchmark_open_vitals_output_contract_pass(output: &Value) -> bool {
...
fn sleep_v1_status_report_contract_pass(output: &Value) -> bool {
    ...
    let Some(trusted_open_vitals_sleep_nights) = status_report
        .get("trusted_open_vitals_sleep_nights")
        .and_then(Value::as_u64)
    else {
        return false;
    };
    ...
    let Some(nights_until_open_vitals_training) = status_report
        .get("nights_until_open_vitals_training")
        .and_then(Value::as_u64)
    else {
        return false;
    };
    ...
    let counts_coherent = valid_sleep_nights
        == trusted_open_vitals_sleep_nights.saturating_add(imported_platform_sleep_nights)
    ...
    status == model_status
        ...
        && (!can_show_final_score || trusted_open_vitals_sleep_nights > 0)
...
fn sleep_v1_benchmark_delta_output_integrity_pass(report: &AlgorithmComparisonReport) -> bool {
    let (Some(open_vitals_output), Some(reference_output)) = (
        report.open_vitals_output.as_ref(),
    ...
    report
        ...
        .all(|delta| match delta.field.as_str() {
            ...
            "disturbance_count" => sleep_v1_benchmark_delta_open_vitals_input_values_match(
                delta,
                &report.provenance,
                "disturbance_count",
                reference_output,
...
fn sleep_v1_benchmark_delta_values_match(
    ...
    open_vitals_output
        .get(open_vitals_key)
        .and_then(Value::as_f64)
        .is_some_and(|value| approx_equal(value, delta.open_vitals_value))
        && reference_output
            .get(reference_key)
            .and_then(Value::as_f64)
...
fn sleep_v1_benchmark_delta_open_vitals_input_values_match(
    delta: &AlgorithmComparisonDelta,
    provenance: &Value,
    open_vitals_key: &str,
    reference_output: &Value,
    ...
    provenance
        .get("open_vitals_comparable_inputs")
        .and_then(Value::as_object)
        .and_then(|inputs| inputs.get(open_vitals_key))
        .and_then(Value::as_f64)
        .is_some_and(|value| approx_equal(value, delta.open_vitals_value))
        && reference_output
            .get(reference_key)
...
fn benchmark_delta_rows_integrity_pass(report: &AlgorithmComparisonReport) -> bool {
    ...
    report.comparable_fields == delta_fields
        && report.deltas.iter().all(|delta| {
            delta.open_vitals_value.is_finite()
                ...
                && approx_equal(
                    ...
                    delta.open_vitals_value - delta.reference_value,
                )
                && expected_relative_delta_matches(
                    delta.relative_delta_fraction,
                    delta.open_vitals_value,
...
fn expected_relative_delta_matches(
    ...
    if reference_value.abs() < f64::EPSILON {
        ...
        observed.is_some_and(|value| {
            approx_equal(
                value,
                (open_vitals_value - reference_value) / reference_value.abs(),
            )
        })
    }
}
...
fn sleep_v1_explanation_stability_report_integrity_pass(
    ...
    report.schema == SLEEP_V1_EXPLANATION_STABILITY_SCHEMA
        && report.generated_by == "open-vitals-sleep-v1-explanation-stability-validator"
...
pub fn validate_sleep_v1_explanation_and_stability(
    ...
    let first_result = open_vitals_sleep_v1(input);
    let second_result = open_vitals_sleep_v1(input);
    let v0_result = open_vitals_sleep_v0(&input.sleep);
    ...
    let perturbed_result = open_vitals_sleep_v1(&perturbed);
...
fn explanation_stability_report(
    ...
    SleepV1ExplanationStabilityReport {
        ...
        generated_by: "open-vitals-sleep-v1-explanation-stability-validator".to_string(),
        pass,
        explanation_pass,
        explanation_quality_pass,
        repeated_run_stability_pass,
...
fn sleep_v1_release_gate_issue_action(issue: &str) -> String {
    match issue {
        ...
        "sleep_v1_benchmark_report_integrity_failed" => {
            "Regenerate benchmark comparison reports with open_vitals.sleep.v1 output, valid reference output, shared deltas, and no errors before promotion.".to_string()
        }
...
pub fn validate_sleep_v1_stage_labels_for_store(
...
) -> OpenVitalsResult<SleepStageLabelValidationReport> {
    let sleep_id = input
        .sleep
        .input_ids
        .first()
    ...
    let start_unix_ms = parse_rfc3339_utc_unix_ms(&input.sleep.start_time).ok_or_else(|| {
        OpenVitalsError::message("sleep v1 stage label validation start_time is invalid")
    })?;
    let end_unix_ms = parse_rfc3339_utc_unix_ms(&input.sleep.end_time).ok_or_else(|| {
        OpenVitalsError::message("sleep v1 stage label validation end_time is invalid")
    })?;
    ...
    let result = open_vitals_sleep_v1(input);
    let labels = store
...
fn sleep_stage_label_comparison(
    ...
    options: &SleepStageLabelValidationOptions,
    issues: &mut Vec<String>,
) -> OpenVitalsResult<Option<SleepStageLabelComparison>> {
...
fn sleep_stage_label_validation_report(
    ...
    SleepStageLabelValidationReport {
        ...
        generated_by: "open-vitals-sleep-stage-label-validator".to_string(),
        pass,
        ...
        provenance: json!({
            ...
            "official_labels_policy": "official_or_platform_stage_values_are_labels_not_open_vitals_outputs",
...
fn sleep_stage_label_validation_action(reason: &str) -> String {
    match reason {
        ...
        "label_provenance_missing" => {
            "Mark stage labels as user-owned labels, not OpenVitals outputs, with review_policy=user_owned_sleep_stage_label or official_labels_are_labels=true.".to_string()
        }
        "label_provenance_source_missing" => {
...
) -> OpenVitalsResult<SleepWindowLabelValidationReport> {
...
fn sleep_window_label_comparison(
    ...
    options: &SleepWindowLabelValidationOptions,
    issues: &mut Vec<String>,
) -> OpenVitalsResult<Option<SleepWindowLabelComparison>> {
    let value_json = serde_json::from_str::<Value>(&label.value_json).map_err(|error| {
        OpenVitalsError::message(format!(
...
fn validation_report(
    ...
    SleepWindowLabelValidationReport {
        ...
        generated_by: "open-vitals-sleep-window-label-validator".to_string(),
        pass,
        start_time: start.to_string(),
        end_time: end.to_string(),
        label_count: labels.len(),
