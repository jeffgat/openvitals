...
pub fn check_storage_database(
    ...
    Ok(StorageCheckReport {
        schema: "open_vitals.storage-check-report.v1".to_string(),
        generated_by: "open-vitals-storage-check".to_string(),
...
fn check_table(store: &OpenVitalsStore, table: &str, required: &[&str]) -> StorageTableCheck {
    let mut issues = Vec::new();
...
fn run_storage_self_test(store: &OpenVitalsStore) -> StorageSelfTestReport {
    let mut issues = Vec::new();
    let raw = hex::decode(GET_HELLO_FRAME).unwrap_or_default();
    let parsed = match parse_frame_hex(DeviceType::OpenVitals, GET_HELLO_FRAME) {
        Ok(parsed) => parsed,
        Err(error) => {
    ...
    let raw_input = RawEvidenceInput {
        evidence_id: "open_vitals.storage-check.raw",
        source: "open_vitals.storage-check",
    ...
    let decoded_inserted = match store.insert_decoded_frame(DecodedFrameInput {
        frame_id: "open_vitals.storage-check.frame",
        evidence_id: "open_vitals.storage-check.raw",
        parsed: &parsed,
        parser_version: "open-vitals-storage-check",
    }) {
        Ok(inserted) => inserted,
        Err(error) => {
    ...
    let query_roundtrip = match store.raw_evidence("open_vitals.storage-check.raw") {
    ...
    let foreign_key_rejected = match store.insert_decoded_frame(DecodedFrameInput {
        frame_id: "open_vitals.storage-check.missing-evidence-frame",
        evidence_id: "open_vitals.storage-check.missing-evidence",
        parsed: &parsed,
        parser_version: "open-vitals-storage-check",
    }) {
        Ok(_) => {
    ...
    for issue in issues {
        if issue.starts_with("expected schema version") {
            actions.push(StorageCheckNextAction {
                ...
                action: "Back up the SQLite store, run OpenVitals migrations with the current Rust core, then rerun storage.check before capture/import/export writes.".to_string(),
            });
            ...
            actions.push(StorageCheckNextAction {
                ...
                action: "Reopen the database through OpenVitalsStore or enable PRAGMA foreign_keys before any write path uses the connection.".to_string(),
            });
            ...
            .map(|issue| {
                if issue == "missing table" {
                    StorageCheckNextAction {
                        ...
                        action: "Run OpenVitals migrations on this SQLite store before trusting app capture, metrics, export, debug, or health-sync paths.".to_string(),
                    }
