...
pub const PERF_BUDGET_REPORT_SCHEMA: &str = "open_vitals.perf-budget-report.v1";
...
pub fn run_perf_budget(options: PerfBudgetOptions) -> OpenVitalsResult<PerfBudgetReport> {
    if options.scale == 0 {
        return Err(OpenVitalsError::message("scale must be greater than 0"));
    }
...
pub fn perf_budget_report_from_workloads(
    ...
    let score_workload_ready = perf_workload_ready(&workloads, "open_vitals_score_batch");
    ...
    PerfBudgetReport {
        ...
        generated_by: "open-vitals-perf-budget".to_string(),
        scale,
        pass,
        input_valid,
...
fn parser_workload(scale: usize, budgets: &PerfBudgets) -> PerfWorkloadReport {
    ...
    for frame in &frames {
        ...
        checks += 1;
        match parse_frame(DeviceType::OpenVitals, frame) {
...
fn deframer_workload(scale: usize, budgets: &PerfBudgets) -> PerfWorkloadReport {
    ...
    let started = Instant::now();
    let mut accumulator = FrameAccumulator::new(DeviceType::OpenVitals);
...
fn algorithms_workload(scale: usize, budgets: &PerfBudgets) -> PerfWorkloadReport {
    ...
    for index in 0..scale {
        ...
        output_failures += open_vitals_hrv_v0(&hrv_input).output.is_none() as usize;
        checks += 1;
...
fn export_workload(scale: usize, budgets: &PerfBudgets) -> OpenVitalsResult<PerfWorkloadReport> {
    let workspace = PerfWorkspace::new()?;
    let db_path = workspace.path.join("open_vitals.sqlite");
    let output_dir = workspace.path.join("export.openvitalsbundle");
    let zip_path = workspace.path.join("export.openvitalsbundle.zip");
    ...
    let frames = (0..scale)
        .map(|index| CapturedFrameInput {
            ...
            device_model: "WHOOP 5.0 OpenVitals synthetic".to_string(),
    ...
    let import_report = import_captured_frame_batch(
        ...
        CapturedFrameBatchOptions {
            parser_version: "open-vitals-core/perf-budget",
        },
    )?;
    ...
    let export_report = export_raw_timeframe(
        ...
        RawExportOptions {
            ...
            app_version: "open-vitals-app/perf-budget",
            core_version: "open-vitals-core/perf-budget",
...
fn perf_budget_action(workload_name: &str, reason: &str) -> &'static str {
    match reason {
        "duration_budget_exceeded" => match workload_name {
            ...
            "open_vitals_score_batch" => {
                "Profile OpenVitals score calculations at this scale, then cache shared feature work or simplify repeated math before raising the mobile budget."
            }
            "raw_export_bundle" => {
        ...
        "memory_budget_exceeded" => match workload_name {
            ...
            "open_vitals_score_batch" => {
                "Reduce temporary vectors in OpenVitals score inputs/outputs or process windows incrementally before raising the mobile memory budget."
            }
            "raw_export_bundle" => {
        ...
        "algorithm_output_failure" => {
            "Fix OpenVitals score input generation or quality gates so generated valid inputs produce score outputs before benchmarking."
        }
        "raw_export_row_mismatch" => {
...
fn directory_size(path: &Path) -> OpenVitalsResult<u64> {
    let mut total = 0u64;
    for entry in fs::read_dir(path).map_err(|source| OpenVitalsError::io(path, source))? {
        let entry = entry.map_err(|source| OpenVitalsError::io(path, source))?;
        let entry_path = entry.path();
        let metadata = entry
            .metadata()
            .map_err(|source| OpenVitalsError::io(&entry_path, source))?;
        if metadata.is_dir() {
...
fn file_size(path: &Path) -> OpenVitalsResult<u64> {
    Ok(fs::metadata(path)
