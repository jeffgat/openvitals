...
extension OpenVitalsBLEClient {
  ...
  func resetConnection() {
    ...
    if isScanning {
      stopScan(reason: "connection_reset")
    }
    if isHistoricalSyncing {
      failHistoricalSync("Connection reset by user.")
    }
    if pendingAlarmCommand != nil {
      failAlarmCommand("Connection reset by user.")
    }
    if pendingClockCommand != nil {
      failClockCommand("Connection reset by user.")
    }
    if !pendingDebugCommands.isEmpty {
      failAllDebugCommands("Connection reset by user.")
    }
    ...
    if let activePeripheral {
      userDisconnectRequestedIDs.insert(activePeripheral.identifier)
  ...
  func sendDebugResearchCommand(
    ...
    guard let activePeripheral, let commandCharacteristic else {
      setDebugCommandStatus("\(definition.title) needs active device command characteristic")
      record(level: .warn, source: "ble.debug_command", title: "command.blocked", body: debugCommandStatus)
      return false
    }
  ...
  func startScan(reason: String, clearDiscovered: Bool) {
    ...
    if clearDiscovered {
      ...
      fullServiceDiscoveryRequestedIDs.removeAll()
      selectedDeviceID = nil
    }
    isScanning = true
    let services = reason == "manual" ? nil : whoopServices
    ...
    record(
      source: "ble",
      title: "scan.started",
      body: "reason=\(reason) services=\(services.map(uuidList) ?? "all") identityFilter=\(uuidList(whoopServices))"
    )
  }
  ...
  func select(_ device: OpenVitalsDiscoveredDevice) {
  ...
  func sendClientHello(reason: String, force: Bool) {
    ...
    guard
      ...
      !OpenVitalsHello.clientHelloFrame.isEmpty
    else {
    ...
    activePeripheral.writeValue(
      OpenVitalsHello.clientHelloFrame,
    ...
    emitCommandWrite(
      ...
      sequence: nil,
      payload: Data(),
      frame: OpenVitalsHello.clientHelloFrame,
      peripheral: activePeripheral,
      characteristic: commandCharacteristic,
      writeType: writeType
    )
    ...
    record(
      source: "ble",
      title: "hello.sent",
      body: "reason=\(reason) \(commandCharacteristic.uuid.uuidString) \(writeTypeName(writeType)) \(OpenVitalsHello.clientHelloFrameHex)"
    )
  }
