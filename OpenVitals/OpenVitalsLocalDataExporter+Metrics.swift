...
extension OpenVitalsLocalDataExporter {
  ...
  static func applyRawNotificationMetrics(from url: URL, to metrics: inout OpenVitalsOvernightExportMetrics) {
  ...
  static func applyHistoricalRangeMetrics(from url: URL, to metrics: inout OpenVitalsOvernightExportMetrics) {
    if enumerateJSONLines(at: url, body: { object in
      guard let object else {
  ...
  static func applyCommandWriteMetrics(from url: URL, to metrics: inout OpenVitalsOvernightExportMetrics) {
    if enumerateJSONLines(at: url, body: { object in
      guard let object else {
  ...
  static func applyEventLogMetrics(from url: URL, to metrics: inout OpenVitalsOvernightExportMetrics) {
    if enumerateJSONLines(at: url, body: { object in
  ...
  static func applySQLiteMirrorMetrics(
    databasePath: String,
    sessionID: String,
    to metrics: inout OpenVitalsOvernightExportMetrics,
    issues: inout [String]
  ) {
    do {
  ...
  static func validateCrashMarker(
    ...
    guard let data = try? Data(contentsOf: url),
          ...
          marker["schema"] as? String == "open_vitals.overnight.crash_marker.v1" else {
