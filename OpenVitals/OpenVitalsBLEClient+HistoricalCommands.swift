...
extension OpenVitalsBLEClient {
  func beginHistoricalSync(
    ...
    guard activePeripheral != nil, commandCharacteristic != nil else {
      failHistoricalSync("Historical sync needs an active device command characteristic. Current connection state: \(connectionState).")
      return
    }
    ...
    guard !commandWriteAuthenticationRequired else {
      failHistoricalSync(commandWriteAuthenticationStatus)
      return
    }
    guard supportsV5HistoricalSync else {
      let characteristic = commandCharacteristic?.uuid.uuidString ?? "missing"
      failHistoricalSync("Historical sync currently supports the V5 fd4b command characteristic. Active command characteristic: \(characteristic).")
      return
    }
  ...
  func debugCommandPayload(
    for definition: OpenVitalsDebugCommandDefinition,
    payloadHex: String?
  ) -> [UInt8]? {
