...
extension OpenVitalsLocalDataExporter {
  static func validate(
    ...
    let exportSelfIncluded = exportedRelativePaths.contains { path in
      path == "Documents/OpenVitals/Exports" || path.hasPrefix("Documents/OpenVitals/Exports/")
    }
    ...
    let sqliteDatabaseIncluded = pathSet.contains("Application Support/OpenVitals/open_vitals.sqlite")
    ...
    let bleLiveLogRelativePath = "Documents/OpenVitals/open-vitals-ble-live.log"
    ...
    var metrics = OpenVitalsOvernightExportMetrics()
    var issues: [String] = []
    ...
    if let requiredOvernightSessionID {
      let base = "Documents/OpenVitals/OvernightGuard/\(requiredOvernightSessionID)/"
    ...
    if exportSelfIncluded {
      issues.append("export contains files from Documents/OpenVitals/Exports")
    }
    ...
    return OpenVitalsLocalDataExportValidation(
