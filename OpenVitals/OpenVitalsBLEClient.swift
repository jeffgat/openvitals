...
final class OpenVitalsBLEClient: NSObject, ObservableObject, @unchecked Sendable {
  ...
  @Published var discoveredDevices: [OpenVitalsDiscoveredDevice] = []
  ...
  @Published var activeDeviceName = "Device"
  @Published var activeDeviceIdentifier: UUID?
  @Published var selectedDeviceID: UUID?
  @Published var scanDiagnostics: [OpenVitalsScanDiagnostic] = []
  ...
  @Published var commandWriteAuthenticationStatus = "No command authentication failures"
  ...
  @Published var syncToast: OpenVitalsSyncToast?
  @Published var lastSyncFailure: OpenVitalsSyncFailure?
  @Published var syncFailureSheet: OpenVitalsSyncFailure?
  @Published var debugCommandStatus = "No debug command sent"
  @Published var debugCommandResponses: [OpenVitalsDebugCommandResponse] = []
  @Published var debugCommandSnapshotPath = "No debug command snapshot"

  var onNotification: ((OpenVitalsNotificationEvent) -> Void)?
  var onRawNotification: ((OpenVitalsNotificationEvent) -> Void)?
  var onRawNotificationWithContext: ((OpenVitalsNotificationEvent, OpenVitalsBLENotificationContext) -> Void)?
  var onCommandWrite: ((OpenVitalsCommandWriteEvent) -> Void)?
  ...
  let logger = Logger(subsystem: "com.open_vitals.swift", category: "ble")
  ...
  let messageStore = OpenVitalsMessageStore(
    ...
    flushInterval: OpenVitalsBLEClient.displayedMessageFlushInterval
  )
  let notificationContextLock = NSLock()
  var notificationContextActiveDeviceName = "Device"
  ...
  let autoStartPhysiologyCaptureOnReady: Bool = {
    ...
    return processInfo.arguments.contains("--open-vitals-start-physiology-capture")
      || processInfo.environment["OPENVITALS_START_PHYSIOLOGY_CAPTURE"] == "1"
  }()
  let autoHistoricalSyncOnReady: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--open-vitals-auto-historical-sync")
      || processInfo.environment["OPENVITALS_AUTO_HISTORICAL_SYNC"] == "1"
  }()
  let diagnosticLoggingEnabled: Bool = {
    let processInfo = ProcessInfo.processInfo
    if processInfo.arguments.contains("--open-vitals-disable-diagnostics")
      || processInfo.environment["OPENVITALS_DISABLE_DIAGNOSTICS"] == "1"
      || processInfo.environment["OPENVITALS_DIAGNOSTIC_LOGGING"] == "0" {
      return false
    }
    return processInfo.arguments.contains("--open-vitals-enable-diagnostics")
      || processInfo.environment["OPENVITALS_ENABLE_DIAGNOSTICS"] == "1"
      || processInfo.environment["OPENVITALS_DIAGNOSTIC_LOGGING"] == "1"
  }()
  let prioritizeLiveCaptureOnReady: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--open-vitals-start-physiology-capture")
      || processInfo.arguments.contains("--open-vitals-start-health-packet-capture")
      || processInfo.arguments.contains("--open-vitals-start-temperature-packet-capture")
      || processInfo.environment["OPENVITALS_START_PHYSIOLOGY_CAPTURE"] == "1"
      || processInfo.environment["OPENVITALS_START_HEALTH_PACKET_CAPTURE"] == "1"
      || processInfo.environment["OPENVITALS_START_TEMPERATURE_PACKET_CAPTURE"] == "1"
  }()
  let autoSendDebugSkinTemperatureCommand: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--open-vitals-send-debug-skin-temp-command")
      || processInfo.environment["OPENVITALS_SEND_DEBUG_SKIN_TEMP_COMMAND"] == "1"
  }()
  let forceDebugMenuWrite: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--open-vitals-force-debug-menu-write")
      || processInfo.environment["OPENVITALS_FORCE_DEBUG_MENU_WRITE"] == "1"
  }()
  let consoleCaptureStatusEnabled: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--open-vitals-console-capture-status")
      || processInfo.environment["OPENVITALS_CONSOLE_CAPTURE_STATUS"] == "1"
  }()
  let debugSkinTemperatureCommandPayload: Data = {
    let processInfo = ProcessInfo.processInfo
    if let hex = processInfo.environment["OPENVITALS_DEBUG_MENU_COMMAND_HEX"],
       let data = Data(hexString: hex),
       !data.isEmpty {
      return data
    }
    if let text = processInfo.environment["OPENVITALS_DEBUG_MENU_COMMAND"],
  ...
  let diagnosticLogURL: URL? = {
    let processInfo = ProcessInfo.processInfo
    let loggingEnabled = processInfo.arguments.contains("--open-vitals-enable-diagnostics")
      || processInfo.environment["OPENVITALS_ENABLE_DIAGNOSTICS"] == "1"
      || processInfo.environment["OPENVITALS_DIAGNOSTIC_LOGGING"] == "1"
    let loggingDisabled = processInfo.arguments.contains("--open-vitals-disable-diagnostics")
      || processInfo.environment["OPENVITALS_DISABLE_DIAGNOSTICS"] == "1"
      || processInfo.environment["OPENVITALS_DIAGNOSTIC_LOGGING"] == "0"
    ...
    guard let directory = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
      OpenVitalsBLEClient.recordDiagnosticLogSetupWarning("open-vitals-ble.log setup failed: Application Support directory unavailable")
      return nil
    }
    let openVitalsDirectory = directory.appendingPathComponent("OpenVitals", isDirectory: true)
    let url = openVitalsDirectory.appendingPathComponent("open-vitals-ble.log")
    do {
      try OpenVitalsBLEClient.prepareDiagnosticLogDirectory(openVitalsDirectory)
      if FileManager.default.fileExists(atPath: url.path) {
        try FileManager.default.removeItem(at: url)
      }
    } catch {
      OpenVitalsBLEClient.recordDiagnosticLogSetupWarning("open-vitals-ble.log setup failed: \(String(describing: error))")
      return nil
    }
    return url
  }()
  let diagnosticLogMirrorURL: URL? = {
    let processInfo = ProcessInfo.processInfo
    let mirrorEnabled = processInfo.arguments.contains("--open-vitals-afc-diagnostic-mirror")
      || processInfo.environment["OPENVITALS_AFC_DIAGNOSTIC_MIRROR"] == "1"
    guard mirrorEnabled else {
      return nil
    }
    guard let directory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
      OpenVitalsBLEClient.recordDiagnosticLogSetupWarning("open-vitals-ble-live.log mirror setup failed: Documents directory unavailable")
      return nil
    }
    let openVitalsDirectory = directory.appendingPathComponent("OpenVitals", isDirectory: true)
    let url = openVitalsDirectory.appendingPathComponent("open-vitals-ble-live.log")
    do {
      try OpenVitalsBLEClient.prepareDiagnosticLogFile(at: url, directory: openVitalsDirectory)
    } catch {
      OpenVitalsBLEClient.recordDiagnosticLogSetupWarning("open-vitals-ble-live.log mirror setup failed: \(String(describing: error))")
      return nil
    }
    return url
  }()
  let overnightSideChannelLogURL: URL? = {
    guard let directory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
      OpenVitalsBLEClient.recordDiagnosticLogSetupWarning("open-vitals-ble-live.log setup failed: Documents directory unavailable")
      return nil
    }
    let openVitalsDirectory = directory.appendingPathComponent("OpenVitals", isDirectory: true)
    let url = openVitalsDirectory.appendingPathComponent("open-vitals-ble-live.log")
    do {
      ...
      OpenVitalsBLEClient.recordDiagnosticLogSetupWarning("open-vitals-ble-live.log setup failed: \(String(describing: error))")
      return nil
    }
  }()
  ...
  var unsupportedAccessoryIDs = Set<UUID>()
  var fullServiceDiscoveryRequestedIDs = Set<UUID>()
  var activePeripheral: CBPeripheral?
  var messages: [OpenVitalsMessage] {
  ...
  enum DefaultsKey {
    static let rememberedDeviceID = "open_vitals.swift.rememberedDeviceID"
    static let rememberedDeviceName = "open_vitals.swift.rememberedDeviceName"
    static let rememberedDeviceValidated = "open_vitals.swift.rememberedDeviceValidatedWhoop"
    static let lastBatteryPercent = "open_vitals.swift.lastBatteryPercent"
    static let lastBatteryCapturedAt = "open_vitals.swift.lastBatteryCapturedAt"
    static let inferredBatteryChargingUntil = "open_vitals.swift.inferredBatteryChargingUntil"
    static let restingHeartRateEstimateBPM = "open_vitals.swift.restingHeartRateEstimateBPM"
    static let restingHeartRateEstimateSampleCount = "open_vitals.swift.restingHeartRateEstimateSampleCount"
    static let restingHeartRateEstimateUpdatedAt = "open_vitals.swift.restingHeartRateEstimateUpdatedAt"
    static let restingHeartRateEstimateSource = "open_vitals.swift.restingHeartRateEstimateSource"
    static let liveHRVRMSSD = "open_vitals.swift.liveHRVRMSSD"
    static let liveHRVRRIntervalCount = "open_vitals.swift.liveHRVRRIntervalCount"
  ...
  static let restorationIdentifier = "com.open_vitals.swift.central"
  ...
  enum ClockCommandKind {
    ...
    var payload: [UInt8] {
      ...
      case .get:
        return []
      case .set(let date):
        let timestamp = OpenVitalsBLEClient.clockTimestampParts(for: date)
        var bytes: [UInt8] = []
        OpenVitalsBLEClient.appendUInt32LE(timestamp.seconds, to: &bytes)
        OpenVitalsBLEClient.appendUInt32LE(timestamp.subseconds, to: &bytes)
        return bytes
      }
    }
  ...
  struct AlarmHapticsPattern {
    ...
    var payloadBytes: [UInt8] {
      ...
      OpenVitalsBLEClient.appendUInt16LE(loopControl, to: &bytes)
  ...
  enum AlarmCommandKind {
    ...
    var payload: [UInt8] {
      ...
      case .set(let alarmID, let date, let pattern):
        ...
        let timestamp = OpenVitalsBLEClient.alarmTimestampParts(for: date)
        OpenVitalsBLEClient.appendUInt32LE(timestamp.seconds, to: &bytes)
        OpenVitalsBLEClient.appendUInt16LE(timestamp.subseconds, to: &bytes)
  ...
  static let debugResearchCommandDefinitions: [OpenVitalsDebugCommandDefinition] = [
    OpenVitalsDebugCommandDefinition(
      id: "get_body_location_and_status",
      title: "Body Location And Status",
      commandNumber: 84,
      family: "research",
      risk: "read",
      detail: "Device status command; likely body placement and band state.",
      defaultPayloadHex: "",
  ...
  var canSendHello: Bool {
    activePeripheral != nil
      ...
      && !commandWriteAuthenticationRequired
  }
  ...
  var debugResearchCommands: [OpenVitalsDebugCommandDefinition] {
  ...
  var alarmWriteSupportSummary: String {
    ...
    if !canSendHello {
      return "Device connection is not ready"
    }
    return "Ready to write alarm to band"
  }
  ...
  var canResetConnection: Bool {
    activePeripheral != nil || isScanning || connectionState != "disconnected"
  }
