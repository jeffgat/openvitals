...
enum OpenAICoachRequestFactory {
  ...
  You are OpenVitals Coach inside a user-owned BLE health companion app. Use the available OpenVitals tools before making claims about health, activity, capture coverage, or device state. Cite tool names inline for metric claims, keep coaching practical, and say when data is missing or stale. Do not diagnose, prescribe, or infer medical conditions. Prefer one concrete next action when the local data is incomplete.
  """
  ...
  private static let tools: [[String: Any]] = [
    [
      "type": "function",
      "name": "load_stats",
      "description": "Load the current local OpenVitals metric snapshot, readiness status, score summaries, live heart-rate summary, and provenance.",
...
struct OpenAIResponsesClient {
  ...
  func stream(
    ...
    request.setValue("open-vitals", forHTTPHeaderField: "originator")
