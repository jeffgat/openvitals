...
@MainActor
final class OpenVitalsAppModel: ObservableObject {
  ...
  @Published var overnightGuardReadinessSummary = "Not sleep-ready | connect device and start Overnight Guard"
  ...
  let captureFrameRowBuildQueue = DispatchQueue(label: "com.open_vitals.swift.capture-frame-row-build", qos: .utility)
  let rustStartupQueue = DispatchQueue(label: "com.open_vitals.swift.rust-startup", qos: .utility)
  let activityTimelineRefreshQueue = DispatchQueue(label: "com.open_vitals.swift.activity-timeline-refresh", qos: .utility)
  let captureStatusSnapshotWriteQueue = DispatchQueue(label: "com.open_vitals.swift.capture-status-snapshot", qos: .utility)
  let heartRateSamplePipeline = HeartRateSamplePipeline(
    timelinePublishInterval: OpenVitalsAppModel.heartRateHourlyRangePublishInterval
  )
  let packetUIStateAggregator = PacketUIStateAggregator(
    publishInterval: OpenVitalsAppModel.packetUIStatePublishInterval,
    maximumPendingDeviceSignalPoints: OpenVitalsAppModel.maxRecentDeviceSignalPoints
  )
  ...
  let healthPacketCaptureFamilyAggregator = HealthPacketCaptureFamilyAggregator(
    publishInterval: OpenVitalsAppModel.healthPacketCaptureUIUpdateInterval
  )
  ...
  let autoStartTemperaturePacketCaptureOnReady: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--open-vitals-start-temperature-packet-capture")
      || processInfo.environment["OPENVITALS_START_TEMPERATURE_PACKET_CAPTURE"] == "1"
  }()
  let autoStartPhysiologyPacketCaptureOnReady: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--open-vitals-start-physiology-packet-capture")
      || processInfo.environment["OPENVITALS_START_PHYSIOLOGY_PACKET_CAPTURE"] == "1"
  }()
  let autoStartRespiratoryPacketWatchOnReady: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--open-vitals-start-respiratory-packet-watch")
      || processInfo.environment["OPENVITALS_START_RESPIRATORY_PACKET_WATCH"] == "1"
  }()
  let autoStartHealthPacketCaptureDuration: TimeInterval = {
    let processInfo = ProcessInfo.processInfo
    if let value = processInfo.environment["OPENVITALS_HEALTH_PACKET_CAPTURE_DURATION_SECONDS"],
       let seconds = Double(value),
       seconds > 0 {
      return seconds
    }
    let prefix = "--open-vitals-health-packet-capture-duration="
  ...
  let autoStartTemperaturePacketCaptureDuration: TimeInterval = {
    let processInfo = ProcessInfo.processInfo
    if let value = processInfo.environment["OPENVITALS_TEMPERATURE_PACKET_CAPTURE_DURATION_SECONDS"],
       let seconds = Double(value),
       seconds > 0 {
      return seconds
    }
    let prefix = "--open-vitals-temperature-packet-capture-duration="
    if let argument = processInfo.arguments.first(where: { $0.hasPrefix(prefix) }),
       let seconds = Double(argument.dropFirst(prefix.count)),
       seconds > 0 {
      return seconds
  ...
  let autoStartPhysiologyPacketCaptureDuration: TimeInterval = {
    let processInfo = ProcessInfo.processInfo
    if let value = processInfo.environment["OPENVITALS_PHYSIOLOGY_PACKET_CAPTURE_DURATION_SECONDS"],
       let seconds = Double(value),
       seconds > 0 {
      return seconds
    }
    let prefix = "--open-vitals-physiology-packet-capture-duration="
    if let argument = processInfo.arguments.first(where: { $0.hasPrefix(prefix) }),
       let seconds = Double(argument.dropFirst(prefix.count)),
       seconds > 0 {
      return seconds
  ...
  let autoStartRespiratoryPacketWatchDuration: TimeInterval = {
    let processInfo = ProcessInfo.processInfo
    if let value = processInfo.environment["OPENVITALS_RESPIRATORY_PACKET_WATCH_DURATION_SECONDS"],
       let seconds = Double(value),
       seconds > 0 {
      return seconds
    }
    let prefix = "--open-vitals-respiratory-packet-watch-duration="
    if let argument = processInfo.arguments.first(where: { $0.hasPrefix(prefix) }),
       let seconds = Double(argument.dropFirst(prefix.count)),
       seconds > 0 {
      return seconds
  ...
  let autoSyncHistoryDuringPhysiologyCapture: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--open-vitals-sync-history-during-physiology-capture")
      || processInfo.environment["OPENVITALS_SYNC_HISTORY_DURING_PHYSIOLOGY_CAPTURE"] == "1"
  }()
  let captureStatusSnapshotURL: URL? = {
    let processInfo = ProcessInfo.processInfo
    let enabled = processInfo.arguments.contains("--open-vitals-afc-capture-status")
      || processInfo.environment["OPENVITALS_AFC_CAPTURE_STATUS"] == "1"
    guard enabled else {
      return nil
    }
    ...
    try? FileManager.default.createDirectory(at: openVitalsDirectory, withIntermediateDirectories: true)
    return openVitalsDirectory.appendingPathComponent("capture-status.txt")
  }()
  ...
  init(startBLE: Bool = true) {
    ...
    ble.onCommandWrite = { [weak self, weak ble] event in
      self?.persistOvernightCommandWrite(
        event,
        activeDeviceName: ble?.activeDeviceName ?? "Device",
