...
struct SleepDataBridgeSection: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      VStack(spacing: 8) {
        HealthInfoRow(row: HealthSummaryRow("Band history", value: "\(ble.historicalSyncStatus) | \(packetText)", source: .live("band historical sync"), systemImage: "antenna.radiowaves.left.and.right"))
        ...
        HealthInfoRow(row: HealthSummaryRow("OpenVitals sleep score", value: store.sleepFeatureScoreSummary(), source: store.packetScoreSource("metrics.sleep_score_from_features"), systemImage: "bed.double"))
      }
      HStack(spacing: 10) {
        Button {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      VStack(spacing: 8) {
        ...
        HealthInfoRow(row: HealthSummaryRow("Last response", value: ble.lastAlarmResponseSummary, source: .bridge("band command response"), systemImage: "checkmark.seal"))
        HealthInfoRow(row: HealthSummaryRow("Last event", value: ble.lastAlarmEventSummary, source: .bridge("band event stream"), systemImage: "waveform.path.ecg"))
        ...
        if !ble.lastAlarmResponsePayloadHex.isEmpty {
          HealthInfoRow(row: HealthSummaryRow("Response hex", value: String(ble.lastAlarmResponsePayloadHex.prefix(38)), source: .bridge("band command response"), systemImage: "number"))
        }
        if !ble.lastAlarmEventPayloadHex.isEmpty {
          HealthInfoRow(row: HealthSummaryRow("Event hex", value: String(ble.lastAlarmEventPayloadHex.prefix(38)), source: .bridge("band event stream"), systemImage: "number"))
        }
      }
      ...
      Button(role: .destructive) {
        ...
        Label("Disable Band Alarms", systemImage: "bell.slash")
          .frame(maxWidth: .infinity)
      }
      .buttonStyle(.bordered)
      .disabled(!ble.canWriteAlarm)
