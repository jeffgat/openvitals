...
extension OpenVitalsBLEClient {
  ...
  func handleAlarmEvent(_ payload: [UInt8]) {
    ...
    case 57:
      alarmCommandStatus = "Band alarm executed"
    ...
    case 58:
      alarmCommandStatus = "App-driven band alarm executed"
  ...
  func handleAlarmSetEvent(_ body: [UInt8]) {
    guard body.count >= 8 else {
      alarmCommandStatus = "Band alarm set event received"
  ...
  func failHistoricalSync(_ message: String) {
    ...
    let failure = OpenVitalsSyncFailure(title: "Sync Failed", message: message, occurredAt: Date())
  ...
  func notifyHistoricalSyncProgress(status: String, detail: String, terminal: Bool, failed: Bool) {
    ...
    onHistoricalSyncProgress?(
      OpenVitalsHistoricalSyncProgress(
        status: status,
        detail: detail,
  ...
  func publishSyncToast(
    phase: OpenVitalsSyncToastPhase,
    ...
    syncToast = OpenVitalsSyncToast(phase: phase, title: titleOverride ?? title, detail: detail)
