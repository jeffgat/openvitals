...
extension OpenVitalsAppModel {
  ...
  private func startOvernightGuard(decodedCaptureDuration: TimeInterval?) {
    ...
    guard ble.connectionState == "ready" else {
      overnightGuardStatus = "Connect device first. Current state: \(ble.connectionState)"
  ...
  nonisolated func persistOvernightRawNotificationBeforeInterpretation(
    _ event: OpenVitalsNotificationEvent,
  ...
  nonisolated func persistOvernightHistoricalRangeTelemetry(_ telemetry: OpenVitalsHistoricalRangeTelemetry) {
    guard overnightRawSpool.isActive else {
      return
    }
  ...
  nonisolated func persistOvernightCommandWrite(
    _ event: OpenVitalsCommandWriteEvent,
    activeDeviceName: String,
    connectionState: String
  ) {
    guard overnightRawSpool.isActive else {
  ...
  nonisolated func persistOvernightEventLog(_ message: OpenVitalsMessage) {
    guard overnightRawSpool.isActive else {
      return
    }
  ...
  func applyOvernightRawNotificationSnapshot(_ snapshot: OvernightRawSpoolSnapshot, event: OpenVitalsNotificationEvent) {
    guard overnightGuardActive else {
      return
    }
  ...
  func applyOvernightHistoricalRangeTelemetrySnapshot(
    ...
    telemetry: OpenVitalsHistoricalRangeTelemetry
  ) {
    guard overnightGuardActive else {
      return
    }
  ...
  func applyOvernightCommandWriteSnapshot(_ snapshot: OvernightRawSpoolSnapshot, event: OpenVitalsCommandWriteEvent) {
    guard overnightGuardActive else {
      return
    }
  ...
  func handleOvernightHistoricalSyncProgress(_ progress: OpenVitalsHistoricalSyncProgress) {
    guard overnightGuardActive else {
      return
    }
  ...
  func scheduleOvernightGuardFinalSyncDrain(reason: String, progress: OpenVitalsHistoricalSyncProgress) {
    guard overnightGuardActive else {
      return
    }
  ...
  func exportOvernightGuardBundle(sessionID: String?, reason: String) {
    ...
    DispatchQueue.global(qos: .userInitiated).async { [weak self] in
      do {
        ...
        let result = try OpenVitalsLocalDataExporter.createBundle(requiredOvernightSessionID: sessionID)
        DispatchQueue.main.async { [weak self] in
          ...
          let recordLevel: OpenVitalsLogLevel
