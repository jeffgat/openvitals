...
extension OpenVitalsBLEClient {
  ...
  func writeOSLog(_ message: OpenVitalsMessage) {
  ...
  func notificationContextSnapshot() -> OpenVitalsBLENotificationContext {
    notificationContextLock.lock()
    let snapshot = OpenVitalsBLENotificationContext(
  ...
  func writeClockCommand(_ kind: ClockCommandKind, syncIfNeeded: Bool) {
    ...
    guard let activePeripheral, let commandCharacteristic else {
      failClockCommand("Clock command needs an active device command characteristic.")
      return
    }
  ...
  func writeAlarmCommand(_ kind: AlarmCommandKind) {
    ...
    guard let activePeripheral, let commandCharacteristic else {
      alarmCommandStatus = "Alarm write needs an active device command characteristic"
      record(level: .warn, source: "ble.alarm", title: "alarm.write.blocked", body: alarmCommandStatus)
      return
    }
    guard connectionState == "ready" else {
  ...
  func writeSensorStreamCommands(
    ...
    guard let activePeripheral, let commandCharacteristic else {
      if updatePhysiologyStatus {
        physiologyCaptureStatus = "Needs an active device command characteristic"
      }
      record(level: .warn, source: "ble.sensor", title: "sensor.write.blocked", body: "Needs an active device command characteristic")
      return
    }
    guard connectionState == "ready" else {
      if updatePhysiologyStatus {
    ...
    guard !commandWriteAuthenticationRequired else {
      if updatePhysiologyStatus {
        physiologyCaptureStatus = "Authentication required for command writes"
      }
      record(level: .warn, source: "ble.sensor", title: "sensor.write.blocked", body: commandWriteAuthenticationStatus)
      return
    }
    guard let writeType = writeType(for: commandCharacteristic) else {
      if updatePhysiologyStatus {
    ...
    for (index, command) in commands.enumerated() {
      ...
      DispatchQueue.main.asyncAfter(deadline: .now() + delay) { [weak self, weak activePeripheral, weak commandCharacteristic] in
        ...
        guard self.connectionState == "ready",
              !self.commandWriteAuthenticationRequired else {
          self.record(level: .warn, source: "ble.sensor", title: "sensor.write.skipped", body: self.commandWriteAuthenticationStatus)
          return
        }
  ...
  func loadRememberedDevice() {
    let migratedLegacyDevice = migrateLegacyRememberedDeviceIfNeeded()
    ...
    if let rememberedDeviceID, rememberedDeviceLooksLikeWhoop {
      whoopCandidateIDs.insert(rememberedDeviceID)
    }
    updateRememberedDeviceDescription()
    if migratedLegacyDevice {
      record(source: "ble", title: "remembered_device.migrated", body: rememberedDeviceDescription)
    }
  }

  func migrateLegacyRememberedDeviceIfNeeded() -> Bool {
    var migrated = false
    if defaults.string(forKey: DefaultsKey.rememberedDeviceID) == nil,
       let legacyID = defaults.string(forKey: LegacyDefaultsKey.rememberedDeviceID) {
      defaults.set(legacyID, forKey: DefaultsKey.rememberedDeviceID)
      migrated = true
    }
    if defaults.string(forKey: DefaultsKey.rememberedDeviceName) == nil,
       let legacyName = defaults.string(forKey: LegacyDefaultsKey.rememberedDeviceName) {
      defaults.set(legacyName, forKey: DefaultsKey.rememberedDeviceName)
      migrated = true
    }
    if defaults.object(forKey: DefaultsKey.rememberedDeviceValidated) == nil,
       defaults.object(forKey: LegacyDefaultsKey.rememberedDeviceValidated) != nil {
      defaults.set(
        defaults.bool(forKey: LegacyDefaultsKey.rememberedDeviceValidated),
        forKey: DefaultsKey.rememberedDeviceValidated
      )
      migrated = true
    }
    return migrated
  ...
  func rememberPeripheral(_ peripheral: CBPeripheral, fallbackName: String? = nil, evidence: String? = nil) {
    ...
    let name = Self.sanitizedWhoopDisplayName(peripheral.name ?? fallbackName ?? rememberedDeviceName ?? "Device")
  ...
  func connect(_ peripheral: CBPeripheral, reason: String) {
    ...
    record(source: "ble", title: "connect.started", body: "reason=\(reason) evidence=\(evidence) \(peripheral.name ?? fallbackName ?? rememberedDeviceName ?? "Device") \(peripheral.identifier.uuidString)")
    pendingConnectionReason = reason
    scheduleConnectionTimeout(for: peripheral, reason: reason)
    central.connect(
      peripheral,
      options: [
  ...
  func scheduleConnectionTimeout(for peripheral: CBPeripheral, reason: String) {
    connectTimeoutWorkItem?.cancel()
    let peripheralID = peripheral.identifier
    let workItem = DispatchWorkItem { [weak self] in
      guard let self,
            self.activePeripheral?.identifier == peripheralID,
            self.connectionState == "connecting" else {
        return
      }
      self.pendingConnectionReason = nil
      self.autoReconnectInFlight = false
      self.updateConnectionState("connect timed out")
      self.updateReconnectState("connect timed out")
      self.record(
        level: .warn,
        source: "ble",
        title: "connect.timed_out",
        body: "reason=\(reason) id=\(peripheralID.uuidString)"
      )
      self.central?.cancelPeripheralConnection(peripheral)
    }
    connectTimeoutWorkItem = workItem
    DispatchQueue.main.asyncAfter(deadline: .now() + Self.connectionAttemptTimeout, execute: workItem)
  }
  ...
  func beginAutoPhysiologyDiscovery(reason: String) {
    ...
    updateReconnectState("scanning for device physiology")
