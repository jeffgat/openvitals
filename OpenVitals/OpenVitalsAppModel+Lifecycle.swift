...
extension OpenVitalsAppModel {
  ...
  func startMovementPacketValidationTest(timeout: TimeInterval = 45) {
    ...
    guard ble.connectionState == "ready" else {
      movementPacketValidationStatus = "Connect device first. Current state: \(ble.connectionState)"
      movementPacketValidationIsRunning = false
    ...
    movementPacketValidationStatus = "Listening for real movement packets"
  ...
  func beginOvernightGuardCriticalBackgroundTask(reason: String) {
    ...
    let taskName = "OpenVitals Overnight \(reason)"
  ...
  func expireOvernightGuardCriticalBackgroundTask() {
    ...
    overnightGuardStatus = "Background time expired during \(reason); keep OpenVitals foregrounded if possible"
