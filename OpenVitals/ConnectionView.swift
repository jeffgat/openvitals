  ...
  var body: some View {
    List {
      ...
      Section("Scan Diagnostics") {
        if ble.scanDiagnostics.isEmpty {
          Text("No advertisements captured")
            .foregroundStyle(.secondary)
        } else {
          ForEach(ble.scanDiagnostics) { diagnostic in
            VStack(alignment: .leading, spacing: 4) {
              HStack {
                Text(diagnostic.name)
                  .font(.subheadline.weight(.semibold))
                Spacer()
                Text("RSSI \(diagnostic.rssi)")
                  .font(.caption)
                  .foregroundStyle(.secondary)
              }
              Text(diagnostic.status)
                .font(.caption)
                .foregroundStyle(diagnostic.status.hasPrefix("compatible") ? .green : .secondary)
      ...
      Section("Client Hello") {
        Text(OpenVitalsHello.clientHelloFrameHex)
