...
extension MoreDataStore {
  ...
  nonisolated static func rawValidationRunbookMarkdown(
    ...
    throw NSError(
      domain: "OpenVitalsRawValidationRunbook",
      code: 1,
  ...
  static func previewDefault() -> MoreDataStore {
    let store = MoreDataStore(databasePath: "/tmp/open-vitals-preview.sqlite")
  ...
  static func previewConnected() -> MoreDataStore {
    ...
    store.liveCaptureStatus = "Ready; notifications are mirrored through the app BLE client"
