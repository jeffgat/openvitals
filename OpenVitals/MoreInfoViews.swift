...
struct MoreAppearanceView: View {
  ...
  private var selectedPreference: OpenVitalsAppearancePreference {
    OpenVitalsAppearancePreference.preference(for: appearancePreferenceRaw)
  }
  ...
  var body: some View {
    List {
      Section("Color Scheme") {
        ...
        ForEach(OpenVitalsAppearancePreference.allCases) { preference in
          MoreAppearanceOptionRow(
      ...
      Section("Current") {
        MoreInfoRow(
          title: "Mode",
          value: selectedPreference.detail,
          systemImage: selectedPreference.systemImage,
          status: .ready
        )
      }
    }
    ...
    .onAppear {
      model.recordUIAction("page.opened", detail: "More Appearance")
    }
  }
...
private struct MoreAppearanceOptionRow: View {
  let preference: OpenVitalsAppearancePreference
  let selected: Bool
  let action: () -> Void
    ...
    List {
      Section {
        HStack(alignment: .center, spacing: 12) {
          OpenVitalsLogoMark(size: 52, cornerRadius: 10)

          VStack(alignment: .leading, spacing: 4) {
            Text("OpenVitals")
              .font(.headline)
            Text("Local-first health and wearable tools")
              .font(.caption)
