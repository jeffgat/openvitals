...
extension HealthDataStore {
  ...
  static func referenceComparisonStatus(from report: [String: Any]) -> String {
    ...
    let openVitals = report["open_vitals_algorithm_id"] as? String ?? "openVitals"
    let reference = report["reference_algorithm_id"] as? String ?? "reference"
    return "benchmark-only \(status) | \(deltas) deltas | \(openVitals) vs \(reference)"
  }
  ...
  static func dailyTrend(
    ...
    return HealthTrendModel(
      ...
      analysis: points.isEmpty
        ? "No trusted packet-derived values have been captured for this trend yet."
        : "Packet-derived daily values from locally captured device frames.",
      resources: resources,
      points: points
    )
  }
  ...
  static func restingHeartRateRollupTrend(
    ...
    let analysis = analysisParts.isEmpty
      ? "Packet-derived daily resting HR from locally captured device heart-rate samples."
      : "Packet-derived daily resting HR from locally captured device heart-rate samples. \(analysisParts.joined(separator: " | "))."
  ...
  static func energyRollupTrend(
    ...
    return HealthTrendModel(
      ...
      analysis: "Local device-derived calorie estimate from packet heart-rate and motion features. Active \(active) kcal | resting \(resting) kcal | total \(total) kcal | \(covered) covered minutes | confidence \(confidence).",
      resources: daily.resources,
      points: daily.points
    )
  }
  ...
  static func energyMetricTrend(
    ...
    return HealthTrendModel(
      ...
      analysis: "Stored daily device-derived \(label) from daily_activity_metrics. Latest \(latestText) kcal | avg \(averageText) kcal | \(deltaText) kcal vs avg | source \(sourceKind) | confidence \(confidence).",
      resources: daily.resources,
      points: daily.points
    )
  }
  ...
  static func stepMetricTrend(
    ...
    switch sourceKind {
    case "device_counter":
      sourceText = "decoded device step counter"
    case "local_estimate":
    ...
    return HealthTrendModel(
      ...
      analysis: "Stored daily device-derived step metric from \(sourceText). Steps \(steps) | confidence \(confidence)\(cadence).",
      resources: daily.resources,
      points: daily.points
    )
  }
