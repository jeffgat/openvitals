...
extension OpenVitalsAppModel {
    ...
    if !overnightGuardActive {
      if overnightGuardExportURL != nil {
        if overnightGuardExportStatus.localizedCaseInsensitiveContains("validation issues")
          ...
          return ("stale", "Final bundle saved with validation issues | inspect before opening other device apps")
        }
        if !inactiveWarnings.isEmpty {
          return ("stale", "Final bundle saved with proof warning | \(inactiveWarnings.joined(separator: ", ")) | inspect before opening other device apps")
        }
        return ("ready", "Final bundle ready | AirDrop before opening other device apps")
      }
      if overnightGuardCanExportLastSession {
        if !inactiveWarnings.isEmpty {
          return ("stale", "Guard stopped with proof warning | \(inactiveWarnings.joined(separator: ", ")) | export and inspect before opening other device apps")
        }
        return ("pending", "Guard stopped or recovered | export before opening other device apps")
      }
  ...
  func refreshOvernightWatchdogState(reason: String) {
    ...
    if snapshot.notificationCount == 0, elapsed >= Self.overnightGuardRawStaleWarningInterval {
      ...
      let warning = "No raw BLE notifications after \(minutes)m; confirm the device is connected and OpenVitals is foregrounded if possible."
  ...
  func applyOvernightRawSpoolWarning(
    ...
    if let lastError = snapshot.lastError {
      overnightGuardRawSpoolWarning = "Raw spool warning: \(lastError). Inspect proof sidecars before opening other device apps."
  ...
  func applyOvernightBLELogFlushIssues(_ issues: [String], reason: String) {
    if issues.isEmpty {
      ...
      overnightGuardBLELogWarning = "BLE log warning: \(issueText). Inspect proof sidecars before opening other device apps."
