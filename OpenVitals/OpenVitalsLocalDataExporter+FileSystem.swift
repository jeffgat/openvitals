...
extension OpenVitalsLocalDataExporter {
  static func validateSQLiteDatabase(
    ...
    guard exists else {
      issues.append("missing open_vitals.sqlite")
      return
    }
    if !included {
      issues.append("open_vitals.sqlite was not included in export")
    }
    ...
    do {
      ...
      if !storageCheckPassed {
        ...
        if storageIssues.isEmpty {
          issues.append("open_vitals.sqlite storage check failed")
        } else {
          issues.append("open_vitals.sqlite storage check failed: \(storageIssues.joined(separator: "; "))")
        }
      }
    } catch {
      openable = false
      storageCheckPassed = false
      issues.append("open_vitals.sqlite could not be opened: \(errorSummary(error))")
    }
  }
  ...
  static func defaultDatabasePath() -> String {
    ...
    return baseDirectory
      ...
      .appendingPathComponent("open_vitals.sqlite")
      .path
  }
  ...
  static func includedBLELogURLs(
    ...
    var urls: [URL] = []
    if pathSet.contains("Application Support/OpenVitals/open-vitals-ble.log"),
       let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
      urls.append(
        appSupport
          .appendingPathComponent("OpenVitals", isDirectory: true)
          .appendingPathComponent("open-vitals-ble.log")
      )
    }
    if pathSet.contains("Documents/OpenVitals/open-vitals-ble-live.log") {
      urls.append(
        documentsDirectory
          .appendingPathComponent("OpenVitals", isDirectory: true)
          .appendingPathComponent("open-vitals-ble-live.log")
      )
    }
    return urls
  }
  ...
  static func errorSummary(_ error: Error) -> String {
    if case let OpenVitalsRustBridgeError.methodFailed(message) = error {
      return message
    }
  ...
  static func exportRoots(fileManager: FileManager, documentsDirectory: URL) -> [(label: String, url: URL)] {
    ...
    if let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
      roots.append(("Application Support/OpenVitals", appSupport.appendingPathComponent("OpenVitals", isDirectory: true)))
    }
    roots.append(("Documents/OpenVitals", documentsDirectory.appendingPathComponent("OpenVitals", isDirectory: true)))
  ...
  static func shouldIncludeFile(
    ...
    switch label {
    case "Application Support/OpenVitals":
      return relativePath == "open_vitals.sqlite"
        || relativePath == "open_vitals.sqlite-wal"
        || relativePath == "open_vitals.sqlite-shm"
        || relativePath == "open-vitals-ble.log"
        || relativePath.hasPrefix("open-vitals-ble.")
    case "Documents/OpenVitals":
      return relativePath == "open-vitals-ble-live.log"
