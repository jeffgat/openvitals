...
extension OpenVitalsAppModel {
  func recoverUncleanOvernightGuardSessionIfNeeded() {
    ...
    do {
      ...
      overnightGuardWarning = "Previous overnight guard did not close cleanly; last heartbeat \(Self.overnightRecoveredStatusSummary(recovered)). Export OpenVitals before opening other device apps."
  ...
  static func finalizeRecoveredOvernightGuardSessionForExport(
    ...
    guard var manifest = readJSONObject(at: manifestURL) else {
      throw NSError(
        domain: "OpenVitalsOvernightGuard",
        code: 1,
    ...
    let readinessText = (recoveredSummary["readiness"] as? String)
      ...
      ?? "Recovered unclean overnight guard | export before opening other device apps"
    recoveredSummary["readiness_status"] = readinessStatusText
    recoveredSummary["readiness"] = readinessText
    recoveredSummary["warning"] = "Recovered unclean overnight guard. Export before opening other device apps."
    ...
    let statusLines = [
      ...
      "crash_marker=\(crashMarkerURL.path)",
	      ...
	      "warning=Recovered unclean overnight guard. Export before opening other device apps.",
	    ]
