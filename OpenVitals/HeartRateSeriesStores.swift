...
final class HRVSeriesStore {
  ...
  static let didUpdateNotification = Notification.Name("OpenVitalsHRVSeriesStoreDidUpdate")
  ...
  private static func loadPersistedLiveSample() -> HRVSamplePoint? {
    ...
    guard defaults.object(forKey: "open_vitals.swift.liveHRVRMSSD") != nil else {
      return nil
    }
    let rmssd = defaults.double(forKey: "open_vitals.swift.liveHRVRMSSD")
    let rrIntervalCount = defaults.integer(forKey: "open_vitals.swift.liveHRVRRIntervalCount")
    let source = defaults.string(forKey: "open_vitals.swift.liveHRVSource") ?? "ble.hr.standard.average.migrated"
    let capturedAt = defaults.object(forKey: "open_vitals.swift.liveHRVUpdatedAt") as? Date ?? Date()
...
final class HeartRateSamplePipeline {
  ...
  private let queue = DispatchQueue(label: "com.open_vitals.swift.heart-rate-sample-pipeline", qos: .utility)
