...
struct MoreCaptureView: View {
  ...
  @EnvironmentObject private var messageStore: OpenVitalsMessageStore
  ...
  private func icon(for level: OpenVitalsLogLevel) -> String {
    switch level {
    case .debug: "terminal"
  ...
  private func status(for level: OpenVitalsLogLevel) -> MoreStatusKind {
    switch level {
  ...
  var body: some View {
    List {
      ...
      if !store.databaseExists {
        Section("Empty State") {
          MoreInfoRow(title: "No Database Yet", value: "The app has not created the local database at this path. Capture or bridge-backed flows can create it.", systemImage: "tray", status: .unavailable)
        }
      }
    }
    .openVitalsListBackground()
  ...
  var body: some View {
    List {
      ...
      Section("Adapter") {
        ...
        MoreInfoRow(title: "Existing OpenVitals Records", value: store.existingOpenVitalsRecordsStatus, systemImage: "externaldrive", status: .pending)
