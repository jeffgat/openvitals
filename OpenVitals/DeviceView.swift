...
private struct DeviceAdvancedPanel: View {
  ...
  @ObservedObject var model: OpenVitalsAppModel
...
private struct DeviceActionGrid: View {
  ...
  var body: some View {
    LazyVGrid(columns: columns, spacing: 10) {
      ...
      DeviceActionButton(title: "Reset", systemName: "xmark.circle") {
        ble.resetConnection()
      }
      .disabled(!ble.canResetConnection)
...
private struct EventLogPreview: View {
  let messages: [OpenVitalsMessage]
...
private let deviceScreenBackground = OpenVitalsTheme.appBackground
