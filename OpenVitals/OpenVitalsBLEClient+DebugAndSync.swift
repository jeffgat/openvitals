...
extension OpenVitalsBLEClient {
  ...
  func completeDebugCommand(
    ...
    let response = OpenVitalsDebugCommandResponse(
      id: UUID(),
    ...
    let level: OpenVitalsLogLevel = status == "ok" ? .info : .warn
    record(
      level: level,
  ...
  func debugCommandDefinitionPayload(_ command: OpenVitalsDebugCommandDefinition) -> [String: Any] {
    [
      "id": command.id,
      "title": command.title,
  ...
  func debugCommandResponsePayload(_ response: OpenVitalsDebugCommandResponse) -> [String: Any] {
    [
