...
1. The Swift app owns login, health-data consent, token storage, and all WHOOP/OpenVitals data access.
...
3. Coach data access is exposed through a small read-only tool registry backed by `OpenVitalsRustBridge` and the existing Rust store/metric methods.
...
- Swift already has a JSON-over-C bridge into Rust via `OpenVitalsRustBridge.request(...)`, and the app model already parses live BLE frames through Rust.
...
2. Add a `OpenVitalsCoachDataProvider` that wraps `OpenVitalsRustBridge`, owns the app database path, calls existing bridge methods, and redacts raw outputs.
