# OpenVitals Swift MVP: Home

Source map: Flutter `TodayView`, Flutter `OpenVitalsShell` Overview tab, Swift `AppShellView` Home tab, Swift `DeviceView`.
...
- [ ] Define a `HomeSnapshot` value type that can be populated from `OpenVitalsAppModel`, `OpenVitalsBLEClient`, and Rust/local store calls.
